...
learning_poverty_df <-wbstats::wb(country="all", #by specifying all, we can get a list of all aggregates (such as region or income level)
    ...
    filter(as.numeric(date)==max(as.numeric(date))) %>% #group by country to create one observation per country which is the latest value
      mutate(tab = 0 , 
           ...
           ID = case_when(iso3c %in% c(country_file_name,region_label,income_label) ~ iso3c),
           grx = SE.LPV.PRIM/100)
```
...
lp_region <- round(learning_poverty_df$`SE.LPV.PRIM`[learning_poverty_df$iso3c == region_label], digits = 0)
lp_category <- round(learning_poverty_df$SE.LPV.PRIM[learning_poverty_df$iso3c == income_label], digits = 0)
