...
### **KEY TAKEAWAYS, \uppercase{`r country`}, `r year`** ###  
`r takeaway`
 ...
 input <- input %>%
   ...
   #Adding new cell with value and colour in new column
    mutate(value_color=case_when(
      #percentages
      (value >= lays_world & (Series == "HD.HCI.LAYS")) ~ paste("\\cellcolor{green!10}",value, sep=""),
      (value <= lays_world & (Series == "HD.HCI.LAYS")) ~ paste("\\cellcolor{red!10}",value, sep=""),
      (value >=80 &            (grepl(grep_color, Series) | grepl('Percent', Indicator.Name))) ~ paste("\\cellcolor{green!10}",value,sep=""),
      (value >=60 & value<80 & (grepl(grep_color, Series) | grepl('Percent', Indicator.Name))) ~ paste("\\cellcolor{yellow!10}",value, sep=""),
      (value <60 &             (grepl(grep_color, Series) | grepl('Percent', Indicator.Name))) ~ paste("\\cellcolor{red!10}",value, sep=""),
      #non-percentages
      (value >=4 & value<=5 &  !(grepl(grep_color, Series) | grepl('Percent', Indicator.Name))) ~ paste("\\cellcolor{green!10}",value, sep=""),
      (value >=3 & value<4 &   !(grepl(grep_color, Series) | grepl('Percent', Indicator.Name))) ~ paste("\\cellcolor{yellow!10}",value, sep=""),
      (value <3 &              !(grepl(grep_color, Series) | grepl('Percent', Indicator.Name))) ~ paste("\\cellcolor{red!10}",value, sep="")
    )
    ) %>%
   #Adding new cell with only colour in new column
    mutate(only_color=case_when(
      #percentages
      (value >=80 &            (grepl(grep_color, Series) | grepl('Percent', Indicator.Name))) ~ paste("\\cellcolor{green!10}"),
      (value >=60 & value<80 & (grepl(grep_color, Series) | grepl('Percent', Indicator.Name))) ~ paste("\\cellcolor{yellow!10}"),
      (value <60 &             (grepl(grep_color, Series) | grepl('Percent', Indicator.Name))) ~ paste("\\cellcolor{red!10}"),
      #non-percentages
      (value >=4 & value<=5 &  !(grepl(grep_color, Series) | grepl('Percent', Indicator.Name))) ~ paste("\\cellcolor{green!10}"),
      (value >=3 & value<4 &   !(grepl(grep_color, Series) | grepl('Percent', Indicator.Name))) ~ paste("\\cellcolor{yellow!10}"),
      (value <3 &              !(grepl(grep_color, Series) | grepl('Percent', Indicator.Name))) ~ paste("\\cellcolor{red!10}")
    )
    )
lays_lp_choice <- if_else(country_file_name == "RWA", lays, lp)
lays_lp_units <- if_else(country_file_name == "RWA", lays_units, lp_units)
lays_lp_cell <- if_else(country_file_name == "RWA", input$value_color[input$Series=="HD.HCI.LAYS"],
...
\cellcolor{outcome}\multirow{-5}{*}{\rotatebox{90}{\textcolor{white}{Outcomes}}} &  Participation and learning  & `r lays_lp_choice` & {`r lays_lp_cell`}\\\cdashline{1-4}
...
\end{table}
\raggedbottom
```{r figure_details, include=FALSE, message=FALSE, echo=FALSE, warning=FALSE}

grade4_overall <- round(input$value[input$Series=="SE.PRM.LERN"], digits = 1)

figure_headline <- if_else(country_file_name == "RWA",
...
LP_para <- paste("Learning poverty is defined as the share of children at end of primary age below minimum reading proficiency, adjusted for out of school children. Learning poverty in ", country, " is ", diff_lp_regiontext, " than the average for ", region_label, " region and ", diff_lp_cattext, " than the average for ", income_label, " countries. ", sep = "")
...
lern_para <- if_else(country_file_name == "RWA", LAYS_para, LP_para)

figure_head <- if_else(country_file_name == "RWA", glue("Figure 2. Learning adjusted years in school comparison"),
                                                   glue("Figure 2. Leaning poverty comparison"))

figure_note <- if_else(country_file_name == "RWA",
                       glue("Grey circles represent other countries. Yellow circle represents {country}. Orange and blue circles represent average LAYS in {country}'s region and income group."),
...
figure <- if_else(country_file_name == "RWA", "lays_figure_", "lp_figure_")
figure2 <- paste(here(), "/LP figures/", figure , country_file_name, ".png", sep="")
...
grade4_overall <- round(input$value[input$Series=="SE.PRM.LERN"], digits = 0)
...
GEPD grade 4 assessment proficiency is defined as \% students scoring greater than 80/100 on student knowledge. Overall GEPD student proficiency in `r country` is `r grade4_overall`%. Student proficiency in language is `r lern_subject_text` in language compared to numeracy, `r lern_gender_text` for boys compared to girls, and `r lern_rururb_text` in urban areas compared to rural areas.
\vfill\null
...
```{r lern_teach, fig.width=5, fig.height=1.5, message=FALSE, echo=FALSE, warning=FALSE}
 ...
 input_lern %>%
    ...
    coord_flip()+
    labs(y = "GEPD- Grade 4 proficiency %") +
...
efft_text <- if_else(teach_efft<60, glue("({teach_efft}%) needs improvement"), if_else(teach_efft>=60 & teach_efft<80, glue("({teach_efft}%) requires caution"), glue("({teach_efft}%) is on target")))
...
Quality of school inputs and infrastrucutre is affected by physical infrastructure standards set in policies, and strength of school monitoring systems. Basic inputs `r inpt_text`. `r inpt_lowest_var_name`(`r inpt_lowest_varval`)% is the lowest score. Basic infrastructure `r infr_text`. `r infr_lowest_var_name`(`r infr_lowest_varval`)%  is the lowest score. `r inpt_policy_lowest_name` policy lever scores the lowest(`r inpt_policy_lowest_varval`/5).
...
defacto_diff <- defacto_master_tbl %>%
    ...
    arrange(desc(diff))
average_diff <- round(mean(abs(defacto_diff$diff)), digits=1)
...
defacto_master_tbl %>%
  ...
  SE.PRM.SATT_plot_1 <- ggplot(data= subset(defacto_subind_final, grepl('SATT.2', Series)), aes(x=label,y=value)) +
      geom_bar(stat="identity", fill= "#002244") +
      geom_text(aes(label= round(value, digits =1),hjust=-0.25, size=3)) +
...
\multirow{-4}{2cm}{Quality of bureaucracy (1-5)} & Motivation and attitudes &`r input$value_color[input$Series=='SE.PRM.BQBR.5']` \\\hline
...
\multirow{-4}{2cm}{Impartial decision making (1-5)} & Employee unions as facilitators &`r input$value_color[input$Series=='SE.PRM.BIMP.5']` \\\hline
...
\multirow{-3}{2cm}{Mandates and accountability (1-5)} & Public official accountability &`r input$value_color[input$Series=='SE.PRM.BMAC.4']` \\\hline
...
\multirow{-4}{2cm}{National learning goals (1-5)} & Community engagement &`r input$value_color[input$Series=='SE.PRM.BNLG.5']` \\\hline
...
\multirow{-4}{2cm}{Financing (Scale 0-1)} & Equity &`r input$value[input$Series=='SE.PRM.BFIN.5']` \\\hline
\end{tabular}}
\\
\greynote{Financing sub-indicators are on a 0-1 scale, the other sub-indicators are on 0-5 scale.}
\end{table}
\raggedbottom
