...
mutate(value = if_else((value <0 | is.na(value)), as.numeric(NA), value))
```
 ...
 input <- input %>%
   ...
   #Adding new cell with value and colour in new column
    mutate(value_color=case_when(
      (value >= lays_world & (Series == "HD.HCI.LAYS")) ~ paste("\\cellcolor{green!15}",value, sep=""),
      (value <= lays_world & (Series == "HD.HCI.LAYS")) ~ paste("\\cellcolor{red!15}",value, sep=""),
...
\cellcolor{outcome} &                             & Proficiency by End of Primary  & {`r input$value_color[input$Series=="SE.LPV.PRIM.BMP"]`\%} \\\cdashline{3-4}
\cellcolor{outcome} &                             & Proficiency on GEPD Assessment & {`r input$value_color[input$Series=="SE.PRM.LERN"]`\%} \\\cdashline{2-4}
\cellcolor{outcome} &  Participation              & Net Adjusted Enrollment Rate   & {`r input$value_color[input$Series=="SE.PRM.TENR"]`\%} \\\cdashline{2-4}
\cellcolor{outcome}\multirow{-5}{*}{\rotatebox{90}{\textcolor{white}{Outcomes}}} &  Participation and learning  & Learning adjusted years of schooling (in years) & {`r input$value_color[input$Series=="HD.HCI.LAYS"]`\%} \\\cdashline{1-4}
...
\multirow{2}{*}{Teaching} & Teacher Effort      & {`r input$value_color[input$Series=="SE.PRM.EFFT.1.M"]`\%} & {`r input$value_color[input$Series=="SE.PRM.EFFT.1.F"]`\%} \\\cdashline{2-4}
...
\multirow{2}{*}{Teaching} & Teacher Effort                 & {`r input$value_color[input$Series=="SE.PRM.EFFT.1.R"]`\%} & {`r input$value_color[input$Series=="SE.PRM.EFFT.1.U"]`\%} \\\cdashline{2-4}
...
\end{table}
\raggedbottom
```{r figure_details, echo=FALSE, message = FALSE, warning = FALSE}

grade4_overall <- round(input$value[input$Series=="SE.PRM.LERN"], digits = 1)

figure_headline <- glue("LEARNING OUTCOMES: {lays_num_country} LEARNING ADJUSTED YEARS IN SCHOOL, {grade4_overall}% GEPD PROFICIENCY IN GRADE 4")

LAYS_para <- paste("Learning adjusted years of school (LAYS) is calculated by adjusting expected years of schooling for schooling quality. Learning adjusted years of schooling in ",country," is ", diff_lays_regiontext, " than the average for ", region_label, " region and ", diff_lays_inctext, " than the average for " , income_label, " countries.")
                     
figure_head <- glue("Figure 2. Learning adjusted years in school comparison")

figure_note <- glue("Grey circles represent other countries. Yellow circle represents {country}. Orange and blue circles represent average LAYS in {country}'s region and income group.")
```
...
\multicolumn{2}{c}{\textbf{Practice Indicators}} & \multicolumn{2}{c}{\textbf{Policy levers (Teaching)}}\\\hline
                                                 & `r input$only_color[input$Series=="SE.PRM.CONT"]`                      & Attraction                                & `r input$value_color[input$Series=="SE.PRM.TATT"]` \\\cdashline{3-4}
\multirow{-2}{*}{Content knowledge}              & \multirow{-2}{*}{`r input$value_color[input$Series=="SE.PRM.CONT"]`\%} & Selection \& deployment                   & `r input$value_color[input$Series=="SE.PRM.TSDP"]` \\\cdashline{1-4}
\tabindent\emph{Maths proficiency}               & {`r input$value_color[input$Series=="SE.PRM.CONT.3"]`\%}               & Support                                   & `r input$value_color[input$Series=="SE.PRM.TSUP"]`\\\cdashline{1-4}
\tabindent\emph{Language proficiency}               & {`r input$value_color[input$Series=="SE.PRM.CONT.2"]`\%}            & Evaluation                                & `r input$value_color[input$Series=="SE.PRM.TEVL"]`\\\cdashline{1-4}
...
\end{tabular}}
\\
\setstretch{0.8}\color{darkgray}\scriptsize{\textit{Notes:} Content knowledge(\& sub-indicators) indicate \% teachers with knowledge\textgreater{80\%}.}
\end{table}
...
lowest_diff_var_text <- glue("{defacto_diff[11,1]} ({defacto_diff[11,4]} points)")
seclow_diff_var_text <- glue("{defacto_diff[10,1]} ({defacto_diff[10,4]} points)")
thirdlow_diff_var_text <- glue("{defacto_diff[9,1]} ({defacto_diff[9,4]} points)")
```
