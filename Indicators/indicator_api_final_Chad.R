...
# wbopendat<-wbstats::wb(country="TCD", 
#             indicator=ind_list,
#             startdate=2015,
#             enddate=2015,
#             return_wide = T,
...
wbopendat<-WDI(country='TD', indicator=ind_list, start=2000, end=2022, extra=T) %>%
  fill(starts_with("SE.")) %>%
  group_by(iso3c) %>%
  arrange(year) %>%
  filter(row_number()==n())
...
TCD_data_2023 <- api_metadata_fn('TCD', 2023)


#export Indicators_metatdata section
write_excel_csv(TCD_data_2023, paste( 'GEPD_Indicators_API_TCD.csv',sep=""))

write_excel_csv(TCD_data_2023, paste(data_dir,'/Indicators/', 'GEPD_Indicators_API_TCD.csv',sep=""))
