...
if (str_to_lower(Sys.getenv("USERNAME")) == "wb469649" ){
  ...
  backup_onedrive="no"
  # save_folder_onedrive <- file.path(paste("C:/Users/wb631589/WBG/Ezequiel Molina - Dashboard (Team Folder)/Country_Work/",country_name,year,"Data/clean/School", sep="/"))
} else {
...
teacher_gender <- read_dta("C:/Users/wb631589/OneDrive - WBG/GEPD-Confidential/General/LEGO_Teacher_Paper/5_output_data/SLE/SLE_teacher_level_updated.dta") 
...
#fixing one school with an extra digit in the school code
school_dta <- school_dta %>%
  mutate(school_code = if_else(school_code == 2105012235, 210501223, school_code))
...
#for now, given that schools under 230802220 definetely look like different schools, try to group them together. all other teachers seem to be in the same group too
school_dta <- school_dta %>%
  mutate(school_code = if_else(school_code == 230802220  & m1s0q2_emis != 230802220 & school_emis_preload == 230502211, 
                               230502211, school_code))

#dropping this interview id since it is duplicated in module 1 and it is unclear where the information is coming from
school_dta <- school_dta %>%
  filter(interview__id != "e5ff01ee838441eb92ee6e53655c3d6f") %>%
  filter(interview__id != "7f0f513fb8a84d3cadf967da5895b5f2")

#this survey is clearly from 230201208, adjust accordingly
school_dta <- school_dta %>%
  mutate(school_code = if_else(interview__id == "c6397979d549412083c5e1c299c1aaa0", 230201208, school_code))
         
#this school is clearly 420516207 , adjust accordingly
school_dta <- school_dta %>%
  mutate(school_code = if_else(
    interview__id == "1293a8ee163446db8c26712ccc48539d" | interview__id == "9ffb0cd82df445638486498d2540296c",
    420516207,
    school_code
  ))


#this schools is 3110202824, adjust accordingly
school_dta <- school_dta %>%
  mutate(school_code = if_else(interview__id == "fa2c2061a99747a4a9e056e2f269bc06", 3110202824 , school_code))
...
#this one school seems to have multiple (3) teachers with the same name, remove 1 of them who teaches the same grade
teacher_roster <- teacher_roster %>%
  filter(!(school_code == 420101207 & TEACHERS__id == 4))
...
#this school has duplicated teachers, drop them
teacher_questionnaire <- teacher_questionnaire %>%
  filter(!(school_code == 129103265 & is.na(m3sb_tnumber)))

#same here 
teacher_questionnaire <- teacher_questionnaire %>%
  filter(!(school_code == 220103210  & is.na(m3sb_tnumber)))
...
final_indicator_data_EFFT_micro <- teacher_absence_dta %>%
  select( -starts_with('interview'), -starts_with('enumerator'), -c('teacher_name'))
...
#I believe this ones should be change in this manner to be consistent with the other implementations, though not completely sure
teacher_pedagogy <- teacher_pedagogy %>%
  mutate(across(
    c(s_0_1_2, s_0_2_2, s_0_3_2),
    ~ case_when(
      . == 1 ~ 2,
      . == 2 ~ 3,
      . == 3 ~ 4,
...
# missing schools
teacher_pedagogy_missings <- school_data_preamble_temp %>% mutate(GRADE=4) %>%
  bind_rows(school_data_preamble_temp %>% mutate(GRADE=2)) %>%
  left_join(teacher_pedagogy %>% group_by(school_code, GRADE) %>% summarise(number_of_segments=n())) %>%
  mutate(data_missing=if_else(is.na(number_of_segments),'Yes','No')) %>%
  select(colnames(school_data_preamble_temp), data_missing, everything()) %>%
  arrange(school_code, GRADE)
  

write_excel_csv(teacher_pedagogy_missings,paste0(download_folder,"/Teach_scores_missing_status.csv"))
...
no_match <- teacher_pedagogy_segments %>%
  filter(is.na(school_name_preload)) 
  
teacher_pedagogy_segments <- teacher_pedagogy_segments %>%  
    filter(!is.na(school_name_preload)) 
  
  write_excel_csv(teacher_pedagogy_segments, paste(confidential_folder, "teach_score_raw.csv", sep="/"))
  ...
  teacher_pedagogy_segments <- teacher_pedagogy_segments %>%
    mutate(classroom_culture=as.numeric(rowMeans(select(.,s_a1, s_a2))),
           instruction=as.numeric(rowMeans(select(.,s_b3, s_b4, s_b5, s_b6))),
           socio_emotional_skills=as.numeric(rowMeans(select(.,s_c7, s_c8, s_c9)))
    ) 
  ...
  final_indicator_data_PEDG <- teacher_pedagogy_segments %>%
    mutate(teach_score=as.numeric((classroom_culture+instruction+socio_emotional_skills)/3),
...
#drop the duplicates, just randomly drop one of them because it is unclear which one is correct
teacher_assessment_dta <- teacher_assessment_dta %>% 
  filter(interview__key != "54-41-57-31")
...
num_boys <- school_dta %>%
  ...
  #correcting some data entry typos (first based on the other values for this same school, not sure how to make sense of the second one since the number of girls is greater than total)
  mutate(m4scq13_girls = ifelse(interview__key == "84-61-00-99" & m4scq13_girls == 127, 12, m4scq13_girls),
         m4scq13_girls = ifelse(interview__key == "55-47-26-50", NA, m4scq13_girls))
...
assess_4th_grade_dta <- assess_4th_grade_dta %>%
  ...
  select(school_code, fourth_grade_assessment__id, m8saq2_id, m8saq3_id, m8sbq1_number_sense, m8s1q1, m8s1q2, m8s1q3)
...
#seem to be duplicates, randomly drop one of them 
ecd_dta <- ecd_dta %>%
  filter(!(interview__key == "04-98-31-80" & m6s1q1 == "ABIBATU USMAN KOROMA"))
...
#   select(school_code, m5_teach_count, m5_teach_count_math, m5_teach_count_literacy, m5s2q1c_number, m5s2q1e_number, m5s1q1f_grammer ) 

#teacher_assessment_math_pknw and teacher_assessment_language_pknw
...
#############################################
  ...
  # For salary, based GDP per capita from 2020 World Bank  https://data.worldbank.org/indicator/NY.GDP.PCAP.PP.CD?locations=SL.  #VF changing this to the value I am getting from the stata api + changing some of the values because they seem to be reported before devaluation (1000)
  
  school_data_SATT <- school_data_SATT %>%
    mutate(principal_satisfaction=attitude_fun_rev_5(m7shq1_satt),
           m7shq2_satt = if_else(m7shq2_satt >= 100000, m7shq2_satt/1000, m7shq2_satt),
           principal_salary=if_else((m7shq2_satt==999 | m7shq2_satt == 0), as.numeric(NA),12*m7shq2_satt/12082.8	)) %>%
    mutate(
...
g4_stud_weights <-school_dta %>%
  ...
  mutate(g4_stud_weight_temp = g4_stud_count/g4_assess_count, 
         ...
         #it looks like for one of the schools we do not have any information on the teachers, assign them the weight of 1
...
g1_stud_weights<-school_dta %>%
  ...
  mutate(g1_class_weight = g1_teacher_count/1,
         ...
         #one school with missing weights, set these weights to 1
...
# 
# teacher_assessment_weights <-teacher_assessment_weights %>%
#   select(school_code,  numEligible4th_manual) %>%
#   group_by(school_code) %>%
#   summarise(numEligible4th_manual=max(numEligible4th_manual, na.rm=T)) %>%
#   mutate(teacher_weight_component=if_else(numEligible4th_manual>=5,
#                                           numEligible4th_manual/5,
#                                           1),
#          teacher_obs_weight_component=numEligible4th_manual) %>%
#   select(school_code, teacher_weight_component,teacher_obs_weight_component)
# 
...
data_set_updated <- read_csv(paste(sampling_folder, '/school_weights_', currentDate,  '.csv', sep="")
                             ) %>%
  select(-internet, -electricity) %>%
