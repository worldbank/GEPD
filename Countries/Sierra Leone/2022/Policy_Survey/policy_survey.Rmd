...
expert_dta_teachers_final <- expert_dta_teachers_final %>%
  mutate(evaluation_law=read_var(A10),
         ...
         evaluation_criteria=4, #changing it manually to be representative of what we have in the data
         negative_evaluations=1, #changing it manually to be representative of what we have in the data
         positive_evaluations=1) %>% #changing it manually to be representative of what we have in the data
...
expert_dta_school_management_final <- expert_dta_school_management_final %>%
  mutate(principal_monitor_law=read_var(C6),
         principal_monitor_criteria=4) %>% #changing it to what we actually have in the data 
...
expert_dta_learners_final <- expert_dta_learners_final %>%
  mutate(immunization=read_var(D6),
         ...
         deworming=0, #adjusting it to what we actually have in the data
  ...
  mutate(health_programs=1+(immunization + healthcare_young_children + antenatal_skilled_delivery))
...
expert_dta_learners_final <- expert_dta_learners_final %>%
  mutate(pre_primary_free_some=read_var(D10),
         ...
         ece_qualifications=4, #based on the answers we have
