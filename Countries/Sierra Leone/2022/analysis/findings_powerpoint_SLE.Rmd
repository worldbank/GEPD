---
title: "Global Education Policy Dashboard - Sierra Leone 2022"
...
svymeantab <- function(data, strata) {
  ...
  get(data) %>%
    ...
    select(one_of(indicators_list), province, private, urban_rural, ipw ) %>%
    pivot_longer(cols=one_of(indicators_list),
                 names_to = 'indicators',
                 values_to='value') %>%
    as_survey_design(strata=strata,
                     weight=ipw) %>%
    select(-c( 'province', 'private', 'urban_rural', 'ipw')) %>%
    group_by(indicators) %>%
...
# School Survey
    ...
    sumstats_school_df <- svymeantab('sumstats_school', strata=c('province', 'private', 'urban_rural'))
...
#public/private
      ...
      #add variable label
      sumstats_school_nonprivate_df <-  svymeantab('sumstats_school_nonprivate', strata=c('province', 'private', 'urban_rural')) %>%
        rename(
          mean_urban=mean,
          ci_urban=ci
        )
      ...
      #add variable label
      sumstats_school_private_df <-  svymeantab('sumstats_school_private', strata=c('province', 'private', 'urban_rural')) %>%
        rename(
          mean_rural=mean,
          ci_rural=ci
        )
...
#     plot_annotation(title = str_wrap('Differences in Dashboard Indicators Between Urban/Rural Schools',80))
      ...
      sumstats_school_north <- school_dta_short_anon %>%
        filter(idregion=="NORTHERN") 




      #add variable label
      sumstats_school_north_df <-  svymeantab('sumstats_school_north', strata=c('province', 'private', 'urban_rural')) %>%
        rename(
          mean_urban=mean,
          ci_urban=ci
        )
      ...
      sumstats_school_east <- school_dta_short_anon  %>%
        filter(idregion=="EASTERN")

 
      sumstats_school_east_df <-  svymeantab('sumstats_school_east', strata=c('province', 'private', 'urban_rural'))   %>%
        rename(
          mean_rural=mean,
          ci_rural=ci
        )
      ...
      sumstats_school_south <- school_dta_short_anon  %>%
        filter(idregion=="SOUTHERN") 


      
      sumstats_school_south_df <-  svymeantab('sumstats_school_south', strata=c('province', 'private', 'urban_rural')) %>%
        rename(
          mean_south=mean,
          ci_south=ci
        )
...
#west
      sumstats_school_west <- school_dta_short_anon  %>%
        filter(idregion=="WESTERN") 


      

      #add variable label
      sumstats_school_west_df <-  svymeantab('sumstats_school_west', strata=c('province', 'private', 'urban_rural')) %>%
        rename(
          mean_west=mean,
          ci_west=ci
        )
...
#northwest
      sumstats_school_nwest <- school_dta_short_anon  %>%
        filter(idregion=="NORTH WESTERN") 



      #add variable label
      sumstats_school_nwest_df <- svymeantab('sumstats_school_nwest', strata=c('province', 'private', 'urban_rural')) %>% 
        rename(
          mean_nwest=mean,
          ci_nwest=ci
        )
      ...
      sumstats_school_df_final <- sumstats_school_df %>%
        ...
        left_join(sumstats_school_nwest_df)
      ...
      sumstats_public_officials_df <- svymeantabpo('public_officials_dta_clean_anon') %>%
        mutate(
               ...
               ci_south=as.character(NA),
               mean_west=as.numeric(NA),
               ci_west=as.character(NA),
               mean_nwest=as.numeric(NA),
               ci_nwest=as.character(NA)) %>%
        select(varlabel, mean, ci, mean_urban, ci_urban, mean_rural, ci_rural, mean_south, ci_south,  mean_west, ci_west, mean_nwest, ci_nwest )
  ...
  sumstats_df %>%
    ...
    # )
    # ) %>%
    kable(caption = "Summary Statistics of Dashboard Indicators - Sierra Leone 2022 - Region",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
          , "Mean", "95% Confident Interval", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval" , "Mean", "95% Confident Interval"),
          digits = 1,
          escape=F) %>%
    kable_styling("striped") %>%
    add_header_above(c(" "=1, "Overall"=2,"North"=2,"East"=2,"South"=2 ,"West"=2,"North West"=2))
...
main_indicator_labels<-c( 
    ...
    expert_dta_final<-haven::read_dta("c:/users/wb577189/OneDrive - WBG/GEPD/CNT/SLE/SLE_2022_GEPD/SLE_2022_GEPD_v01_M/Data/Policy_Survey/expert_dta_final.dta") 
