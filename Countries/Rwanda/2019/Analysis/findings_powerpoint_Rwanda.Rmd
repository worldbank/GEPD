...
learning_data_plot2 <- final_indicator_data_LERN_anon %>%
  ...
  select(-Female, -Male) %>%
  mutate(Type=if_else(m8_language==1, "Kinyarwanda Tested School", "English Tested School"))


learning_dataMedian <- summarise(group_by(learning_data_plot2, Gender , Type), MD = round(wtd.mean(literacy_student_knowledge, ipw, na.rm=T),1))
