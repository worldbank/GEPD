---
title: "Mozambique Public Officials Analysis"
author: "Brian Stacy"
date: "`r Sys.Date()`"
output:
...
work_dir<- "//wbgfscifs01/GEDEDU/datalib-edu/projects/GEPD-Confidential/CNT/MOZ/MOZ_2019_GEPD/MOZ_2019_GEPD_v02_RAW/Data/confidential/"

#load public official data
load(paste(work_dir, "Public_Officials/public_officials_indicators_data.RData", sep=""))

#load school data
school_dta_short<-read_dta(paste('//wbgfscifs01/GEDEDU/datalib-edu/projects/GEPD-Confidential/CNT/MOZ/MOZ_2019_GEPD/MOZ_2019_GEPD_v02_RAW/Data/SDI 2018/', "school_indicators_data.dta", sep=""))

#load set of districts that match to schools
office_list <- read_csv("//wbgfscifs01/GEDEDU/datalib-edu/projects/GEPD-Confidential/CNT/MOZ/MOZ_2019_GEPD/MOZ_2019_GEPD_v02_RAW/Data/sampling/district_sample_10region_18ugel2019-10-31.csv") %>%
  mutate(District=if_else(District=="Homo�ne", "Homoine", District )) %>% #fix a few district names
  mutate(District=if_else(District=="Alto Mol�cu�", "Alto Molocue", District )) 
  
#convert names of office list to lowercase
names(office_list) <- tolower(names(office_list))
...
public_officials_agg <- public_officials_dta_clean %>%
  ...
  mutate(office_preload=str_remove(office_preload,pattern='DPEDH|SDEJT')) %>%
  mutate(office_preload=str_trim(office_preload)) %>%
  mutate(office_preload=str_to_title(office_preload)) %>% # do some formatting of office names
...
#rename offices to match those in original province list
#first form regular expression with list of provinces
if (exists('province_regex')) {
  rm('province_regex')
}


for (i in office_list$province) {
  if (!exists('province_regex')) {
  province_regex<-i
  } else {
  province_regex<-paste(province_regex, i, sep="|")
  }
}
...
public_officials_agg <- public_officials_agg %>%
  left_join(office_list,  by=c('office_preload'='district')) %>%
  mutate(province=if_else(govt_tier!='Regional office (or equivalent)',
                          province,office_preload)) %>%
  # mutate(school_province_preload=if_else(govt_tier=='Regional office (or equivalent)',
  # str_trim(str_extract(office_preload,paste('\\b(',province_regex,')\\b', sep=""))),
...
#get list of schools that have a province match
school_dta_matched <- school_dta_short %>%
  filter(province %in% office_list$province) %>%
...
public_offical_link_key <- public_officials_agg %>%
  select(office_preload, province)
...
#select data just for Mozambique
fc_Mozambique <- fc[fc@data$ADM0_NAME=="Mozambique",]



#plot base map of Mozambique with labels for subnational units.  http://rstudio.github.io/leaflet/popups.html

region_regex<-c('North', 'South', 'East','West', 'Kigali City')

maps <- fortify(fc_Mozambique, region="ADM1_NAME") %>%
...
#collapse to province level for mapping
...
acm<-  ggplot() +
  ...
  theme_bw() +
    xlab('Longitude') + 
    ylab('Latitude') + 
    # ggtitle(str_wrap('Map Survey of Public Officials Scores for Mandates and Accountability by Mozambiquen Districs', width=50)) +
...
patch_plot + plot_annotation(
  ...
  caption = 'Regional shading shows average of all public officials in that province including officials at Directorate level'
)
```
...
merge_df_collapse<-school_dta_matched %>%
  mutate(province=school_province_preload) %>%
  group_by(province) %>%
...
patch_scatter + plot_annotation(
  ...
  caption = str_wrap('Regional average of all public officials in that province including officials at Directorate level',150)
)
...
write_dta(merge_df_collapse, path="C:/Users/wb469649/OneDrive - WBG/Dashboard - School - Public Officials - Confidential/Countries/Mozambique/Data/public_officials_school_merged_df_collapsed.dta")

#rename some data frames for saving  
merge_df_collapse_Mozambique<-  merge_df_collapse
top_50_table_Mozambique<-top_50_table
sumstats_df_Mozambique<-sumstats_df
maps_Mozambique<-maps 
school_df_collapse_Mozambique<-school_df_collapse 
po_df_collapse_Mozambique<-po_df_collapse
public_officials_dta_clean_Mozambique <- public_officials_dta_clean
school_dta_matched_Mozambique <- school_dta_matched


save(list=c('merge_df_collapse_Mozambique', 'top_50_table_Mozambique', 'sumstats_df_Mozambique','maps_Mozambique','school_df_collapse_Mozambique','po_df_collapse_Mozambique',
            'public_officials_dta_clean_Mozambique','school_dta_matched_Mozambique' ), 
     file = file.path("C:/Users/wb469649/OneDrive - WBG/Dashboard - School - Public Officials - Confidential/Countries/Mozambique/Data/public_officials_school_merged_files.RData"))
