...
server_add<- svDialogs::dlgInput("Please Enter Server http Address:", 'http://ec2-18-212-6-69.compute-1.amazonaws.com/')$res
...
POST(paste(server_add,"/api/v1/export/stata/dd5d031dba1e4333b4ff5dfc7bff4e2d$",quest_version,"/start",approval, sep=""),
         authenticate(user, password))

#sleep for 10 seconds to wait for stata file to compile
Sys.sleep(10)

dataDownload <- GET(paste(server_add,"/api/v1/export/stata/dd5d031dba1e4333b4ff5dfc7bff4e2d$", quest_version,"/",approval,sep=""),
                    authenticate(user, password))
...
public_officials_dta<-read_dta(file.path(download_folder, "public_officials_final.dta")) 
