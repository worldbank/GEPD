...
#Overall, we draw a sample of 300 public schools from each of the regions of 
#Ethiopia. As a comparison to the total number of schools in Ethiopia, 
#this consistutes an approximately 1% sample. Because of the large size of 
#the country, and because there can be very large distances between Woredas
#within the same region, we chose a cluster sampling approach. In this approach,
#100 Woredas were chosen with probability proportional to 4th grade size. 
#Then within each Woreda two rural and one urban school were chosen with 
...
df_weights_function <- function(dataset,scode, snumber, prov) {
  ...
  dataset %>%
    ...
    mutate(province=Region,
           ...
           ipw_new = sample/count, 
           ipw_new_new = 1/ipw_new)
}
...
if (same_sample == 1) {
  ...
  save(list=c(anon_dta_list,'metadta','indicators'), file = file.path("C:/Users/wb631589/OneDrive - WBG/HEDGE Files - GEPD/CNT/ETH/ETH_2021_GEPD/ETH_2021_GEPD_v02_M/Data/School/same_sample_school_indicators_data_anon.RData"))
} else {
  save(list=c(anon_dta_list,'metadta','indicators'), file = file.path(save_folder, "school_indicators_data_anon.RData"))
  save(list=c(anon_dta_list,'metadta','indicators'), file = file.path("C:/Users/wb631589/OneDrive - WBG/HEDGE Files - GEPD/CNT/ETH/ETH_2021_GEPD/ETH_2021_GEPD_v02_M/Data/School/school_indicators_data_anon.RData"))
}