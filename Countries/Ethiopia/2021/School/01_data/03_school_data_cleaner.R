...
school_dta <- school_dta %>%
  filter(interview__key != "80-52-17-42") %>% #duplicated school code, keeping the other observation because there are more teachers on the roster there
  filter(interview__key != "35-38-09-87") # drop because the location does not match with what we have in the sampling frame, so keeping the one where gps matches 

#there are duplicated modules in this survey, keep the one that was conducted later
school_dta <- school_dta %>%
  mutate(across(
    starts_with("m1"),
    ~ if_else(interview__key == "56-61-08-56", NA, .x)  
  ))
...
numeligible <- teacher_roster %>%
  mutate(eligible = pmax(m2saq7__1, m2saq7__2, m2saq7__3, m2saq7__4, na.rm = TRUE)) %>%
...
# #clean up duplicates in terms of teacher id numbers before collapsing
# teacher_absence_dta <-teacher_absence_dta %>%
#   group_by(interview__key) %>%
#   mutate(counter = if_else(
#     interview__key == "80-52-17-42",      
#     row_number() + 46,       
#     NA_integer_              
# )) %>%
#   ungroup()

#drop the repeated observation
teacher_absence_dta <- teacher_absence_dta %>%
  filter(interview__key != "56-61-08-56")  

# teacher_absence_dta <-teacher_absence_dta %>%
#   group_by(interview__key) %>%
#   mutate(counter = if_else(
#     interview__key == "35-38-09-87",      
#     row_number() + 6,       
#     NA_integer_              
#   )) %>%
#   ungroup()

# #for the ones where we need to replace the ids, replace them now
# teacher_absence_dta <- teacher_absence_dta %>%
#   mutate(teacher_number = if_else(!is.na(counter), counter, teacher_number),
#          TEACHERS__id = if_else(!is.na(counter), counter, TEACHERS__id))
...
school_data_INPT <- school_data_INPT %>%
  filter(interview__key != "80-52-17-42") %>% #duplicated school code, keeping the other observation because there are more teachers on the roster there
  filter(interview__key != "35-38-09-87")
...
num_boys <- school_dta %>%
  ...
  mutate(m4scq13_girls = ifelse(m4scq13_girls == 98, NA, m4scq13_girls),
         #the numbers in this school do not make a lot of sense
         m4scq13_girls = if_else(school_code == 209933, NA, m4scq13_girls))
...
final_indicator_data_OPMN <- school_data_OPMN %>%
  ...
  mutate(vignette_2_resp=if_else(m7scq1_opmn==98, 0, 0.5), # no one responsible that is known
    vignette_1=vignette_1_resp+vignette_1_finance+vignette_1_address,
...
final_indicator_data_TINM_micro <- teacher_questionnaire_TINM %>%
  filter(interview__key != "80-52-17-42") %>% #duplicated school code, keeping the other observation because there are more teachers on the roster there
  filter(interview__key != "35-38-09-87") %>%
...
final_indicator_data_TINM <- final_indicator_data_TINM_micro %>%
   group_by(school_code) %>%
   summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
...
school_data_SATT <- school_data_SATT %>%
  ...
  mutate(principal_satisfaction=attitude_fun_rev_5(m7shq1_satt),
         m7shq2_satt = if_else(m7shq2_satt == 0, NA, m7shq2_satt),
...
school_data_SEVL<- school_data_SEVL %>%
  ...
  mutate(principal_evaluation_mult_dummy = case_when(
    m7sgq8_sevl == 1 & principal_eval_tot >= 2 ~ 1,
    m7sgq8_sevl == 1 & principal_eval_tot <= 1 ~ 0,
    m7sgq8_sevl == 0 ~ 0,
    TRUE ~ NA_real_
  ),
    principal_evaluation_multiple=if_else(m7sgq8_sevl==1,
                                               case_when(
...
g4_stud_weights <-school_dta %>%
  ...
  mutate(g4_stud_weight_temp = g4_stud_count/g4_assess_count, 
         ...
         #some classes did not have any information on the number of students, assign them the weight of 1
...
teacher_assessment_weights <-teacher_assessment_weights %>%
  ...
  mutate(numEligible4th_manual = if_else(teacher_quest_count>numEligible4th_manual & !is.na(teacher_quest_count), teacher_quest_count, numEligible4th_manual),
         ...
         teacher_obs_weight_component=numEligible4th_manual) %>% #9 201450
...
#for now assuming that the ones with the missing weights are the ones that were observed in 2021, so dropping them
final_indicator_data_PEDG <- final_indicator_data_PEDG %>%
  filter(!is.na(teacher_obs_weight_component))
...
if (same_sample == 1) {
  write.csv(final_school_data, file = file.path(confidential_folder, "same_sample_final_complete_school_data.csv"))
  #write_dta(final_school_data, path = file.path(confidential_folder, "final_complete_school_data.dta"), version = 14)
  write.csv(school_weights, file = file.path(confidential_folder, "same_sample_school_weights.csv"))
...
if (backup_onedrive=="yes") {
  if (same_sample == 1) {
    write.csv(final_school_data, file = file.path(save_folder_onedrive, "same_sample_final_complete_school_data.csv"))
  } else {
...
for (i in indicator_names ) {
  if (exists(paste("final_indicator_data_",i,"_micro", sep=""))) {
    ...
    write.csv(temp, file = file.path(paste(confidential_folder,"/Indicators", sep=""), paste(i, same_sample, "_final_indicator_data.csv", sep="")))
    if (backup_onedrive=="yes") {
      write.csv(temp, file = file.path(paste(save_folder_onedrive,"/Indicators", sep=""), paste(i, same_sample, "_final_indicator_data.csv", sep="")))
      #}
    }
  }
}
...
data_list <- c(ind_dta_list, 'school_dta', 'school_dta_short', 'school_dta_short_imp', 'school_data_preamble', 'final_school_data', 'teacher_questionnaire','teacher_absence_final', 'ecd_dta', 'teacher_assessment_dta', 'teacher_roster', 
               "indicators", 'school_metadta', 'school_gdp', 'assess_4th_grade_anon', 'ecd_dta_anon', 'school_weights')

if (same_sample == 1) {
  save(list=data_list, file = file.path(confidential_folder, "same_sample_school_survey_data.RData"))
  save(list=c(ind_dta_list,"school_dta_short", 'school_dta_short_imp', "indicators", 'school_metadta',  'school_gdp' ), file = file.path(confidential_folder, "same_sample_school_indicators_data.RData"))
} else {
  save(list=data_list, file = file.path(confidential_folder, "school_survey_data.RData"))
  save(list=c(ind_dta_list,"school_dta_short", 'school_dta_short_imp', "indicators", 'school_metadta',  'school_gdp' ), file = file.path(confidential_folder, "school_indicators_data.RData"))
