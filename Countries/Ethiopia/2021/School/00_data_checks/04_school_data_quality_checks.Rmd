...
bscols(widths=c(3,NA),
       list(
         filter_slider("Time", "Time Section Completed", linked_t_df, ~timestamp_utc),
       ...
       list (
         plot_ly(linked_t_df, x=~section, y=~timestamp_utc, type='scatter', mode='markers', color=~responsible) %>%
...
sample_frame_name <- paste("C:/Users/WB469649/WBG/Ezequiel Molina - Dashboard (Team Folder)/Country_Work/Ethiopia/2020/Data/Sampling/school_sample_",currentDate,".RData", sep="")
...
school_dta_short <- school_dta_short %>%
  mutate(Admin_Code=school_emis_preload) %>%
  left_join(data_set_updated) %>%
  mutate(school_ipw=sum(grd4_total)/grd4_total)
...
indicator_labels<-c("4th Grade Student Knowledge (Fraction Correct)", 
                    ...
                    "Teacher Content Knowledge Proficiency", 
