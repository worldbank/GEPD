...
ind_list<-c('student_knowledge', 'math_student_knowledge', 'literacy_student_knowledge', 
            ...
            'teacher_attraction', 'teacher_satisfied_job', 'teacher_satisfied_status', 'better_teachers_promoted' ,'teacher_bonus', 'salary_delays', 'salary_delays_indicator', 
...
#reconstruct numeligible that is used for absence
numeligible_absence <- teacher_roster_dta %>%
  mutate(eligible = ifelse((s_m2aa_04 == 6), 1, 0), #teacher in the contract status
          eligible = if_else(s_m2aa_06 != 1, 0, eligible), #excluding part-time
           eligible = if_else(s_m2aa_05 == 5, 0, eligible), #excluding volunteers
         school_code = s_ecole_code,
         numEligible = eligible) %>%
  group_by(school_code) %>%
  dplyr::summarize(numEligible = sum(numEligible)) 


#get the data for numEligible that is later used for the weights (for the assessment and the questionnaire)
numeligible <- teacher_roster_dta %>%
  mutate(eligible = pmin(s_m2aa_08aa, s_m2aa_08ab, s_m2aa_08ac, s_m2aa_08ad, s_m2aa_08ae, 
                         s_m2aa_08ba, s_m2aa_08bb, s_m2aa_08bc, s_m2aa_08bd, s_m2aa_08be, na.rm = TRUE)) %>%
  mutate(eligible_subject = pmin(s_m2aa_07a, s_m2aa_07b, s_m2aa_07c)) %>% #keep the teachers that taught either math or languages in at least one of these grades 
  mutate(eligible = ifelse(eligible == 2, 0, eligible),
         eligible_subject = ifelse(eligible_subject == 2, 0, eligible_subject),
         eligible = ifelse(eligible_subject == 0 & eligible == 1, 0, eligible),
  #Mirror what we had as an eligibility criteria on SS
         eligible = if_else(s_m2aa_06 != 1, 0, eligible),
         eligible = if_else(s_m2aa_05 == 5, 0, eligible)) %>%
  group_by(school_code) %>%
...
#try to merge in the data from the roster with the data from the questionnaire to see if we can get the grades
teacher_roster_dta_selected <- teacher_roster_dta %>%
  select(s_m2aa_08ad, s_m2aa_08bd, s_m2aa_08aa,
         s_ecole_code, s_m2aa_02c, s_m2aa_02n, school_code, s_m2aa_09a)

teacher_roster_dta_selected <- teacher_roster_dta_selected %>%
  mutate(new_name = paste0("(", s_m2aa_02c, ") ", s_m2aa_02n))

teacher_questionnaire_dta_raw <- teacher_questionnaire_dta_raw %>%
  mutate(new_name = s_m2ba1_01n)

#we have 1497 observations in the raw questionnaire file and I managed to bring in 1469!
merged <- left_join(teacher_questionnaire_dta_raw, teacher_roster_dta_selected, by = c("school_code", "new_name")) %>%
  mutate(teacher_grd4 = ifelse((s_m2aa_08ad == 1), 1, 0),
         teacher_grd1 = ifelse((s_m2aa_08aa == 1), 1, 0))

teacher_questionnaire_weights <- merged %>%
  group_by(school_code) %>%
  dplyr::summarize(g4_teacher_count = sum(teacher_grd4),
            g1_teacher_count = sum(teacher_grd1))

teacher_questionnaire_weights_1 <- merged %>%
  group_by(school_code) %>%
  dplyr::summarize(teacher_quest_count = n())

# #to get some kind of estimate for the number of students in the 1st grade, I am using the data from question D_ M2bA2_02 and D_ M2bA2_03 just for the first grade teachers
# merged_1st_grade <- merged %>%
#   select(school_code, d_m2ba2_02, s_m2ba2_03, teacher_grd1)
...
#get the ungrade version too
teacher_assessment_dta_ungraded <- read_dta(paste0(download_folder, '/22-M6.dta')) %>% #read in the corrected version of the teacher assessment
  select(-starts_with('s_m2')) %>% #drop some information that may conflict with primary school file.  Just match on school code
  left_join(school_dta_preamble) %>%
  filter(s_type_ecole==3) #just keep dashboard schools
...
# Recode ECD variables to be 1 if student got it correct and zero otherwise
  first_grade<- first_grade %>%
    ...
    mutate_at(vars(ends_with("comprehension"),
                   ...
                   ends_with("conflict_resol")), ~bin_var_3(.,1)  ) %>% #any score other than 1 is 0
   mutate(across(ends_with("head_shoulders"), ~if_else(.x==2 | .x==1,1,0,missing=NULL))) %>%
    mutate(across(ends_with("vocabn"), ~case_when(.x==97 ~ 0,
                                                    .x==99 ~ 0,
                                                    .x==77 ~ 0,
                                                    (.x!=97 & .x!=99 & .x>=10) ~ 1,
                                                    (.x!=97 & .x!=99 & .x<10) ~ as.numeric(.x)/10,
                                                    is.na(.x) ~ 0))) %>%
    mutate(across(ends_with("counting"), ~case_when(.x==97 ~ 0,
                                                      .x==99 ~ 0,
                                                      .x==77 ~ 0,
                                                      (.x!=97 & .x!=99 & .x>=30) ~ 1,
...
skim(ecd_dta_anon, where(is.numeric))
...
```{r g4}
 ...
 ## 8. Cleaning : 4th grade Level ----  
  ...
  assess_4th_grade_dta_weights <- fourth_grade_raw %>%
    group_by(s_ecole_code) %>%
  ...
  ## NOTE - no question on the lonely giraffe from the SDI team
    ...
    #           ~call_out_scorer(.,0.8)) %>%
    #given that the circling questions already have the number of correct circlies, what we want to do here to make them comparable with other countries is to divide by 3 (total maximum number of correct answers), so that 1 is the maximum point for each of the questions
    mutate(m8saq2_id = m8saq2_id/3,
           m8saq3_id = m8saq3_id/3,
           m8sbq1_number_sense = m8sbq1_number_sense/3) %>%
    ungroup() %>%
...
final_indicator_data_ATTD <- read_dta(paste0(download_folder, '/18-M5_autres_infos.dta')) %>%
  ...
  mutate(student_attendance=if_else(s_m4ab_01>m4seq00_fin, m4seq00_fin/s_m4ab_01,as.numeric(student_attendance))) %>%
...
teacher_absence_dta <- teacher_absence_dta %>%
  mutate(
    sch_absence_rate_1st=100*case_when(
      s_m2ab1a_02==1 ~ as.numeric(NA) , #excuse teachers that didn't have a shift that day
      s_m2ab1a_01==5 ~ 1,
      s_m2ab1a_01!=5   ~ 0,
    ...
    sch_absence_rate_2nd=100*case_when(
      s_m2ab1b_02==1   ~ as.numeric(NA), #excuse teachers that didn't have a shift that day
      s_m2ab1b_01==5   ~ 1,
      s_m2ab1b_01!=5   ~ 0,
...
teacher_absence_final<- teacher_absence_dta %>%
  select(preamble_info, preamble_info_absence, contains('absence'), contains('presence'), s_m2aa_02n) %>%
...
teacher_assessment_dta_graded<- teacher_assessment_dta_ungraded %>%
  filter(consent==1) %>% #keep those who consented
  mutate(
      #    TASK 1 EXERCICES 1 & 2: GRAMMAR
	    m5s1q1a_grammer=case_when(
      	 (m6sa1q2aa==2 & m6sa1q2ab==1) ~ 1,
      	 (m6sa1q2ab==1) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s1q1b_grammer=case_when(
      	 (m6sa1q2ba==2 & m6sa1q2bb==4) ~ 1,
      	 (m6sa1q2bb==4) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s1q1c_grammer=case_when(
      	 (m6sa1q2ca==1) ~ 1,
      	 (m6sa1q2cb==1) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s1q1d_grammer=case_when(
      	 (m6sa1q2da==1) ~ 1,
      	 (m6sa1q2db==1) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s1q1e_grammer=case_when(
      	 (m6sa1q2ea==2 & m6sa1q2eb==2) ~ 1,
      	 (m6sa1q2eb==2) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s1q1f_grammer=case_when(
      	 (m6sa1q2fa==2 & m6sa1q2fb==4) ~ 1,
      	 (m6sa1q2fb==4) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s1q2a_cloze=case_when(
      	 (m6sa1q3aa==1) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s1q2b_cloze=case_when(
      	 (m6sa1q3ba!=2) ~ 0,
      	 TRUE ~ 1
    	 ),
      m5s1q2c_cloze=case_when(
      	 (m6sa1q3ca==1) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s1q2d_cloze=case_when(
      	 (m6sa1q3da==1) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s1q2e_cloze=case_when(
      	 (m6sa1q3ea!=2) ~ 0,
      	 TRUE ~ 1
    	 ),
      m5s1q2f_cloze=case_when(
      	 (m6sa1q3fa==1) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s1q2g_cloze=case_when(
      	 (m6sa1q3ga!=2) ~ 0,
      	 TRUE ~ 1
    	 ),
      m5s1q2h_cloze=case_when(
      	 (m6sa1q3ha!=2) ~ 0,
      	 TRUE ~ 1
    	 ),
      m5s1q2i_cloze=case_when(
      	 (m6sa1q3ia!=2) ~ 0,
      	 TRUE ~ 1
    	 ),
      m5s1q2j_cloze=case_when(
      	 (m6sa1q3ja==1) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s1q2k_cloze=case_when(
      	 (m6sa1q3ka==1) ~ 1,
      	 TRUE ~ 0
    	 ),
      #math (First two questions differ from traditional dashboard survey)
      m5s2q1c_number=case_when(
      	 (m6sa2q1ca ==1) ~ 1,
      	 (m6sa2q1cb ==685) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q1d_number=case_when(
      	 (m6sa2q1da ==2 & m6sa2q1db ==8) ~ 1,
      	 (m6sa2q1db ==8) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q1e_number=case_when(
      	 (m6sa2q1ea ==2 & m6sa2q1eb ==481) ~ 1,
      	 (m6sa2q1eb ==481) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q2_number=case_when(
      	 ((m6sa2q2b==1 & m6sa2q2c==5) | (m6sa2q2b==5 & m6sa2q2c==1)) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q3_number=case_when(
      	 (m6sa2q3a==1) ~ 1,
      	 (m6sa2q3b==1) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q4a_number=case_when(
      	 (m6sa2q4aa==1) ~ 1,
      	 (m6sa2q4ab=="27" | m6sa2q4ab=="9/27") ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q4b_number=case_when(
      	 (m6sa2q4bb=="/20" | m6sa2q4bb=="08/20" | m6sa2q4bb=="8/20" | m6sa2q4bb=="820" | m6sa2q4bb=="=8/20") ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q5_number=case_when(
      	 (m6sa2q5a==2 & m6sa2q5b==1) ~ 1,
      	 (m6sa2q5b==1) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q6_geometric=case_when(
      	 (m6sa2q6a==1) ~ 1,
      	 (m6sa2q6b==1) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q7_geometric=case_when(
      	 (m6sa2q7a==2 & m6sa2q7b==2) ~ 1,
      	 (m6sa2q7b==2) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q8_data=case_when(
      	 (m6sa2q8a==1) ~ 1,
      	 (m6sa2q8b=="7H20" | m6sa2q8b=="0720") ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q9a_data=case_when(
      	 (m6sa2q9ab==47) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q9b_data=case_when(
      	 (m6sa2q9bb==3) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q10a_data=case_when(
      	 (m6sa2q10ab==110) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q10b_data=case_when(
      	 (m6sa2q10bb==60) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q11a_number=case_when(
      	 (m6sa2q11aa==1) ~ 1,
      	 (m6sa2q11ab==12) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q11b_number=case_when(
      	 (m6sa2q11ba==1) ~ 1,
      	 (m6sa2q11bb=="0,32" |  m6sa2q11bb=="0.32" |  m6sa2q11bb=="00,32" |  m6sa2q11bb=="032") ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q11c_number=case_when(
      	 (m6sa2q11cb=="12/14" |  m6sa2q11cb=="24 28" |  m6sa2q11cb=="24/28" |  m6sa2q11cb=="6/7") ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q12_number=case_when(
      	 (m6sa2q12a==2 & m6sa2q12b==4) ~ 1,
      	 (m6sa2q12b==4) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q13a_geometric=case_when(
      	 (m6sa2q13aa==2 & m6sa2q13ab==30) ~ 1,
      	 (m6sa2q13ab==30) ~ 1,
      	 TRUE ~ 0
    	 ),
      m5s2q13b_geometric=case_when(
      	 (m6sa2q13ba==2 & m6sa2q13bb==54) ~ 1,
      	 (m6sa2q13bb==54) ~ 1,
      	 TRUE ~ 0
    	 )
  )
  ...
  #calculate # of literacy items
  #note: grep is a tool for searching text with specified pattern.  In our case, we are looking for m5s1q, which is the prefix of literacy items
  teacher_assessment_dta_graded$literacy_length<-length(grep(x=colnames(teacher_assessment_dta_graded), pattern="m5s1q"))
  
  
  lit_items<-colnames(teacher_assessment_dta_graded[,grep(x=colnames(teacher_assessment_dta_graded), pattern="m5s1q")])
  
  #calculate teachers lit items correct
  teacher_assessment_dta_graded <- teacher_assessment_dta_graded %>%
    mutate(literacy_content_knowledge=100*rowMeans(.[grep(x=colnames(teacher_assessment_dta_graded), pattern="m5s1q")], na.rm=TRUE),
           correct_letter=100*rowMeans(.[grep(x=colnames(teacher_assessment_dta_graded), pattern="m5s1q3")], na.rm=TRUE),
           cloze=100*rowMeans(.[grep(x=colnames(teacher_assessment_dta_graded), pattern="m5s1q2")], na.rm=TRUE),
           grammar=100*rowMeans(.[grep(x=colnames(teacher_assessment_dta_graded), pattern="m5s1q1")], na.rm=TRUE),
           read_passage=100*rowMeans(.[grep(x=colnames(teacher_assessment_dta_graded), pattern="m5s1q4")], na.rm=TRUE)) #there is no such item in the test in madagascar
  
  ####Math####
  #calculate # of math items
  teacher_assessment_dta_graded$math_length<-length(grep(x=colnames(teacher_assessment_dta_graded), pattern="m5s2q"))
  
  math_items<-colnames(teacher_assessment_dta_graded[,grep(x=colnames(teacher_assessment_dta_graded), pattern="m5s2q")])
  
  #calculate teachers math items correct
  teacher_assessment_dta_graded <- teacher_assessment_dta_graded %>%
    mutate(math_content_knowledge=100*rowMeans(.[grep(x=colnames(teacher_assessment_dta_graded), pattern="m5s2q")], na.rm=TRUE),
           arithmetic_number_relations=100*rowMeans(.[grep(x=colnames(teacher_assessment_dta_graded), pattern="number")], na.rm=TRUE),
           geometry=100*rowMeans(.[grep(x=colnames(teacher_assessment_dta_graded), pattern="geometric")], na.rm=TRUE),
           interpret_data=100*rowMeans(.[grep(x=colnames(teacher_assessment_dta_graded), pattern="data")], na.rm=TRUE))
  
  #get the data for principal school knowledge
  #literacy
teacher_assessment_language_pknw <-teacher_assessment_dta %>%
  select(literacy_content_knowledge, school_code, m5s1q1f_grammer) %>%
  filter(!is.na(literacy_content_knowledge)) %>%
  group_by(school_code) %>%
  dplyr::summarize(share_sentence_correct = mean(m5s1q1f_grammer, na.rm = F))

#math
teacher_assessment_math_pknw <-teacher_assessment_dta %>%
  select(math_content_knowledge, school_code, m5s2q1c_number, m5s2q1e_number) %>%
  filter(!is.na(math_content_knowledge)) %>%
  group_by(school_code) %>%
 ...
 hist(teacher_assessment_dta_graded$literacy_content_knowledge)    
 hist(teacher_assessment_dta_graded$math_content_knowledge)  
 
   teacher_assessment_dta_graded <- teacher_assessment_dta_graded %>% 
    mutate(school_code = s_ecole_code)
 
  #calculate % correct for literacy, math, and total
  final_indicator_data_CONT <- teacher_assessment_dta_graded %>%
    select(-c("s_region_code"      ,
              "s_cisco_code" ,
              "s_commune_name",    
              "s_commune_code" ,   
              "s_fokontany_name",
  ...
  #calculate % correct for literacy, math, and total
  final_indicator_data_CONT_micro <- teacher_assessment_dta_graded %>%
    select(-c("s_region_code"      ,
              "s_cisco_code" ,
              "s_commune_name",    
              "s_commune_code" ,   
...
#    group_by(s_ecole_code) %>%
    add_count(s_ecole_code,name='m5_teach_count') %>%
    add_count(s_ecole_code,name='m5_teach_count_literacy') %>%
    add_count(s_ecole_code,name='m5_teach_count_math') %>%
  mutate(m5_teach_count_math= if_else(!is.na(math_content_knowledge), as.numeric(m5_teach_count_math), as.numeric(NA)),
         m5_teach_count_literacy = if_else(!is.na(literacy_content_knowledge), as.numeric(m5_teach_count_literacy), as.numeric(NA))) %>%
    mutate(content_knowledge=case_when(
      (!is.na(math_content_knowledge) & !is.na(literacy_content_knowledge)) ~ (math_content_knowledge+literacy_content_knowledge)/2,
      is.na(math_content_knowledge)  ~ literacy_content_knowledge,
      is.na(literacy_content_knowledge)  ~ math_content_knowledge)) %>%
 ...
 skim(final_indicator_data_CONT, one_of(ind_list))
  ...
  cont_var <- c("content_knowledge", "content_proficiency", "literacy_content_knowledge", "literacy_content_proficiency", "math_content_knowledge", "math_content_proficiency")
  
  #skim(final_indicator_data_CONT, one_of(cont_var)) %>% select(skim_variable, numeric.mean, numeric.p50) %>% rename(variable = skim_variable, mean = numeric.mean, median = numeric.p50) 
...
final_indicator_data_INPT <- school_data_INPT %>%
  ...
  mutate(inputs=1.25*textbooks+1.25*blackboard_functional + 1.25*pens_etc + 1.25*access_ict) %>%
...
```{r infr}
  ...
  infrastructure_dta <- infrastructure_dta %>%
    #
    mutate(drinking_water=if_else(d_m12c_08a==1, 1,0 ))
  
#   computers <- infrastructure_dta %>%
#     select(d_m1c_11a, s_ecole_code)
# 
# infrastructure_dta <- left_join(infrastructure_dta, computers, by = "s_ecole_code")
  ...
  final_indicator_data_INFR <- infrastructure_dta %>%
    ...
    mutate(internet=case_when(
      d_m1c_11c==1  ~ 1,
      d_m1c_11c==2   ~ 0,
      (is.na(as.numeric(d_m1c_11c)) & d_m1c_11a == 0) ~ 0
       ))
...
pknw_actual_exper_full <- teacher_questionnaire_dta_raw %>%
  add_count(s_ecole_code,name='m7_teach_count_pknw') %>%
  select(s_ecole_code, s_m2ba1_05, m7_teach_count_pknw ) %>%
  mutate(experience=(as.numeric(year)-s_m2ba1_05)) %>%
  group_by(s_ecole_code) %>%
  add_count(s_ecole_code, wt = !is.na(experience), name = "counter") 
...
pknw_actual_exper <- left_join(pknw_actual_exper_full, pknw_actual_exper_yes_less_than_3, by = "s_ecole_code")
...
pknw_actual_exper <- pknw_actual_exper %>%
  select(s_ecole_code, mean_experience_less3) %>%
  group_by(s_ecole_code) %>%
...
pknw_actual_school_inpts <- final_indicator_data_INPT %>%
  left_join(final_indicator_data_ATTD) %>%
  select(s_ecole_code, blackboard_functional, s_m4ab_04, s_m4ab_01, m4seq00_fin) %>%
  mutate(attendance = s_m4ab_01/m4seq00_fin)

teacher_assessment_math_pknw <- teacher_assessment_math_pknw %>%
  mutate(s_ecole_code = school_code)

teacher_assessment_language_pknw <- teacher_assessment_language_pknw %>%
  mutate(s_ecole_code = school_code)
...
pknw_guess_exper <- read_dta(paste0(download_folder, '/12-M3A_D_M3AF_10_EXP.dta')) %>%
  mutate(d_m3af_10_exp = ifelse(d_m3af_10_exp == 2, 0, d_m3af_10_exp)) %>%
  group_by(s_ecole_code) %>%
  dplyr::summarize(experience_pknw_shr=mean(d_m3af_10_exp))
...
max <- max(final_indicator_data_INPT$s_m4ab_01, na.rm = T)
...
final_indicator_data_PKNW <- management_dta %>%
  ...
  #given that the question seemed to have been asked hypothetically without showing the names and asking to think about the 5 teachers from the school, I am dividing each number given by a principal by 5 
  mutate(add_triple_digit_pknw_shr = m7sfq5_pknw/5, 
         multiply_double_digit_pknw_shr = m7sfq6_pknw/5,
         complete_sentence_pknw_shr = m7sfq7_pknw/5,
         #adjust the number of books guessed by a principal with attendance
         m7sfq10_pknw = m7sfq10_pknw*attendance,
        m7sfq10_pknw = if_else(m7sfq10_pknw > max_textbooks, NA, m7sfq10_pknw),
         m7sfq11_pknw = ifelse(m7sfq11_pknw == 2, 0, m7sfq11_pknw)) %>%
  mutate(
             ...
             textbooks_pknw=principal_scorer(m7sfq10_pknw, s_m4ab_01),
  ...
  select(s_ecole_code,principal_knowledge_score, principal_knowledge_avg, add_triple_digit_pknw, multiply_double_digit_pknw,complete_sentence_pknw, experience_pknw, textbooks_pknw, blackboard_pknw, m7sfq6_pknw, m7sfq7_pknw, m7sfq10_pknw, s_m4ab_04, s_m4ab_01,  starts_with('share_'), mean_experience_less3, ends_with('_shr'), m7sfq11_pknw, blackboard_functional)

final_indicator_data_PKNW <- final_indicator_data_PKNW %>%
  mutate(school_code = s_ecole_code)
  ...
  final_indicator_data_PMAN <- management_dta %>%
    ...
    mutate(
      problem_solving_proactive=case_when(
       ...
       (m7seq1_pman==1 | m7seq1_pman==98 | (!is.na(d_m3aa_09) & is.na(m7seq1_pman))) ~ 0
      ),
      ...
      problem_solving_stomach=case_when(
        ...
        (m7seq3_pman==98 | (!is.na(d_m3aa_09) & is.na(m7seq3_pman))) ~ 0 #changed this part. m7saq10 is used as an indicator for a principal interview. all other cases remain missing
       )
    ) %>%
...
final_indicator_data_ILDR_micro <- teacher_questionnaire_dta_raw %>%
  mutate(obs_date=ymd(paste(d_m2bd_16a, d_m2bd_16m,"01", sep="/")), #convert to time since interview
         obs_gap=ymd("2021/06/15")-obs_date,
         obs_gap_months=time_length(obs_gap, unit='month')) %>%
  rename(m3sdq15_ildr=d_m2bd_15,
...
```{r tatt}
 ...
 #############################################
  ...
  teacher_questionnaire_dta <- teacher_questionnaire_dta_raw %>%
    mutate(teacher_satisfied_job=attitude_fun_rev_5(d_m2be_01)/5,
           teacher_satisfied_status=attitude_fun_rev_5(d_m2be_02)/5,
           ...
           m3sbq1_tatt_2 = d_m2bb2_01b, 
           m3sbq1_tatt_2 = if_else(m3sbq1_tatt_2 == 2, 0, m3sbq1_tatt_2),
           salary_delays_indicator = if_else(salary_delays == 0, 0, 1)) %>%
 ...
 teacher_questionnaire_dta <- teacher_questionnaire_dta %>%
  ...
  final_indicator_data_TATT <- teacher_questionnaire_dta %>%
    ...
    group_by(s_ecole_code) %>%
    select(s_ecole_code, teacher_attraction,teacher_satisfied_job,teacher_satisfied_status,better_teachers_promoted,teacher_bonus, teacher_bonus_hard_staff, school_code, salary_delays, salary_delays_indicator, m3sbq1_tatt_2) %>%
    summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
  
  final_indicator_data_TATT_micro <- teacher_questionnaire_dta %>%
    mutate(n_mssing_TATT=n_miss_row(.)) %>%
    #group_by(s_ecole_code) %>%
    select(s_ecole_code, teacher_attraction,teacher_satisfied_job,teacher_satisfied_status,better_teachers_promoted,teacher_bonus, teacher_bonus_hard_staff,salary_delays, school_code, salary_delays_indicator, m3sbq1_tatt_2) 
    #summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
...
```{r tsdp}
 ...
 teacher_questionnaire_dta <- teacher_questionnaire_dta %>%
  ...
  final_indicator_data_TSDP <- teacher_questionnaire_dta %>%  
    ...
    group_by(s_ecole_code) %>%
    select(s_ecole_code, teacher_selection_deployment,teacher_selection,teacher_deployment, school_code) %>%
    summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
  
  final_indicator_data_TSDP_micro <- teacher_questionnaire_dta %>%  
    mutate(n_mssing_TSDP=n_miss_row(.)) %>%
    #group_by(s_ecole_code) %>%
    select(s_ecole_code, teacher_selection_deployment,teacher_selection,teacher_deployment, school_code)
    #summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
 ...
 hist(teacher_questionnaire_dta$teacher_support) 
  ...
  final_indicator_data_TSUP <- teacher_questionnaire_dta %>%
    ...
    select(s_ecole_code, teacher_support,pre_service,practicum,in_service,opportunities_teachers_share,
           ...
           in_service_exists,in_servce_lngth,in_service_classroom, school_code, contains('tsup')) %>%
    summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.)))  
  
  final_indicator_data_TSUP_micro <- teacher_questionnaire_dta %>%
    mutate(n_mssing_TSUP=n_miss_row(.)) %>%
    #group_by(s_ecole_code) %>%
    select(s_ecole_code, teacher_support,pre_service,practicum,in_service,opportunities_teachers_share,
           pre_training_exists,pre_training_useful,pre_training_practicum,pre_training_practicum_lngth,
...
#plot the indicator  
 hist(teacher_questionnaire_dta$teaching_evaluation)
  ...
  final_indicator_data_TEVL <- teacher_questionnaire_dta %>%
    ...
    group_by(s_ecole_code) %>%
    select(s_ecole_code, teaching_evaluation,formally_evaluated,evaluation_content,negative_consequences,positive_consequences, school_code) %>%
    summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
  
  final_indicator_data_TEVL_micro <- teacher_questionnaire_dta %>%
    mutate(n_mssing_TEVL=n_miss_row(.)) %>%
    #group_by(s_ecole_code) %>%
    select(s_ecole_code, teaching_evaluation,formally_evaluated,evaluation_content,negative_consequences,positive_consequences, school_code) 
    #summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
...
```{r tmna}
  ...
  teacher_questionnaire_dta <- teacher_questionnaire_dta %>%
    ...
    mutate(number_criteria_indicator = m3sbq8_tmna__1	+m3sbq8_tmna__2 + m3sbq8_tmna__3 + m3sbq8_tmna__4 + m3sbq8_tmna__5 + m3sbq8_tmna__6 + m3sbq8_tmna__7 + m3sbq8_tmna__8 + m3sbq8_tmna__97,
      attendance_evaluated=if_else(d_m2bb2_06==1,
                                        case_when(
                                          (d_m2bb2_08a==1) ~ 1,
                                          TRUE ~ 0
                                        ),
           ...
           attendence_sanctions=case_when(
             (d_m2bb2_021==1 | d_m2bb2_022==1 | d_m2bb2_023==1 | d_m2bb2_024==1 | d_m2bb2_025==1 |d_m2bb2_026 ==1 | d_m2bb2_028==1) ~ 1,
             TRUE ~ 0
           ),
...
#plot the indicator  
 ...
 hist(teacher_questionnaire_dta$miss_class_admin)
  ...
  final_indicator_data_TMNA <- teacher_questionnaire_dta %>%
    ...
    group_by(s_ecole_code) %>% 
    select(s_ecole_code,teacher_monitoring,attendance_evaluated,attendance_rewarded,attendence_sanctions,miss_class_admin, school_code, starts_with("m3sbq8_tmna"), number_criteria_indicator ) %>%
    summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
  
  final_indicator_data_TMNA_micro <- teacher_questionnaire_dta %>%
    mutate(n_mssing_TMNA=n_miss_row(.)) %>%
    #group_by(s_ecole_code) %>% 
    select(s_ecole_code,teacher_monitoring,attendance_evaluated,attendance_rewarded,attendence_sanctions,miss_class_admin, school_code, starts_with("m3sbq8_tmna"), number_criteria_indicator) 
    #summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
...
```{r tinm}
  ...
  teacher_questionnaire_dta <- teacher_questionnaire_dta %>%
    ...
    mutate(
      d_m2bc_04 = ifelse(d_m2bc_04 == 97, NA, d_m2bc_04),
      d_m2bc_05 = ifelse(d_m2bc_05 == 97, NA, d_m2bc_05),
...
#plot the indicator  
 ...
 hist(teacher_questionnaire_dta$d_m2bd_02)
  ...
  final_indicator_data_TINM <- teacher_questionnaire_dta %>%
    ...
    group_by(s_ecole_code) %>%
    summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.)))  %>%
    select(s_ecole_code, intrinsic_motivation,acceptable_absent,students_deserve_attention,motivation_teaching,d_m2bd_02,m3sdq2_tmna, starts_with("SE_PRM_TINM"), school_code)
  
  final_indicator_data_TINM_micro <- teacher_questionnaire_dta %>%
    mutate(n_mssing_TINM=n_miss_row(.)) %>%
    #group_by(s_ecole_code) %>%
    #summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.)))  %>%
    select(s_ecole_code, intrinsic_motivation,acceptable_absent,students_deserve_attention,motivation_teaching,d_m2bd_02,m3sdq2_tmna, starts_with("SE_PRM_TINM"), school_code)
...
```{r SATT}
  ...
  school_data_SATT <- management_dta %>%
    mutate(principal_satisfaction=attitude_fun_rev_5(m7shq1_satt),
           m7shq2_satt = ifelse(m7shq2_satt <28800, NA, m7shq2_satt), #other values are either 0 or seem to be too low
...
```{r SEVL}
  ...
  school_data_SEVL<- management_dta %>%
    ...
    principal_negative_consequences = case_when(
      !is.na(m7sgq11_sevl01) | !is.na(m7sgq11_sevl02) | !is.na(m7sgq11_sevl03) | 
      !is.na(m7sgq11_sevl04) | !is.na(m7sgq11_sevl99) ~ 
        if_else(m7sgq11_sevl01==1 | m7sgq11_sevl02==1 | m7sgq11_sevl03==1 | 
                m7sgq11_sevl04==1 | m7sgq11_sevl99==1, 1, 0),
      TRUE ~ NA_integer_  # NA if all are missing
    ),
    principal_positive_consequences = case_when(
      !is.na(m7sgq12_sevl01) | !is.na(m7sgq12_sevl02) | !is.na(m7sgq12_sevl03) | 
      !is.na(m7sgq12_sevl04) | !is.na(m7sgq12_sevl99) ~ 
        if_else(m7sgq12_sevl01==1 | m7sgq12_sevl02==1 | m7sgq12_sevl03==1 | 
                m7sgq12_sevl04==1 | m7sgq12_sevl99==1, 1, 0),
      TRUE ~ NA_integer_  # NA if all are missing
    )
  ) %>%
...
```{r}
  ...
  school_data_IMON <- read_dta(file.path(download_folder, "/01-SECABD24DE.dta")) %>% 
    ...
    rename(
      ...
      m1scq2_imon = d_m1e_02, 
      m1scq8_imon = d_m1e_08,
      ) %>% 
    setNames(tolower(gsub("d_m1_e11","m1scq9_imon__",names(.)))) %>% 
    setNames(tolower(gsub("d_m1e_04","m1scq4_imon__",names(.)))) %>% 
    setNames(tolower(gsub("d_m1e_13","m1scq13_imon__",names(.)))) %>%
...
school_data_IMON <- school_data_IMON %>%
  mutate(m1scq7_imon = if_else(m1scq7_imon == 2, 0, m1scq7_imon)) %>%
    ...
    mutate(
      parents_involved = case_when(
        !is.na(m1scq3_imon) & (m1scq3_imon %in% c(0, 2, 97) | m1scq1_imon == 0 | m1scq2_imon == 4) ~ 0,
        !is.na(m1scq3_imon) & (m1scq3_imon == 1 | m1scq2_imon == 6) ~ 1,
        TRUE ~ NA_integer_  # only NA if m1scq3_imon is missing
      )
    ) %>%
  mutate(
    parents_involved_infr = case_when(
      !is.na(m1scq10_imon) & (m1scq10_imon %in% c(0, 2, 97) | m1scq7_imon == 0 | m1scq8_imon == 6) ~ 0,
      !is.na(m1scq10_imon) & (m1scq10_imon == 1) ~ 1,
      TRUE ~ NA_integer_  # only NA if m1scq10_imon is missing
    )
  ) %>%
...
g4_stud_weights <-final_indicator_data_INPT %>%
  select(s_ecole_code,  s_m4ab_01 ) %>%
  group_by(s_ecole_code) %>%
  summarise(g4_stud_count=mean(s_m4ab_01, na.rm=T)) %>%
  mutate(school_code = s_ecole_code) %>%
...
teacher_absence_weights <- numeligible_absence %>%
  select(school_code,  numEligible ) %>%
  group_by(school_code) %>%
  summarise(numEligible=max(numEligible, na.rm=T)) %>%
...
data_list <- c(ind_dta_list, 'school_dta', 'school_dta_short', 'school_dta_short_imp', 'school_dta_preamble', 'final_school_data', 'teacher_questionnaire_dta','teacher_absence_final', 'ecd_dta_anon', 'teacher_assessment_dta', 'teacher_roster_dta', 
               "indicators", 'school_gdp', 'assess_4th_grade_anon', 'school_weights')
save(list=data_list, file = file.path(confidential_folder, "school_survey_data.RData"))

save(list=c(ind_dta_list,"school_dta_short", 'school_dta_short_imp', "indicators",  'school_gdp', 'school_weights' ), file = file.path(confidential_folder, "school_indicators_data.RData"))
...
df_weights_function <- function(dataset) {
  ...
  dataset %>%
    ...
    mutate(strata_prob=if_else(is.na(strata_prob),median(strata_prob, na.rm=T), strata_prob)) %>%
    select(-one_of(colnames(data_set_updated[, -which(names(data_set_updated) == "urban_rural" | names(data_set_updated) == "province" | names(data_set_updated) == "district" |
                                                      ...
                                                      | names(data_set_updated) == "ipw" | names(data_set_updated) == "strata_prob" | names(data_set_updated) == "ipw_school")])))
}
...
school_dta_short <- school_dta_short %>%
  mutate(s_ecole_code = if_else(is.na(s_ecole_code), school_code, s_ecole_code))
...
save(list=c(anon_dta_list,'indicators'), file = file.path("C:/Users/wb631589/OneDrive - WBG/GEPD/CNT/MDG/MDG_2021_GEPD/MDG_2021_GEPD_v02_M/Data/School/school_indicators_data_anon.RData"))
