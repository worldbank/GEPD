...
```{r tatt}
 ...
 teacher_questionnaire_dta <- teacher_questionnaire_dta %>%
  ...
  final_indicator_data_TATT <- teacher_questionnaire_dta %>%
    ...
    group_by(s_ecole_code) %>%
    select(s_ecole_code, teacher_attraction,teacher_satisfied_job,teacher_satisfied_status,better_teachers_promoted,teacher_bonus, teacher_bonus_hard_staff, school_code, salary_delays, salary_delays_indicator) %>%
    summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
  
  final_indicator_data_TATT_micro <- teacher_questionnaire_dta %>%
    mutate(n_mssing_TATT=n_miss_row(.)) %>%
    #group_by(s_ecole_code) %>%
    select(s_ecole_code, teacher_attraction,teacher_satisfied_job,teacher_satisfied_status,better_teachers_promoted,teacher_bonus, teacher_bonus_hard_staff,salary_delays, school_code, salary_delays_indicator) 
    #summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
