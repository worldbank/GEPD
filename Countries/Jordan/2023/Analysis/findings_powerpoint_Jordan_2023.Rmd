...
svymeantab <- function(data, strata) {
  ...
  get(data) %>%
    ...
    select(one_of(indicators_list), strata, ipw, weight ) %>%
    pivot_longer(cols=one_of(indicators_list),
                 names_to = 'indicators',
                 values_to='value') %>%
    as_survey_design(strata=strata,
                     weight=weight) %>%
    select(-strata, -ipw, -weight) %>%
    group_by(indicators) %>%
...
fixest::feols(
  student_knowledge ~ student_male,
  data=assess_4th_grade_anon_anon,
  weights=assess_4th_grade_anon_anon$ipw,
  cluster=c('hashed_school_code'),
  se='cluster'
) %>%
  summary()
...
fixest::feols(
  math_student_knowledge ~ student_male,
  data=assess_4th_grade_anon_anon,
  weights=assess_4th_grade_anon_anon$ipw,
  cluster=c('hashed_school_code'),
  se='cluster'
...
fixest::feols(
  literacy_student_knowledge ~ student_male,
  data=assess_4th_grade_anon_anon,
  weights=assess_4th_grade_anon_anon$ipw,
  cluster=c('hashed_school_code'),
  se='cluster'
...
```{r}
final_indicator_data_LERN_F_anon %>%
  ungroup() %>%
  as_survey_design(strata=c('strata'),
                   weight=ipw) %>%
  summarise(
    student_knowledge=survey_mean(student_knowledge, na.rm=TRUE),
    student_proficient=survey_mean(student_proficient, na.rm=TRUE),
    literacy_student_knowledge=survey_mean(literacy_student_knowledge, na.rm=TRUE),
    math_student_knowledge=survey_mean(math_student_knowledge, na.rm=TRUE)
  )

final_indicator_data_LERN_M_anon %>%
  ungroup() %>%
  as_survey_design(strata=c('strata'),
                   weight=ipw) %>%
  summarise(
...
final_indicator_data_LERN_anon %>%
  ungroup() %>%
  filter(!is.na(ipw)) %>%
  as_survey_design(strata=c('strata'),
                   weight=ipw) %>%
...
region_content_long <-region_content %>%
  ...
  mutate(Assessment=case_when(
    ...
    contMedian <- summarise(group_by(region_content_long,  Type, Assessment), MD = round(weighted.median(content_proficiency, ipw, na.rm=T),1))
...
final_indicator_data_CONT_F_anon %>%
  ungroup() %>%
  as_survey_design(strata=c('strata')) %>%
  summarise(
    content_proficiency=survey_mean(content_proficiency, na.rm=TRUE),
    literacy_content_proficiency=survey_mean(literacy_content_proficiency, na.rm=TRUE),
    math_content_proficiency=survey_mean(math_content_proficiency, na.rm=TRUE),
    content_knowledge=survey_mean(content_knowledge, na.rm=TRUE),
    literacy_content_knowledge=survey_mean(literacy_content_knowledge, na.rm=TRUE),
    math_content_knowledge=survey_mean(math_content_knowledge, na.rm=TRUE)
  )

final_indicator_data_CONT_M_anon %>%
  ungroup() %>%
  as_survey_design(strata=c('strata')) %>%
  summarise(
    content_proficiency=survey_mean(content_proficiency, na.rm=TRUE),
...
final_indicator_data_CONT_anon %>%
  ungroup() %>%
  filter(!is.na(ipw)) %>%
  as_survey_design(strata=c('strata')) %>%
  summarise(
...
final_indicator_data_CONT_F_anon %>%
  ...
  summarise(
    count=n(),
    content_proficiency_mean=survey_mean(content_proficiency, na.rm=TRUE),
    literacy_content_proficiency_mean=survey_mean(literacy_content_proficiency, na.rm=TRUE),
    math_content_proficiency_mean=survey_mean(math_content_proficiency, na.rm=TRUE),
    content_proficiency_median=survey_median(content_proficiency, na.rm=TRUE),
    literacy_content_proficiency_median=survey_median(literacy_content_proficiency, na.rm=TRUE),
    math_content_proficiency_median=survey_median(math_content_proficiency, na.rm=TRUE),    
    content_knowledge_mean=survey_mean(content_knowledge, na.rm=TRUE),
    literacy_content_knowledge_mean=survey_mean(literacy_content_knowledge, na.rm=TRUE),
    math_content_knowledge_mean=survey_mean(math_content_knowledge, na.rm=TRUE)
  )
