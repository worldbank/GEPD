...
teach_df <- read_dta(file=here(confidential_folder, "data/JOR_2023_teacher_level.dta")) %>%
  filter(!is.na(s_a1))
...
ecd_dta<-read_dta(file.path(download_folder, "ecd_assessment.dta"))  %>%
  #drop missing sex
  filter(!is.na(m6s1q3)) %>%
  filter(m6s1q4==1) #only include students with consent
...
#############################################
  ...
  # For salary, based GDP per capita from 2020 World Bank  https://data.worldbank.org/indicator/NY.GDP.PCAP.PP.CD?locations=JO.  
...
data_set_updated <- read_csv(paste(sampling_folder, '/GEPD_JOR_weights_', currentDate,  '.csv', sep="")
                             ) %>%
...
This section will save any additional modules that are available (e.g. ETRI, climate change, etc)

```{r echo=FALSE}
#| label: extras

regular_data_list<-c('school_dta', 'school_dta_short', 'school_dta_short_imp', 'school_data_preamble', 'final_school_data', 'teacher_questionnaire','teacher_absence_final', 'ecd_dta', 'teacher_assessment_dta', 'teacher_roster', 
...
other_data_list<-c('ability_to_use_etri','before_after_closure','climatebeliefs', 'digital_use_inschool_etri', 'direct_instruction_etri',
                   'etri_roster','planning_lesson_etri','proficiency_ict_etri','schoolcovid_roster', 'teacherimpact', 'use_outsideschool_etri' )

#loop through the datasets
for (i in regular_data_list ) {
  if (exists(i)) {
    #form temp data frame with each schools data
    temp<-get(i) 
  ...
  temp %>%
    ...
    janitor::clean_names() %>%
    write_dta( path= file.path(paste(confidential_folder,"/data", sep=""), paste(i,".dta", sep="")))  
    
  }
}

#loop through the datasets
for (i in other_data_list ) {

    #form temp data frame with each schools data
    temp<-read_dta(paste0(download_folder,"/", i,'.dta'))
