...
#expert_dir <- "//wbgfscifs01/GEDEDU/datalib-edu/projects/GEPD/CNT//JOR/JOR_2019_GEPD/JOR_2019_GEPD_v01_M/Data/Expert_Survey"
#read in data

expert_dir <- "C:/Users/wb631589/OneDrive - WBG/GEPD/CNT//JOR/JOR_2019_GEPD/JOR_2019_GEPD_v02_M/Data/Expert_Survey"
...
#start with teachers
##########################
expert_dta_teachers <- readxl::read_xlsx(path=paste(expert_dir, 'PolicySurvey_Jordan_final.xlsx', sep="/"), sheet = 'Teachers', .name_repair = 'universal') 
...
expert_dta_teachers_final <- expert_dta_teachers_final %>%
  mutate(evaluation_law=read_var(A10),
         ...
         evaluation_criteria=5, #to make it aligned with what we have in the new data
...
# Inputs
##############################
expert_dta_inputs <- readxl::read_xlsx(path=paste(expert_dir, 'PolicySurvey_Jordan_final.xlsx', sep="/"), sheet = 'Inputs', .name_repair = 'universal')
...
# School Management
###############################
expert_dta_school_management <- readxl::read_xlsx(path=paste(expert_dir, 'PolicySurvey_Jordan_final.xlsx', sep="/"), sheet = 'School_Management', .name_repair = 'universal')
...
expert_dta_school_management_final <- expert_dta_school_management_final %>%
  mutate(principal_rubric=read_var(A4),
         principal_factors=5) %>% #adjusting it to make it consistent with what we have in the new workflow
...
expert_dta_school_management_final <- expert_dta_school_management_final %>%
  mutate(principal_monitor_law=read_var(A6),
         principal_monitor_criteria=4) %>% #adjusting it to make it consistent with what we have in the new workflow
  mutate(principal_evaluation=1+principal_monitor_law+(3/5*principal_monitor_criteria))

################################
# Learners 
################################
expert_dta_learners <- readxl::read_xlsx(path=paste(expert_dir, 'PolicySurvey_Jordan_final.xlsx', sep="/"), sheet = 'Learners', .name_repair = 'universal')
...
expert_dta_learners_final <- expert_dta_learners_final %>%
  mutate(iodization=4/4, #adjusting this to make it consistent with what we have in the new workflow
         iron_fortification=4/4, #adjusting this to make it consistent with what we have in the new workflow
...
expert_dta_learners_final <- expert_dta_learners_final %>%
  mutate(immunization=read_var(A6),
         ...
         deworming=read_var(A8), #since deworming is missing in Jordan, adjusting the final score so that each of the remaining components is given an equal weight
  ...
  mutate(health_programs=1+(1.25*immunization + 1.25*healthcare_young_children + 1.25*antenatal_skilled_delivery))
...
expert_dta_learners_final <- expert_dta_learners_final %>%
  mutate(pre_primary_free_some=read_var(A10),
         ...
         ece_qualifications=4, #adjusting this to be consistent with what we have in the new workflow
