...
numeligible <- teacher_roster %>%
  ...
  #for the two schools that for some reason do not have the roster, just use the data from automatically computed numeligible from the school data
  mutate(numEligible4th_manual = ifelse(school_code == 110305, 14, numEligible4th_manual),
         numEligible4th_manual = ifelse(school_code == 114292, 18, numEligible4th_manual)) %>%
  ungroup()
...
teacher_questionnaire <- teacher_questionnaire %>%
  group_by(school_code, m3sb_troster) %>%
  mutate(!!dupl_var := n() > 1) %>%
  ungroup()
...
preamble_info_absence <- c('interview__key', 'TEACHERS__id', 'teacher_name', 'teacher_number', 
                   ...
                   'teacher_language', 'teacher_math', 'teacher_both_subj', 'teacher_other_subj', 'subject_joined', 'm2sbq6_efft', 'm2saq3', 'm2saq2', 'm2saq5', 'm2saq6')
...
#teachers are not unique but it does look like most of the information is missing 
...
num_boys <- school_dta %>%
  ...
  mutate(m4scq13_girls = ifelse(m4scq13_girls == 98, NA, m4scq13_girls),
         m4scq13_girls = ifelse(m4scq13_girls == 272, 27, m4scq13_girls)) #typo
...
final_indicator_data_ATTD_M<- school_data_INPT %>%
  ...
  mutate(student_attendance = if_else((m4scq13_girls < m4scq4n_girls & !is.na(m4scq13_girls) & !is.na(m4scq4n_girls)), (m4scq13_girls/m4scq4n_girls), student_attendance)) %>%  mutate(student_attendance=100*student_attendance) %>%
  group_by(school_code) %>%
  summarise_all(~first(na.omit(.))) %>%
  mutate(n_mssing_SSLD=n_miss_row(.)) %>%
  select( -starts_with('interview'), -starts_with('enumerator'))
...
ecd_dta_anon_for_micro <- ecd_dta_anon
...
final_indicator_data_INFR <- school_data_INFR %>%
  ...
  )# 1 point if internet working, 0.5 if doesn't work well, 0 if not at all ) # 1 point if internet working, 0.5 if doesn't work well, 0 if not at all
...
g4_stud_weights <-school_dta %>%
  ...
  #unclear what is happening with these three schools but they donot seem to have the information on teachers
  #in the roster or questionnaire files but it is in the school data, so filling these in manually from the school file
  mutate(g4_teacher_count = ifelse(school_code == 110305, 2, g4_teacher_count),
         g4_teacher_count = ifelse(school_code == 113785, 4, g4_teacher_count),
         g4_teacher_count = ifelse(school_code == 114292, 5, g4_teacher_count),
         g1_teacher_count = ifelse(school_code == 110305, 0, g1_teacher_count),
         g1_teacher_count = ifelse(school_code == 113785, 0, g1_teacher_count),
         g1_teacher_count = ifelse(school_code == 114292, 0, g1_teacher_count)) %>%
  ...
  #for the schools without any information on the classroom size, impute the weight of 1 to make sure we keep all the observation
  mutate(g4_stud_weight = ifelse(is.na(g4_stud_weight), 1, g4_stud_weight),
...
teacher_assessment_weights <-teacher_assessment_weights %>%
  ...
  mutate(numEligible4th_manual = if_else(teacher_quest_count>numEligible4th_manual, teacher_quest_count, numEligible4th_manual),
         teacher_weight_component=numEligible4th_manual/teacher_quest_count,
         #for the schools where it is still missing, replace with 1 
...
g1_stud_weights<-school_dta %>%
  ...
  mutate(g1_class_weight = g1_teacher_count/1,
         ...
         #a couple of classes with missing information on the count of students in that particular classroom, replace with 1
...
data_list <- c(ind_dta_list, 'school_dta', 'school_dta_short', 'school_dta_short_imp', 'school_data_preamble', 'final_school_data', 'teacher_questionnaire','teacher_absence_final', 'ecd_dta', 'teacher_assessment_dta', 'teacher_roster', 
               "indicators", 'school_metadta', 'school_gdp', 'assess_4th_grade_anon', 'ecd_dta_anon', 'ecd_dta_anon_for_micro', 'school_weights')
