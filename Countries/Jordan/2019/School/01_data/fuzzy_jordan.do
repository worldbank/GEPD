...
foreach v in teacher_absence teacher_assessment teacher_pedagogy teacher_questionnaire {
	...
	use "$dir/3_input_data/JOR/JOR_`v'", clear 
...
*seem to be 2 different teachers but not fully sure, add 1 and 2 to differentiate them
replace m2saq2 = "امل شديفات 1" if school_code == 114403 & m2saq2 == "امل شديفات" & teachers_id == 2
replace m2saq2 = "1 حنان الطراونه" if school_code == 112205 & m2saq2 == "امل شديفات" & teachers_id == 3
replace m2saq2 = "فاطمه المساعيد 1" if school_code == 111984 & m2saq2 == "فاطمه المساعيد" & teachers_id == 1
replace m2saq2 = "هبه 1" if school_code == 114110 & m2saq2 == "فاطمه المساعيد" & teachers_id == 14
...
*****
use `teacher_assessment', clear // 1082

*replace some names to be able to merge with roster
replace m5sb_troster = "ايه" if m5sb_troster == "اية" & school_code == 113016
replace m5sb_troster = "ساجدة فحماوي" if m5sb_troster == "ساجدة" & school_code == 171100
replace m5sb_troster = "الاء السحيمات" if m5sb_troster == "الاء" & school_code == 172529
replace m5sb_troster = "وجد التميمي" if m5sb_troster == "وجد" & school_code == 176109 
...
use `teacher_pedagogy', clear // 246

*correcting some names to be able to merge with the roster data 
*replace m4saq1 = "ايه حسن" if m4saq1 == "اية حسن" & school_code == 110763
replace m4saq1 = "كفا" if m4saq1 == "كفى محمد الوديان" & school_code == 171381
...
*****
use `teacher_questionnaire', clear // 1091

*a couple of schools have missing names, fill them in based on the names from the roster
replace m3sb_troster = "ساجدة فحماوي" if m3sb_troster == "ساجدة" & school_code == 171100
...
replace txtusing = "بخيته القواشمة" if txtusing == "بخيته" & school_code == 112511
...
foreach i in teacher_quest teacher_asmnt teacher_pedag {
	...
	*does not look like a correct match 
	drop if school_code == 171100 & txtmaster == "ساجدة" & txtusing == "سائدة دراج"
	drop if school_code == 172529 & txtmaster == "الاء" & txtusing == "الاء السحيمات" // not sure what's going on here, drop for now 
	drop if school_code == 112511 & txtusing == "صفاء الهواشلة" & txtmaster == "بخيته القواشمة" 
	drop if school_code == 172529 & txtusing == "الاء السحيمات" & txtmaster == "آلاء سليمان"
	drop if school_code == 113016 & txtusing == "هيا" & txtmaster == "اية"
	drop if school_code == 110222 & txtusing == "عالية الشعار" & txtmaster == "نعايم الشعار"
