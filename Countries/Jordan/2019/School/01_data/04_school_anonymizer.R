...
df_weights_function <- function(dataset,scode, snumber, prov) {
  ...
  dataset %>%
    ...
    mutate(ipw=if_else(is.na(.data$weights), median(.data$weights, na.rm=T), .data$weights)) %>%
    mutate(province=governorate) %>%
    mutate(strata_count = STRATUM) %>%
    mutate(urban_rural = if_else(rural == "TRUE", "Rural", "Urban")) %>%
    select(-one_of(colnames(data_set_updated[, -which(names(data_set_updated) == "rural" | names(data_set_updated) == "governorate" | names(data_set_updated) == "province" |
                                                      ...
                                                      names(data_set_updated) == "property_type" | names(data_set_updated) == "supervisory_authority") | names(data_set_updated) == "stratum"])))
}
...
save(list=c(anon_dta_list,'school_metadta','indicators'), file = file.path(save_folder, "school_indicators_data_anon.RData"))
save(list=c(anon_dta_list,'school_metadta','indicators'), file = file.path("C:/Users/wb631589/OneDrive - WBG/GEPD/CNT/JOR/JOR_2019_GEPD/JOR_2019_GEPD_v02_M/Data//School/school_indicators_data_anon.RData"))