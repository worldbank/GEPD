---
title: "Global Education Policy Dashboard - Gabon 2023"
...
Roadmap:  
- Below is a set of tables and charts containing findings for the Gabon 2023 Global Education Policy Dashboard survey. - We will start with breakdowns of our Practice indicators  
...
if (str_to_lower(Sys.getenv("USERNAME")) == "wb469649"){
  ...
  download_folder <-file.path(paste(project_folder,country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v02_RAW", sep="_"),"Data/raw/", sep="/"))
  confidential_folder <- file.path(paste(project_folder,country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v02_RAW", sep="_"),"Data/confidential/", sep="/"))
  save_folder <- file.path(paste(project_folder,country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v02_RAW", sep="_"),"Data/anonymized/", sep="/"))
  backup_onedrive="no"
...
#list countries
countries <- c("GAB", "PER", "JOR", "RWA", "MDG", "ETH", "SLE", "TCD","NER")
...
combined_df_raw <- bind_rows(GAB_data,PER_data,JOR_data, ETH_data, RWA_data, MDG_data, SLE_data, TCD_data, NER_data)
...
#build table
tab_df <- tab_df  %>%
  select(`Indicator Name`,`Gabon`, Peru, Jordan, Rwanda, Ethiopia, Madagascar, `Sierra Leone`, Niger, Chad)
...
svymeantab <- function(data, strata) {
  ...
  get(data) %>%
    mutate(strata_prob=if_else(is.na(strata_prob), median(strata_prob, na.rm=TRUE),strata_prob)) %>%
    mutate(weight=1/strata_prob) %>%
    filter(!is.na(ipw)) %>%
    filter(!is.infinite(ipw)) %>%
    select(one_of(indicators_list), Province, private, rural, weight ) %>%
    pivot_longer(cols=one_of(indicators_list),
                 names_to = 'indicators',
                 values_to='value') %>%
    as_survey_design(strata=strata,
                     weight=weight) %>%
    select(-c( 'Province', 'private', 'rural', 'weight')) %>%
    group_by(indicators) %>%
...
# School Survey
    ...
    sumstats_school_df <- svymeantab('sumstats_school', strata=c('Province', 'private', 'rural'))
    ...
    sumstats_school_urban_df <- svymeantab('sumstats_school_urban', strata=c('Province', 'private'))
    ...
    #rural
      ...
      sumstats_school_rural_df <- svymeantab('sumstats_school_rural', strata=c('Province', 'private'))
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Gabon 2023 - Urban/Rural",
...
#public/private
      ...
      #add variable label
      sumstats_school_nonprivate_df <-  svymeantab('sumstats_school_nonprivate', strata=c('Province', 'private', 'rural')) %>%
        rename(
          mean_urban=mean,
          ci_urban=ci
        )
      ...
      #add variable label
      sumstats_school_private_df <-  svymeantab('sumstats_school_private', strata=c('Province', 'private', 'rural')) %>%
        rename(
          mean_rural=mean,
          ci_rural=ci
        )
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Gabon 2023 - Public/Private Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
...
attach(here(confidential_folder, "School/school_survey_data.RData"))
...
```{r}
crosstab <- assess_4th_grade_anon_anon %>%
  select(hashed_school_code, student_number, math_student_proficient, literacy_student_proficient) 

crosstab %>%
  group_by(math_student_proficient,literacy_student_proficient) %>%
  summarise(n=n())
...
```
## Proportion of teacher speaking english

```{r}
#read in weights 
weights_df <- read_csv(here(save_folder, "School", "data/school_weights_anon.csv" ))

#read in module
english_df <- read_dta(here(download_folder, "School","roster_english_t_gbn.dta")) %>%
  pivot_wider(id_cols=c("interview__key","interview__id","questionnaire_roster__id"),
                names_from = roster_english_t_gbn__id,
                values_from=m3saq8_gbn)

english_merge_df <- english_df %>%
  left_join(distinct(teacher_questionnaire_anon %>% select(hashed_school_code, interview__key, questionnaire_roster__id, ipw, rural, province, private))) %>%
  left_join(weights_df) %>%
  mutate(teach_weight = ipw*teacher_obs_weight_component,
         teach_weight=if_else(is.na(teach_weight), median(teach_weight, na.rm=TRUE),teach_weight),
         province=if_else(is.na(province), "unknown", province),
         rural=if_else(is.na(rural), median(rural, na.rm=TRUE),rural),
         private=if_else(is.na(private), median(private, na.rm=TRUE),private))


stats <- english_merge_df %>%
  as_survey_design(
    strata=c('province','private','rural'),
    weights=teach_weight
  ) %>%
  summarise(
    speak_perfectly=100*survey_mean(`1`==1),
    speak_well=100*survey_mean(`1`==2),
    speak_average=100*survey_mean(`1`==3),
    speak_somewhat=100*survey_mean(`1`==4),
    speak_notatall=100*survey_mean(`1`==5),

    understand_perfectly=100*survey_mean(`2`==1),
    understand_well=100*survey_mean(`2`==2),
    understand_average=100*survey_mean(`2`==3),
    understand_somewhat=100*survey_mean(`2`==4),
    understand_notatall=100*survey_mean(`2`==5),
    
    write_perfectly=100*survey_mean(`3`==1),
    write_well=100*survey_mean(`3`==2),
    write_average=100*survey_mean(`3`==3),
    write_somewhat=100*survey_mean(`3`==4),
    write_notatall=100*survey_mean(`3`==5),
    
    read_perfectly=100*survey_mean(`4`==1),
    read_well=100*survey_mean(`4`==2),
    read_average=100*survey_mean(`4`==3),
    read_somewhat=100*survey_mean(`4`==4),
    read_notatall=100*survey_mean(`4`==5),
  ) 
...
stats_final <- stats_mean %>% bind_rows(stats_se)

flextable(stats_final)
```
...
#read in module
bullying_df <- read_dta(here(download_folder, "School","roster_bullying_gbn.dta")) %>%
  pivot_wider(id_cols=c("interview__key","interview__id","questionnaire_roster__id"),
                names_from = roster_bullying_gbn__id,
                values_from=m3saq9_gbn)

bullying_merge_df <- bullying_df %>%
  left_join(distinct(teacher_questionnaire_anon %>% select(hashed_school_code, interview__key, questionnaire_roster__id, ipw, rural, province, private))) %>%
  left_join(weights_df) %>%
  mutate(teach_weight = ipw*teacher_obs_weight_component,
         teach_weight=if_else(is.na(teach_weight), median(teach_weight, na.rm=TRUE),teach_weight),
...
stats <- bullying_merge_df %>%
  as_survey_design(
    strata=c('province','private','rural'),
    weights=teach_weight
  ) %>%
  summarise(
    stolen_onceaweek=100*survey_mean(`1`==1),
    stolen_oncetwiceamonth=100*survey_mean(`1`==2),
    stolen_fettimesyear=100*survey_mean(`1`==3),
    stolen_never=100*survey_mean(`1`==4),
    
    hithurt_onceaweek=100*survey_mean(`2`==1),
    hithurt_oncetwiceamonth=100*survey_mean(`2`==2),
    hithurt_fettimesyear=100*survey_mean(`2`==3),
    hithurt_never=100*survey_mean(`2`==4),
    
    threatened_onceaweek=100*survey_mean(`3`==1),
    threatened_oncetwiceamonth=100*survey_mean(`3`==2),
    threatened_fettimesyear=100*survey_mean(`3`==3),
    threatened_never=100*survey_mean(`3`==4)
  ) 
...
main_indicator_labels<-c( 
                        'Enseignement - Attrait de la profession',
                         'Enseignement - Recrutement & carrière',
                         'Enseignement - Soutien' ,
                         'Enseignement - Évaluation', 
                         'Enseignement - Surveillance et reddition de comptes', 
                         # "Enseignement - Motivation intrinsèque", 
                         # "Intrants et infrastructure - suivi",
                         'GESTION SCOLAIRE - Attrait de la profession' ,                   
                         'GESTION SCOLAIRE - Recrutement & carrière' ,      
                         'GESTION SCOLAIRE - Soutien' ,                      
                         'GESTION SCOLAIRE - Évaluation'
                         #GESTION SCOLAIRE - urveillance et reddition de comptes'
    ...
    expert_dta_final<-haven::read_dta(here(data_dir,"Policy_Survey/expert_dta_final.dta") )
   ...
   #create radar plot
    ggradar(data_radar,
      ...
      ggtitle(str_wrap('Graphique radar des leviers politiques de facto et de jure',50)) +
      theme_minimal()+
...
data_plot <- public_officials_dta_clean_anon %>%
  ...
  #   govt_tier=='Ministry of Education '  ~ 'MBBSE/TSC'
  # )) %>%
  group_by(govt_tier) %>%
...
demo_po_dist <- public_officials_dta_clean_anon %>% 
  distinct(interview__id, .keep_all = T) %>% 
  select(govt_tier, age, gender, years_current_position, years_current_org, years_civil_service, num_pub_sector_org, net_monthly_salary) %>% 
  mutate(gender = case_when(
    gender ==1 ~ "Male",
    gender !=1 ~ "Female"
  ...
  rename(
    ...
    Age = age,
    Gender = gender,
    `Years in current position` = years_current_position,
    `Years in current organisation` = years_current_org,
    `Years in civil service` = years_civil_service,
    `Number of public organisation worked in`=num_pub_sector_org

  )

demo_po_central <- public_officials_dta_clean_anon %>% 
  distinct(interview__id, .keep_all = T) %>% 
  select(govt_tier, age, gender, years_current_position, years_current_org, years_civil_service, num_pub_sector_org, net_monthly_salary) %>% 
   mutate(gender = case_when(
    gender ==1 ~ "Male",
...
ggplotGrid(ncol = 2,
  lapply(c("Age", "Gender", "`Years in current position`", "`Years in current organisation`", "`Years in civil service`", "`Number of public organisation worked in`"),
    function(col) {
        ggplot(demo_po_dist, aes_string(col)) + geom_bar() + coord_flip()
    }))
...
ggplotGrid(ncol = 2,
  lapply(c("Age", "Gender", "`Years in current position`", "`Years in current organisation`", "`Years in civil service`", "`Number of public organisation worked in`"),
        function(col) {
        ggplot(demo_po_central, aes_string(col)) + geom_bar() + coord_flip() 
    }))
```
