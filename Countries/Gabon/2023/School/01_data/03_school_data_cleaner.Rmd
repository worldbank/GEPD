...
```{r echo=FALSE}
 ...
 #############################################
  ...
  #############################################
teach_df <- read_dta(file.path(download_folder, "TEACH.dta"))
...
final_indicator_data_PEDG_F<- school_dta %>%
    ...
    left_join(teach_df %>% select(school_code, teachers_id, m2saq3) %>% rename(TEACHERS__id=teachers_id), by=c('school_code', 'TEACHERS__id'))    group_by(school_code) %>%
    summarise_all(~first(na.omit(.))) %>%
    select( -starts_with('interview'), -starts_with('enumerator')) %>%
    left_join(teacher_pedagogy_segments) %>%
    filter(m2saq3==1) %>%
