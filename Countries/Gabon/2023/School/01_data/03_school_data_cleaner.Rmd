...
school_dta<- school_dta %>%
 ...
 mutate(school_name_preload = gsub("[[:punct:]]", " ", school_name_preload),
  ...
  left_join(data_set_updated, by=c("school_name_preload")) %>% 
...
teach_df <- read_dta(file=here(confidential_folder, "data/GAB_teacher_level.dta")) %>%
  filter(!is.na(s_a1))
...
#read in teach data
  teach_dta <- teach_df %>% 
    select(school_code, m2saq3, starts_with("s_"), starts_with("s2_")) %>% 
  ...
  pivot_longer(
    ...
    pivot_wider(id_cols =c(id, school_code, m2saq3, school_clip), names_from = behaviour, values_from = count) %>% select(-id) 
  ...
  teach_dta <- teach_dta %>% 
    group_by(school_code,m2saq3, school_clip) %>% 
...
#   select(school_code, everything())
  ...
  teacher_pedagogy_segments <- teach_dta %>%
    ...
    select(school_code, m2saq3, clip, everything()) 
   # mutate(across(everything(), as.numeric))
...
final_indicator_data_CONT_M <- teacher_assessment_dta %>%
  ...
  left_join(teach_df %>% select(school_code, teachers_id, m2saq3) %>% rename(TEACHERS__id=teachers_id), by=c('school_code', 'TEACHERS__id')) %>%
  filter(m2saq3!=1) %>%
  group_by(school_code) %>%
  add_count(school_code,name='m5_teach_count') %>%
  mutate(content_knowledge=case_when(
...
data_list <- c(ind_dta_list, 'school_dta', 'school_dta_short', 
               ...
               "indicators", 'metadta', 
               #'school_gdp', 
               'assess_4th_grade_anon', 'ecd_dta_anon')
