...
if (Sys.getenv("USERNAME") == "WB469649" | Sys.getenv("USERNAME") == "wb469649"){
  ...
  project_folder  <- "C:/Users/wb631589/OneDrive - WBG/GEPD-Confidential/CNT/"
   download_folder <-file.path(paste(project_folder,country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v02_RAW", sep="_"),"Data_2024", sep="/"))
   download_folder2 <- file.path(paste(project_folder,country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v02_RAW", sep="_"),"Data/raw/School/", sep="/"))
  confidential_folder <- file.path(paste(project_folder,country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v02_RAW", sep="_"),"Data/confidential/School", sep="/"))
  save_folder <- file.path(paste(project_folder,country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v02_RAW", sep="_"),"Data/anonymized/School", sep="/"))
}  else {
...
#teacher gender data
teacher_gender <- read_dta("C:/Users/wb631589/OneDrive - WBG/GEPD-Confidential/General/LEGO_Teacher_Paper/5_output_data/GAB/GAB_teacher_level_updated.dta") 
teacher_gender <- teacher_gender %>%
  select(school_code, teacher_male, m2saq2_original, m5sb_troster_original, m3sb_troster_original, m4saq1_original)

# get unique ID from the sampling frame for schools
...
data_set_updated$school_code <-as.character(lapply(data_set_updated$group_id, function(x) {digest(x, algo="xxhash64", seed=531254, serialize = T)}))

#save a hashed version of the dataset, to produce a link file
key<-data_set_updated %>%
  group_by(school_code) %>%
  slice(1) %>%
  select(school_name_preload, temp_school_code, school_code) 

write_excel_csv(key, file.path(confidential_folder, "school_idfile_hashed.csv"))
...
###########################
## Adding the urban/rural variables
data_set_updated <- read_csv(paste(sample_folder, '/GEPD_GAB_weights_', currentDate,  '.csv', sep="")
) %>%
  mutate(school_code_bis=Code_Etablissement,
         school_name_preload=`Nom Officiel de l'Etablissement`,
         urban_rural=if_else(rural==FALSE, "Urban", "Rural"),
         public=if_else(private==1, "Private", "Public")) %>% 
  select(ipw:public) %>% 
  mutate(school_name_preload = gsub("[[:punct:]]", " ", school_name_preload),
         ...
         school_code = paste0(school_name_preload, " _ ", school_code_bis))

data_set_updated <- data_set_updated[!duplicated(data_set_updated), ]
...
school_dta<- school_dta %>%
  ...
  mutate(temp_school_code = paste(school_code)) %>% 
 ...
 mutate(school_name_preload = gsub("[[:punct:]]", " ", school_name_preload),
         school_name_preload = str_squish(school_name_preload),
         school_name_preload = iconv(school_name_preload,to="ASCII//TRANSLIT")) %>% 
  #there seem to be one mistake, correct it here 
  mutate(school_code = if_else(school_code == "ECOLE PUBLIQUE COMMUNALE C DE TCHIBANGA _ 0501401006X505",
                               "ECOLE PUBLIQUE COMMUNALE C DE TCHIBANGA _ 0501401009X505", school_code)) %>%
  ...
  # )) %>% 
  # 
  left_join(data_set_updated, by=c("school_code")) %>%
  select(-school_name_preload.x, -ipw) %>%
  rename(school_name_preload = school_name_preload.y) %>%
  #drop closed schools
  filter(m1s0q1==1) %>%
  mutate(feeding_program = m1saq9_lnut)
...
teacher_absence_final<- teacher_absence_dta %>%
  select(preamble_info, preamble_info_absence, contains('absen'), contains('presence')) %>%
...
```{r echo=FALSE}
 ...
 #############################################
  ...
  #############################################
#teach_df <- read_dta(file.path(download_folder2, "TEACH.dta"))
...
#correct the variable that does not have the same values across segments
school_dta <- school_dta %>%
  mutate(across(
    c(s1_0_1_2, s1_0_2_2, s1_0_3_2),
    ~ case_when(
      . == 1 ~ 2,
      . == 2 ~ 3,
      . == 3 ~ 4,
...
#read in teach data
  teach_dta <- school_dta %>% 
    ...
    filter(!is.na(s1_a1)) %>%
    rename_with(.cols=starts_with("s_"),.fn=~gsub("s_","s1_",.x) ) %>%
  pivot_longer(
    cols = c(3:88),
...
final_indicator_data_PEDG_F<- school_dta %>%
  select(preamble_info, m4saq1, m4saq1_number )  %>%
  mutate(m4saq1_original = m4saq1) %>%
     left_join(teacher_gender, by=c('school_code', 'm4saq1_original')) %>%
    group_by(school_code) %>%
    summarise_all(~first(na.omit(.))) %>%
    select( -starts_with('interview'), -starts_with('enumerator')) %>%
    left_join(teacher_pedagogy_segments) %>%
...
final_indicator_data_PEDG_M<- school_dta %>%
    select(preamble_info, m4saq1, m4saq1_number )  %>%
    mutate(m4saq1_original = m4saq1) %>%
    left_join(teacher_gender, by=c('school_code', 'm4saq1_original')) %>%
    group_by(school_code) %>%
...
num_boys <- school_dta %>%
  ...
  #for this school, it looks like they were trying to do 1
  mutate(m4scq13_girls = ifelse(school_code == "ECOLE PUBLIQUE IGOUMA 2 _ 0401102001X511" & m4scq13_girls == 10, 1, m4scq13_girls))
...
assess_4th_grade_dta <- assess_4th_grade_dta %>%
  ...
  select(-starts_with("m8saq2_id__"), -starts_with("m8saq3_id__"), -starts_with("m8sbq1_number_sense__"), m8s1q1, m8s1q2, m8s1q3)
...
ecd_dta<-read_dta(file.path(download_folder, "ecd_assessment.dta")) %>%
  #drop students that don't consent
  filter(m6s1q4==1)
...
call_yr <- as.numeric(year)-2
  ...
  school_data_SATT <- school_data_SATT %>%
    mutate(principal_satisfaction=attitude_fun_rev_5(m7shq1_satt),
           ...
           #seem to be too low compared to other values and the gdp
           m7shq2_satt = if_else(m7shq2_satt <700, as.numeric(NA), m7shq2_satt),
...
keep_info <-       c('school_code',
                     ...
                     'school_info_correct', 'm1s0q2_name', 'm1s0q2_code', 'm1s0q2_emis',
                     'survey_time', 'lat', 'lon', 'total_enrolled', 'm1saq4')
...
final_school_data <- school_data_preamble_short
  ...
  final_indicator_data_PEDG_F <- left_join(final_indicator_data_PEDG_F, teacher_assessment_weights, by = "school_code")
  final_indicator_data_PEDG_M <- left_join(final_indicator_data_PEDG_M, teacher_assessment_weights, by = "school_code")
...
data_list <- c(ind_dta_list, 'school_dta', 'school_dta_short', 
               #'school_dta_short_imp', 
               'school_data_preamble', 'final_school_data', 'teacher_questionnaire','teacher_absence_final', 'ecd_dta', 'teacher_assessment_dta', 'teacher_roster', 
               "indicators",  
               #'school_gdp', 
...
save(list=c(ind_dta_list,"school_dta_short", 
            #'school_dta_short_imp', 
            "indicators"            #'school_gdp' 
            ), file = file.path(confidential_folder, "school_indicators_data.RData"))
