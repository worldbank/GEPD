...
combined_df_raw <- bind_rows(NER_data, SLE_data,PER_data,JOR_data, ETH_data, RWA_data, MDG_data)
...
#build table
tab_df <- tab_df  %>%
  select(`Indicator Name`,Niger,`Sierra Leone`, Peru, Jordan, Rwanda, Ethiopia, Madagascar)
...
school_dta_short_anon <- school_dta_short_anon %>% select(-public)
...
# School Survey
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Sierra Leone 2022 - Urban/Rural",
...
#public/private
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Sierra Leone 2022 - Public/Private Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
...
#LIRE/non LIRE schools
 
      #get data with public/private status
        LIRE_status <- school_dta_anon %>%
          group_by(hashed_school_code) %>%
          select(LIRE) %>%
          filter(!is.na(LIRE)) %>% 
          summarise(LIRE=first(LIRE))
      ...
      sumstats_school_LIRE <- school_dta_short_anon %>%
        left_join(LIRE_status) %>%
        filter(LIRE=="LIRE School") 
      
      

      
      #add variable label
      sumstats_school_LIRE_df <-  svymeantab('sumstats_school_LIRE', strata=strat) %>%
        rename(
          mean_lire=mean,
          ci_lire=ci
        )
      ...
      #private
      sumstats_school_nonlire <- school_dta_short_anon %>%
        left_join(LIRE_status) %>%
        filter(LIRE=="Non-LIRE School")       
      
      
      
      #add variable label
      sumstats_school_nonlire_df <-  svymeantab('sumstats_school_nonlire', strata=strat) %>%
        rename(
          mean_nonlire=mean,
          ci_nonlire=ci
        )
      ...
      #now bind urban/rural with the main results
      sumstats_school_df_final <- sumstats_school_df %>%
        left_join(sumstats_school_LIRE_df) %>%
        left_join(sumstats_school_nonlire_df)
      ...
      sumstats_df <- sumstats_df %>%
        select(varlabel,  mean, ci, mean_lire, ci_lire, mean_nonlire, ci_nonlire)
      

      

      sumstats_df <- sumstats_df %>%
        mutate(ratio=(as.numeric(mean_nonlire))/as.numeric(mean_lire))
      
  sumstats_df %>%
    mutate(ratio=cell_spec(
      round(ratio,2), background = spec_color(ratio,  alpha=0.4, option='cividis', direction=-1)
    )) %>%
    kable(caption = "Summary Statistics of Dashboard Indicators - Niger 2022 - LIRE/Non LIRE Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
          , "Mean", "95% Confident Interval", "Ratio of Non LIRE Schools to LIRE Schools"),
          digits = 1, 
          escape=F) %>%
    kable_styling("striped") %>%
    add_header_above(c(" "=1, "Overall"=2,"LIRE"=2,"Non-LIRE"=2, " "=1))
...
learning_dataMean <- summarise(group_by(learning_data_plot, Gender , Type), MD = round(wtd.mean(student_knowledge, ipw, na.rm=TRUE),1))
  ...
  ggtitle("Résultats des évaluations des élèves de 4e année (CE2) par genre et par zone urbaine/rurale") +
  xlab(str_wrap("Statut urbain/rurals", width=75))+
  ylab(str_wrap("Pourcentage de réponses correctes à l'évaluation de 4e année (CE2)",35)) +
  scale_fill_discrete(name="Genre", 
                      labels=c('Tous les étudiants','Filles','Garçons' )) +
  theme_bw() +
...
regplots<- ggplot(data=df_reg, aes(x=ecd_student_knowledge, y=student_knowledge, weight=ipw, color="#0081a8")) +
  ...
  ylab(paste0("Score en 4ème année")) +
  xlab(paste0("Score en 1ère année")) +
  geom_richtext(
...
regplots +
  plot_annotation(title = str_wrap("Diagramme de dispersion des résultats moyens des élèves de CE2 par rapport aux résultats des élèves de CI",90))
...
regplots<- ggplot(data=df_reg, aes(x=teach_score, y=student_knowledge, weight=ipw, color="#0081a8")) +
  ...
  geom_richtext(
    aes(x=10,y=10,label = eq_plot_txt(df_reg, student_knowledge, teach_score), hjust=0.2)
  ) 



regplots +
  plot_annotation(title = str_wrap("Scatterplot of School Average 4th Grade Student Scores against TEACH Scores",90))
    ...
    summary(lm(student_knowledge~teach_score, data=df_reg))
...
#get list of de facto indicators
    ...
    main_indicator_labels<-c( 
                        'Enseignant - Attrait de la profession',
                         'Enseignant - Recrutement et carriere',
                         'Enseignant - Soutien', 
                         'Enseignant - Evaluation', 
                         'Enseignant - Suivi et responsabilité', 
                         #'Teaching - Intrinsic Motivation', 
                         # 'Inputs & Infrastructure - Monitoring',
                         "Gestion des écoles - Attrait de la profession",                   
                         "Gestion des écoles - Recrutement et carriere"  ,      
                         "Gestion des écoles - Soutien" ,                      
                         "Gestion des écoles - Evaluation"
    ...
    expert_dir <- "C:/Users/wb577189/OneDrive - WBG/GEPD/CNT/NER/NER_2022_GEPD/NER_2022_GEPD_v01_M/Data/Policy_Survey"
...
data_plot <- public_officials_dta_clean_anon %>%
  ...
  mutate(govt_tier=case_when(
    govt_tier=='District office ' ~ 'Districts',
    govt_tier=='Regional office ' ~ 'Régions',
...
## Rearrange the order of the boxplot


data_plot$govt_tier <- factor(data_plot$govt_tier , levels=c("MEN", "Régions", "Districts"))
...
dataMedian <- summarise(group_by(data_plot, indicators , govt_tier), MD = round(median(indicator_values, na.rm=T),1))
  ...
  ggplot(data_plot, aes(x=indicators, y=indicator_values, fill=govt_tier)) + #now plot the output in a bar graph
    ...
    scale_x_discrete(labels = str_wrap(c("Prise de décision impartiale",
                                         "Mandats et responsabilité",
                                         "Objectifs nationaux d'apprentissage",
                                         "Qualité de la bureaucratie"), width = 25)) + 
  xlab(str_wrap("Enquête auprès des fonctionnaires Indicateurs", width=75))+
  ylab("Échelle (1-5)") +
  ggtitle(str_wrap("Diagramme en boîte des indicateurs de l'enquête auprès des fonctionnaires", width=75)) +
  scale_fill_discrete(name="Niveau administratif") +
  theme_bw() +
    theme(legend.position = "bottom",
          axis.text = ggplot2::element_text(
                                      size=12
...
po_school <- final_school_data_anon %>% left_join(public_officials_dta_clean_anon)
