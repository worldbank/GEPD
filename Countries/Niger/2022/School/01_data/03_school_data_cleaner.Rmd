...
} else {download_folder <- choose.dir(default = "", caption = "Select folder to open data downloaded from API")
...
teacher_gender <- read_dta("C:/Users/wb631589/OneDrive - WBG/GEPD-Confidential/General/LEGO_Teacher_Paper/5_output_data/NER/NER_teacher_level_updated.dta") 
...
teacher_roster<-read_dta(file.path(download_folder, "TEACHERS_Clean.dta")) %>%
  mutate(teacher_name=m2saq2,
         teacher_number=TEACHERS__id) %>%
  select(-c("school_name_preload", "school_province_preload", "school_district_preload", "school_emis_preload", "m1s0q2_name", "m1s0q2_code", "m1s0q2_emis", "school_code"))
...
num_boys <- school_dta %>%
  select(interview__key, m4scq4n_girls, m4scq13_girls, school_code ) %>%
    mutate(m4scq13_girls = ifelse(m4scq13_girls == 98, NA, m4scq13_girls),
           m4scq13_girls = ifelse(school_code == 552107, 25, m4scq13_girls)) #clearly a typo
...
call_yr <- as.numeric(year)-1
  ...
  school_data_SATT <- school_data_SATT %>%
    mutate(principal_satisfaction=attitude_fun_rev_5(m7shq1_satt),
           #remove 0 negative and values that seem to be too low 
           m7shq2_satt = if_else(m7shq2_satt <10000, NA, m7shq2_satt),
...
g4_stud_weights <-school_dta %>%
  ...
  mutate(g4_stud_weight_temp = g4_stud_count/g4_assess_count, 
         ...
         #the missing weights are all coming from the missing data from the observation module, set the weight to 1 in these cases
...
#for the schools where numEligible4th_manual is lower than teacher_quest_count, replace it with numEligible4th_manual. this is particularly relevant for the schools with volunteer teachers in case only volunteer teachers get intervie
  mutate(numEligible4th_manual = if_else(teacher_quest_count>numEligible4th_manual & !is.na(teacher_quest_count), teacher_quest_count, numEligible4th_manual),         teacher_weight_component=numEligible4th_manual/teacher_quest_count,
        ...
        #3 schools with the missing weight, assign the weight of 1
...
g1_stud_weights<-school_dta %>%
  ...
  mutate(g1_class_weight = g1_teacher_count/1,
         ...
         #this school has some crazy value for the total number of students, set it to 3
         m6_class_count = ifelse(school_code==6661241, 3, m6_class_count),
...
data_set_updated <- read_csv(paste(sampling_folder, '/school_weights_revised_', currentDate,  '.csv', sep="")
                             ) %>%
...
data_list <- c(ind_dta_list, 'school_dta', 'school_dta_short', 'school_dta_short_imp', 'school_data_preamble', 'final_school_data', 'teacher_questionnaire','teacher_absence_final', 'ecd_dta', 'teacher_assessment_dta', 'teacher_roster', 
               ...
               'school_weights', 'final_indicator_data_PEDG')
