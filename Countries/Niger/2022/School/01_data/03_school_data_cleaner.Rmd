...
principal_scorer <- function(var_guess, var_actual,  margin1, margin2) {
  if_else(
    var_guess >= (var_actual * margin1) & var_guess <= (var_actual * margin1),
    1,
    0)
}
...
final_indicator_data_PKNW <- school_data_PKNW %>%
  ...
  mutate(add_triple_digit_pknw=principal_scorer(rowMeans(select(.,starts_with('m7sfq5_pknw')), na.rm=T), m5s2q1c_number,0.8,1.2),
         multiply_double_digit_pknw=principal_scorer(rowMeans(select(.,starts_with('m7sfq6_pknw')), na.rm=T), m5s2q1e_number ,0.8,1.2),
         complete_sentence_pknw=principal_scorer(rowMeans(select(.,starts_with('m7sfq7_pknw')), na.rm=T), m5s1q1f_grammer, 0.8,1.2),
