...
foreach v in teacher_absence teacher_assessment teacher_pedagogy teacher_questionnaire {
	...
	use "$dir/3_input_data/NER/NER_`v'", clear 
...
use `teacher_absence', clear // 2359
...
replace m2saq2 = "mme nouhou hadiza djibo1" if m2saq2 == "mme nouhou hadiza djibo" & teachers_id == 12 & school_code == 441313
...
*****
use `teacher_assessment', clear // 1106

replace m5sb_troster = "manirou rahila" if m5sb_troster == "rahila sayabou" & school_code == 442282 // seems to be a similar teacher, though not fully sure
replace m5sb_troster = "faizatou kimba abdou" if m5sb_troster == "mme faïza kimba" & school_code == 441844 
replace m5sb_troster = "mme adamou fatchima" if m5sb_troster == "fatima yacouba" & school_code == 55948 // not sure about this one 
replace m5sb_troster = "mme barchir rahama mahamadou" if m5sb_troster == "mme bachir rahama" & school_code == 7772502
replace m5sb_troster = "mme mamoudou aissa hassane" if m5sb_troster == "aissa hassane" & school_code == 6663830
replace m5sb_troster = "mme hassan aminatou rabiou" if m5sb_troster == "mme hassane aminatou" & school_code == 551663
replace m5sb_troster = "adama elhadji" if m5sb_troster == "mme halidou adama" & school_code == 441803
replace m5sb_troster = "maïmouna amadou" if m5sb_troster == "mme maimouna adamou maman" & school_code == 7771805
...
use `teacher_pedagogy', clear // 262

*correcting some names to be able to merge with the roster data 
replace m4saq1 = "omar lab mainassara d.bako" if m4saq1 == "oumar labo" & school_code ==  44976 

replace m4saq1 = "habsatou yahaya" if m4saq1 == "mme mamadou fannami habsatou" & school_code == 442148 // not sure about this one
...
*****
use `teacher_questionnaire', clear // 1058

*change some names to be able to merge with the roster since they seem to be the best matches
replace m3sb_troster = "99" if school_code == 55445
replace m3sb_troster = "99" if school_code == 7773608

replace m3sb_troster = "manirou rahila" if m3sb_troster == "rahila sayabou" & school_code == 442282
replace m3sb_troster = "mme hamissou l'araignée gueraou" if m3sb_troster == "larray guerao" & school_code == 441415
replace m3sb_troster = "zoulahatou beche" if m3sb_troster == "zoulha béché" & school_code == 441226
replace m3sb_troster = "mme hassan aminatou rabiou" if m3sb_troster == "mme hassane aminatou" & school_code == 551663
replace m3sb_troster = "mr baou moïse bansou donatien" if m3sb_troster == "mr moïse" & school_code == 8881215
replace m3sb_troster = "maimouna mainassara" if m3sb_troster == "maÏmouna madougou" & school_code == 33579
...
foreach i in teacher_quest teacher_asmnt teacher_pedag {
	...
	*drop the ones that were merged multiple times and only keep the relevant observation
	drop if txtmaster == "mme hassane aminatou" & txtusing == "mme alassane amina" & school_code == 551663
	drop if txtmaster == "mme harouna salmou" & txtusing == "mme souley halimatou hassane" & school_code == 552033
	drop if txtmaster == "aissa hassane" & txtusing == "mme aissatou hassane" & school_code == 6663830
	drop if txtmaster == "mme maimouna adamou maman" & inlist(txtusing, "mme abdoul moumouni mariama", "mme abdoul moumouni mariama") & school_code == 7771805 // unclear about these ones
	drop if txtmaster == "mme bachir rahama" & txtusing == "mme bachir rabi" & school_code == 7772502 // unclear whats hapenning there
	drop if txtmaster == "maÏmouna madougou" & txtusing == "mainassara madougou hakimi" & school_code == 33579
	drop if txtmaster == "madame harouna salmou salé" & txtusing == "mme souley halimatou hassane" & school_code == 552033
