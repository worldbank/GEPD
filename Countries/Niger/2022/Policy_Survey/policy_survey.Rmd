...
file_name <- 'Policy_Survey-Niger.xlsx'
...
expert_dta_teachers_final <- expert_dta_teachers_final %>%
  mutate(teacher_attraction=read_var(A4),
         teacher_salary=(1560/1816.2)) # GDP per capita https://data.worldbank.org/indicator/NY.GDP.PCAP.CD?locations=SL

#teacher selection and deployment
#
expert_dta_teachers_final <- expert_dta_teachers_final %>%
  mutate(criteria_admittance=read_var(A5),
         criteria_become=3, #adjusting to what we have in the data
...
expert_dta_teachers_final <- expert_dta_teachers_final %>%
  mutate(evaluation_law=read_var(A10),
         ...
         evaluation_criteria=4, #changing it manually based on the answers provided
...
expert_dta_school_management_final <- expert_dta_school_management_final %>%
  mutate(principal_rubric=read_var(C4),
         principal_factors=5) %>% #changing this based on the answers provided
...
expert_dta_school_management_final <- expert_dta_school_management_final %>%
  mutate(principal_monitor_law=read_var(C6),
         principal_monitor_criteria=4) %>% #changing this based on the answers provided
...
expert_dta_learners_final <- expert_dta_learners_final %>%
  mutate(iodization=3/4, #adjusting this manually to reflect what we have in the data
         iron_fortification=3/4,  #adjusting this manually to reflect what we have in the data
...
write_dta(expert_dta_final, path = paste0("C:/Users/wb631589/OneDrive - WBG//GEPD/CNT/NER/NER_2022_GEPD/NER_2022_GEPD_v02_M/Data/Policy_Survey/expert_dta_final.dta"))
```
