...
school_folder <- file.path(paste(project_folder,country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v02_RAW", sep="_"),"Data/anonymized/School/", sep="/"))
  ...
  final_indicator_data_INPT_anon <- final_indicator_data_INPT_anon %>%
    mutate(weight = 1/strata_prob)
  
  class_size <- weighted.mean(final_indicator_data_INPT_anon$m4scq4_inpt, w=final_indicator_data_INPT_anon$weight, na.rm=TRUE)
  
  #need to adjust this
  school_absence <- final_indicator_data_EFFT_anon %>%
    mutate(weight = 1/strata_prob)
  
  #need to adjust this
  absence <- weighted.mean(school_absence$sch_absence_rate, w=school_absence$weight, na.rm=TRUE) 
