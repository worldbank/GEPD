---
title: "Global Education Policy Dashboard - Pakistan - KP 2022"
...
Roadmap:  
- Below is a set of tables and charts containing findings for the Pakistan - KP 2022 Global Education Policy Dashboard survey. - We will start with breakdowns of our Practice indicators  
...
#list countries
countries <- c("PER", "JOR", "RWA", "MDG", "ETH", "SLE", "NER", "PAK-ICT", "PAK-KP")
...
PAK_KP_data <- read_csv(paste0(indicators_dir, "/GEPD_Indicators_API_", 'PAK_KP', ".csv")) %>%
  mutate(country="Pakistan-KP",
         date='2022') 

PAK_ICT_data <- read_csv(paste0(indicators_dir, "/GEPD_Indicators_API_", 'PAK_ICT', ".csv")) %>%
...
combined_df_raw <- bind_rows(PAK_KP_data,PAK_ICT_data, NER_data,SLE_data,PER_data,JOR_data, ETH_data, RWA_data, MDG_data)
...
#build table
tab_df <- tab_df  %>%
  select(`Indicator Name`,`Pakistan-KP`,`Pakistan-ICT`,`Niger`,`Sierra Leone`, Peru, Jordan, Rwanda, Ethiopia, Madagascar)
...
svymeantab <- function(data, strata) {
  ...
  get(data) %>%
    ...
    select(one_of(indicators_list), Location, Gender, urban_rural, ipw ) %>%
    pivot_longer(cols=one_of(indicators_list),
                 names_to = 'indicators',
                 values_to='value') %>%
    as_survey_design(strata=strata,
                     weight=ipw) %>%
    select(-c( 'Location', 'Gender','urban_rural', 'ipw')) %>%
    group_by(indicators) %>%
...
# School Survey
    ...
    sumstats_school_df <- svymeantab('sumstats_school', strata=strata)
    ...
    sumstats_school_urban_df <- svymeantab('sumstats_school_urban', strata=strata)
    ...
    #rural
      ...
      sumstats_school_rural_df <- svymeantab('sumstats_school_rural', strata=strata)
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Pakistan - KP 2022 - Urban/Rural",
...
data_plot <- public_officials_dta_clean_anon %>%
  ...
  mutate(govt_tier=case_when(
    govt_tier=='District office ' ~ 'Regional office (or equivalent)',
    govt_tier=='Ministry of Education '  ~ 'Ministry of Education (or equivalent)'
  )) %>%
  group_by(govt_tier) %>%
