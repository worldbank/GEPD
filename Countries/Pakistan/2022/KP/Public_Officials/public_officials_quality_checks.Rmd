...
load(file="C:/Users/wb577189/OneDrive - WBG/My files/Dashboard (Team Folder)/Country_Work//Pakistan_all/2021/Data/01. Pilot/clean/public_officials/paradata.RData")
```
...
overall_time_comb<-bind_rows(overall_time, overall_time_avg) %>%
  ...
  ungroup() %>%
  select( Overall, DEM, NLG, ACM, QB, IDM ) %>%
  my_skim() %>%
           yank("numeric") %>%
           mutate(variable=skim_variable) %>%
           select(variable, mean, sd, p0, p25, p50, p75, p100, complete,  hist)  %>%
  arrange(factor(variable, levels=c('Overall', 'NLG','ACM' ,'QB', 'IDM','DEM'))) %>%
  mutate(variable=factor(variable, levels=c('NLG','ACM' ,'QB', 'IDM','DEM', 'Overall'),
                         labels=c('National Learning Goals','Mandates & Accountability' ,'Quality of Bureaucracy', 'Impartial Decision Making', 'Demographics', 'Overall')))
```
...
load("C:/Users/wb577189/OneDrive - WBG/My files/Dashboard (Team Folder)/Country_Work//Pakistan_all/2021/Data/01. Pilot/clean/public_officials/public_officials_survey_data.RData")
...
write_excel_csv(sumstats_df, 'C:/Users/wb577189/OneDrive - WBG/My files/Dashboard (Team Folder)/Country_Work//Pakistan_all/2021/Data/01. Pilot/clean/public_officials/GEPD_Public_Officials_Indicators.csv')
```
