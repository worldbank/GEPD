...
if (str_to_lower(Sys.getenv("USERNAME")) == "wb469649"){
  ...
  backup_onedrive="no"
  save_folder_onedrive <- file.path(paste("C:/Users/wb577189/OneDrive - WBG/My files/Dashboard (Team Folder)/Country_Work/",country_name,"_all/", year,"/Data/02. Provinces/KP/school_survey/clean/", sep=""))
} else {
...
PAK_2023_data <- read_csv(paste0(indicators_dir, "/GEPD_Indicators_API_", 'PAK_KP', ".csv")) %>%
  mutate(country="Pakistan KP-2023",
         date='2023') 
...
combined_df_raw <- bind_rows(PAK_2023_data, JOR_2023_data,JOR_data,NER_data, SLE_data,PER_data, ETH_data, RWA_data, MDG_data)
...
Pakistan_2023_col <- tab_color_df$`Pakistan KP-2023`
...
#build table
tab_df <- tab_df  %>%
  select(`Indicator Name`,`Pakistan KP-2023`,`Jordan-2023`, `Jordan-2019`, Niger,`Sierra Leone`, Peru, Rwanda, Ethiopia, Madagascar)
...
#colors
ovr_table <- ovr_table %>%
    bg(j = c('Pakistan KP-2023'),
     bg = Pakistan_2023_col) %>%
  bg(j = c('Jordan-2023'),
     bg = Jordan_2023_col) %>%
  bg(j = c('Niger'),
     bg = NER_col) %>%
