...
saber_frame <- 'SamplingData_Baseline.dta'
...
saber_sample_df <- read_dta(paste(sampling_folder, saber_frame, sep="/"))

#join the frames
df_frame_joined_df <- df_raw %>%
  left_join(saber_sample_df %>% filter(public==1), by=c('csl_emis_code'='emiscode'))

districts <- df_frame_joined_df %>% group_by(sampled, csl_district, csl_area) %>% summarise(n=n())
```
...
sample_df_200 <- df_updated %>%
  mutate(sampled=if_else(is.na(sampled),0,sampled)) %>%
  ...
  group_by(csl_district,csl_area, sampled) %>%
