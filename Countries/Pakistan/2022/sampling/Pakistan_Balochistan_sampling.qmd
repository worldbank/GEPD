...
strata_df_200 <- read_csv(paste0(sampling_folder, "/GEPD_Balochistan_strata_200_",'2023-07-05',".csv"))
...
#| tbl-cap: Number of Schools by District in the Sample
...
## August 28, 2023 Replacements

23 schools fell into Tehsils that were judged to pose a risk to the enumerators, either because of war/conflict, other security issues, or very remote terrain.

```{r}
#| label: aug28replacements

#read in the list of schools to be replaced
bemis <- c( '09682'
            ,'06924'
            ,'06742'
            ,'08356'
...
sample_chosen <- read_csv(paste0(sampling_folder, "/GEPD_Balochistan_sample_200_","2023-07-05",".csv")) %>%
  mutate(sample="Sampled School") %>%
  select(-YearofNonFunctinal) %>%
  filter(BemisCode %in% bemis)

#add sample schools back to original database
data_set_updated_chosen <- df_updated_gepd %>%
  left_join(sample_chosen) %>%
  filter(!(Tehsil %in% tehsils))

#get list of tehsil
sampled_districts <- sample_chosen %>%
  #replace DERA BUGTI with KOHLU
  mutate(District=case_when(
    District=="DERA BUGTI" ~ "KOHLU",
    TRUE ~ District
  )) %>%
  group_by( District) %>% 
  summarise(sampled_districts=n()
...
sample_replace2 <- sample_chosen %>%
    bind_rows(sample_chosen) %>% #add a duplicate observation for matching to two replacement schools
    mutate(District_sort=case_when(
    District=="DERA BUGTI" ~ "KOHLU",
    TRUE ~ District
  )) %>%
    arrange(District_sort,Location, BemisCode)  
...
write_excel_csv(sample_replace, paste(sampling_folder, '/GEPD_Balochistan_security_replacement_schools_', Sys.Date(),  '.csv', sep=""))



```

## September 18, 2023 Replacements

Another round of replacements were needed, because of security issues.

```{r}
#| label: sep18replacements
...
#define sampled schools in sample dataset
sample_chosen <- read_csv(paste0(sampling_folder, "/GEPD_Balochistan_sample_200_","2023-08-28",".csv")) %>%
  mutate(sample="Sampled School") %>%
  select(-YearofNonFunctinal) %>%
  filter(BemisCode %in% bemis)
...
sampled_districts <- sample_chosen %>%
  #replace DERA BUGTI with KOHLU
  mutate(District=case_when(
    District=="DERA BUGTI" ~ "JHAL MAGSI",
    District=="KOHLU" ~ "SIBI",
    TRUE ~ District
  )) %>%
  group_by( District) %>% 
