...
tab_fun <- function(data) {
  ...
  urban_tab <- data %>%
    group_by(Location) %>%
    summarise(N=n(),
              student_count=sum(total_enrollment),
              class_4=sum(class_4, na.rm=TRUE)) %>%
    ungroup() %>%
    mutate(`Percentage Schools` = paste0(round(100*N/sum(N),1),"%"),
  ...
  total_tab <- bind_rows(
    ...
    #owner_tab %>% rename(`Sub-group`=SchoolType) %>% mutate(group="School Ownership")
    )
...
```
 ...
 Typically for the GEPD only schools with at least 3 1st grade and 3 4th grade students will be selected.  Applying these restrictions results in `r nrow(df_updated_gepd_nmd)` schools eligible. 

However, there is interest in interviewing pre-primary students and students in 5th grade for the AIM-ECD assessment and the AMPL-b assessment.  As a result, additionaly schools are excluded that do not have at least 3 kachi and 3 5th grade students.  This results in `r nrow(df_updated)` being eligible.  There were `r nrow(df_updated_gepd_nmd %>% filter(class_prep<3))` schools that were eliminated because of the preprimary restriction and `r nrow(df_updated_gepd_nmd %>% filter(class_5<3))` because of the grade 5 restriction.
...
weights_df_200 <- df_updated_gepd_nmd %>%
  filter(!(Location=="NULL")) %>%
  left_join(strata_df_200) %>%
  mutate(
    totalstudents=sum(total_enrollment),
...
write_excel_csv(weights_df_200, file=paste0(sampling_folder, "/GEPD_KP_weights_200_",Sys.Date(),".csv"))
...
region_tab_sample <- sample_df_200 %>%
  ...
  as_survey_design(strata=c('Gender', 'Location'),
...
#ownership
owner_tab_sample <- sample_df_200 %>%
  mutate(school_weight=1/strata_prob) %>%
  as_survey_design(strata=c('Gender', 'Location'),
                   weight=school_weight)  %>%
  group_by(SchoolType) %>%
  ...
  select(-ends_with("_se")) %>%
  rename(`School Ownership`=SchoolType)
...
total_tab <- bind_rows(
  ...
  #owner_tab_sample %>% rename(`Sub-group`="School Ownership") %>% mutate(group="School Ownership")
  )
...
sample_df_200 %>%
  ...
  summarise(n=n(),
            ...
            class_4=sum(class_4, na.rm=TRUE)) %>%
  ungroup() %>%
  mutate(share=total_enrollment/sum(total_enrollment)) %>%
  flextable() %>%
  add_header_lines('Number of Schools by District') %>%
  autofit()
...
sample_chosen <- read_csv(paste0(sampling_folder, "/GEPD_KP_sample_200_","2022-11-08",".csv")) %>%
