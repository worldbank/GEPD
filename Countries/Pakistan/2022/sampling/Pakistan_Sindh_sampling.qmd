...
file_frame<-"Key Indicator (WB) with Class wise enrollment.xlsx"
frame_sheet1 <- "Key Indicator"
frame_sheet2 <- "Class wise enrollment"
...
df_enrollment <- read_excel(paste(sampling_folder, file_frame, sep="/"),
              sheet=frame_sheet2) %>%
  rename(semis_code=`Semis Code`)

df_combined <- df_raw %>%
  left_join(df_enrollment) %>%
  mutate(
    total_enrollment=`Total`,
    class_prep=`Katchi Female`+`Katchi Male`,                
    class_1=`Class-I Male`+`Class-I Female`,                   
    class_1_girls=`Class-I Female`,                   
    class_1_boys=`Class-I Male`,                   
    class_2=`Class-II Male`+`Class-II Female`,                   
    class_3=`Class-III Male`+`Class-III Female`,                     
    class_4=`Class-IV Male`+`Class-IV Female`,                     
    class_4_girls=`Class-IV Female`,                    
    class_4_boys=`Class-IV Male`,                    
    class_5=`Class-V Male`+`Class-V Female`,                
    class_6=`Class-VI Male`+`Class-VI Female`,
  )


#keep open schools
df_combined <- df_combined %>%
  filter(status_detail !='Closed')
...
#how many students in grade 4 eliminated by various restrictions
sum(df_combined$class_4, na.rm=TRUE)
sum(df_updated_gepd$class_4, na.rm=TRUE)
sum(df_updated$class_4, na.rm=TRUE)

```

Typically for the GEPD only schools with at least 3 1st grade and 3 4th grade students will be selected.  The sampling frame does not have student counts by grade, but has the number primary classrooms, so a restriction is imposed to have at least one primary room.  Applying these restrictions results in `r nrow(df_updated_gepd)` schools eligible.
...
Basic summary stats of restricted frame

```{r}
#| label: tbl-sumstats
#| tbl-cap: Summary Statistics for Balochistan restricted Sampling Frame
#level
df_updated %>% 
...
tab_fun <- function(data) {
  ...
  region_tab <- data %>%
    ...
    summarise(N=n(),
              ...
              class_4=sum(class_4, na.rm=TRUE)) %>%
    ungroup() %>%
  ...
  total_tab <- bind_rows(
    #region_tab %>% rename(`Sub-group`=district) %>% mutate(group="District"),
    urban_tab %>% rename(`Sub-group`=location) %>% mutate(group="Urban/Rural")
    )
...
write_excel_csv(strata_df_200, file=paste0(sampling_folder, "/GEPD_Sindh_strata_200_",Sys.Date(),".csv"))
```
...
```{r large, echo=TRUE}
# get numbers to select from each district
strata_df_200 <- read_csv(paste0(sampling_folder, "/GEPD_Sindh_strata_200_2023-10-12_rev.csv"))



# set the numbers of schools selected by strata (grade)

sample_df_200 <- df_updated_gepd %>%
...
#pick one extra school that was not included by mistake
sample_chosen <- sample_df_200 %>%
  mutate(sample="Sampled School") 



write_excel_csv(sample_df_200, file=paste0(sampling_folder, "/GEPD_Sindh_sample_200_",Sys.Date(),".csv"))

weights_df_200 <- df_updated_gepd %>%
  left_join(strata_df_200) %>%
  ungroup() %>%
  mutate(
...
write_excel_csv(weights_df_200 %>% 
                  select(semis_code, school_name, district, taluka, location, shift, `School Level`, strata_prob, ipw)
                  , file=paste0(sampling_folder, "/GEPD_Sindh_weights_200_",Sys.Date(),".csv"))
...
region_tab_sample <- sample_df_200 %>%
  ...
  group_by(district) %>%
  summarise(N=n(),
            student_count=survey_total(total_enrollment),
            g4_student_count=survey_total(class_4, na.rm=TRUE)) %>%
  ungroup() %>%
  mutate(`Percentage Schools` = paste0(round(100*N/sum(N),1),"%"),
         `Percentage of Primary Students`=paste0(round(100*student_count/sum(student_count),1),"%"),
...
Table. Number of Schools Selected in the Sample by District

```{r}
#| label: tbl_sample_district
#| tbl-cap: Number of Schools Selected in the Sample by District
#number of schools by district in a flextabe
sample_df_200 %>%
  group_by(district) %>%
  summarise(n=n(),
            total_enrollment=sum(total_enrollment, na.rm=TRUE)) %>%
  rename(
    `Number of Schools`="n",
    `Number of Students`="total_enrollment"
  ) %>%
  flextable() %>%
  add_header_lines("Number of Schools Selected in the Sample by District") %>%
  autofit()
...
sample_chosen <- read_csv(paste0(sampling_folder, "/GEPD_Sindh_sample_200_","2023-10-12",".csv")) %>%
  mutate(sample="Sampled School") 
...
sample_replace <- data_set_updated_chosen %>%
  ...
  arrange(district,location, semis_code) 
...
sample_replace$replaced_EmisCode=sample_replace2$semis_code
sample_replace$replaced_District=sample_replace2$district



# sample_replace %>%
#   select(`School Name`, semis_code, district, replaced_School_Name, replaced_EmisCode, replaced_District) %>%
#   DT::datatable( caption = 'List of Replacement Schools Chosen for Public School Sample') 

sample_replace <- sample_replace %>%
  select(`School Name`, semis_code, district, replaced_School_Name, replaced_EmisCode, replaced_District, everything())
  
write_excel_csv(sample_replace, paste(sampling_folder, '/GEPD_Sindh_replacement_schools_', Sys.Date(),  '.csv', sep=""))
...
df_updated_gepd %>%
  ungroup() %>%
  summarise(total_teachers=mean(total_teacher),
            electricity=mean(electricity_availability=='Yes'), 
            drinking_water=mean(water_available=='Yes')) %>%
  flextable() %>%
  add_header_lines("Population estimate")

df_updated_gepd %>%
  ungroup() %>%
  summarise(total_teachers=wtd.mean(total_teacher, weight=class_4),
            electricity=wtd.mean(electricity_availability=='Yes', weight=class_4), 
            drinking_water=wtd.mean(water_available=='Yes', weight=class_4)) %>%
  flextable() %>%
  add_header_lines("Population estimate")
...
sample_df_200 %>%
  ...
  as_survey_design(strata=c('district','location'),
                   weight=ipw) %>%
    summarise(total_teachers=survey_mean(total_teacher),
            electricity=survey_mean(electricity_availability=='Yes'), 
            drinking_water=survey_mean(water_available=='Yes')
              ) %>%
  flextable() %>%
