...
file_frame<-"1st Draft Data ASC 2022-23.xlsx"
frame_sheet <- "ASC"
...
df_raw<-read_excel(paste(sampling_folder, file_frame, sep="/"),
              sheet=frame_sheet) %>% 
  mutate(
    class_prep_rooms=ecce,    
    class_primary_rooms=primary,
    class_secondary_rooms=post_primary,
    class_language = case_when(
      sindhi_medium_enrolment==total_enrollment ~ "Sindhi School",
      urdu_medium_enrolment==total_enrollment ~ "Urdu School",
      TRUE ~ "Mixed or other"
    )
  )
...
df_updated_gepd <- df_raw %>%
  filter(class_primary_rooms>=1) 


df_updated <- df_raw %>%
  filter(class_prep_rooms>=1,
         class_primary_rooms>=1)
```
