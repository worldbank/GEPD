...
shared_country_name <-'Nigeria - Edo State'
country <- "Gabon"
year <- '2023'
api_user <- "GEPD_api_EDO"
need_api=1
#########################
# File paths #
#########################
...
if (str_to_lower(Sys.getenv("USERNAME")) == "wb469649"){
  ...
  download_folder <-file.path(paste(project_folder,shared_country_name,year,"Data/School/raw/", sep="/"))
  save_folder <- file.path(paste(project_folder,shared_country_name,year,"Data/School/clean/", sep="/"))
  
  backup_onedrive="yes"
  save_folder_onedrive <- file.path(paste(project_folder,shared_country_name,year,"Data/School/clean/", sep="/"))
...
source('03_school_paradata.R', local=TRUE)
 ...
 rmarkdown::render('04_school_data_quality_checks.Rmd',
                   output_file =  paste("school_data_quality_checks_",shared_country_name,".html", sep=''),
