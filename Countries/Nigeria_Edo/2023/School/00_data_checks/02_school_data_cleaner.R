...
teacher_roster<-read_dta(file.path(download_folder, "TEACHERS_combined.dta")) %>%
...
###########################
school_dta<-read_dta(file.path(download_folder, "EPDash_combined.dta"))
...
#########################################
teacher_questionnaire<-read_dta(file.path(download_folder, "questionnaire_roster_combined.dta"))
...
teacher_absence_dta<-read_dta(file.path(download_folder, "TEACHERS_combined.dta"))
...
teacher_assessment_dta <- read_dta(file.path(download_folder, "teacher_assessment_answers_combined.dta")) %>%
  left_join(school_data_preamble) %>%
  select(preamble_info, everything()) 
...
final_indicator_data_PEDG<- school_dta %>%
  ...
  write_excel_csv(path = paste(save_folder, "teach_raw_data.csv", sep="/")) %>%
  group_by(school_code) %>%
...
assess_4th_grade_dta<-read_dta(file.path(download_folder, "fourth_grade_assessment_combined.dta")) %>% 
  left_join(school_dta %>% select(school_code_preload, school_name_preload, interview__id))
...
tevl<-c(
  'm3sbq7_tmna__2', 'm3sbq7_tmna__3', 'm3sbq7_tmna__4','m3sbq7_tmna__5', 'm3sbq7_tmna__6', 'm3sbq7_tmna__97',
...
school_data_SATT <- school_data_SATT %>%
  ...
  mutate(principal_salary_score=case_when(m7shq2_satt==1~3 ,
                                          m7shq2_satt==2~4 ,
                                          m7shq2_satt>2~5 ,
...
for (i in indicator_names ) {
  if (exists(paste("final_indicator_data_",i, sep=""))) {
    ...
    if (!exists('final_school_data')) {
      ...
      print(i)
      write_csv(temp, file = file.path(paste(save_folder,"/Indicators", sep=""), paste(i,"_final_indicator_data.csv", sep="")))
      ...
      write_csv(temp, file = file.path(paste(save_folder,"/Indicators", sep=""), paste(i,"_final_indicator_data.dta", sep="")))
...
write.csv(school_weights, file = file.path(save_folder, "school_weights.csv"))
#write_dta(school_weights, path = file.path(save_folder, "school_weights.dta"), version = 14)
if (backup_onedrive=="yes") {
...
data_list <- c(ind_dta_list, 'school_dta', 'school_dta_short', 'school_dta_short_imp', 'school_data_preamble', 'final_school_data', 'teacher_questionnaire','teacher_absence_final', 'ecd_dta', 'teacher_assessment_dta', 'teacher_roster', 
               "indicators", 'school_gdp', 'assess_4th_grade_anon', 'ecd_dta_anon')

save(list=data_list, file = file.path(save_folder, "school_survey_data.RData"))

save(list=c(ind_dta_list,"school_dta_short", 'school_dta_short_imp', "indicators",  'school_gdp' ), file = file.path(save_folder, "school_indicators_data.RData"))
