...
combine_main_files <- function(file_names) {
  ...
  #read in  file
  file_list <- list.files(download_folder, pattern = paste0(file_names,"\\.tab$"), full.names = TRUE, recursive = TRUE)
  ...
  para_df<-read_dta(paste(download_folder, "paradata_combined.dta", sep="/"))
...
indicators <- read_delim(paste0(here(),'/Indicators/indicators.md'), delim="|", trim_ws=TRUE)
...
raw_school_dta<-read_dta(paste(download_folder,"/","EPDash_combined.dta", sep = ""))
...
raw_teacher_questionnaire<-read_dta(file.path(download_folder, "/questionnaire_roster_combined.dta"))
...
raw_teacher_absence_dta<-read_dta(file.path(download_folder, "/TEACHERS_combined.dta"))
...
raw_ecd_dta<-read_dta(file.path(download_folder, "/ecd_assessment_combined.dta"))
...
raw_assess_4th_grade_dta<-read_dta(file.path(download_folder, "/fourth_grade_assessment_combined.dta"))
  # 
...
para_df_tab <- para_df %>%
  select( responsible, date, timestamp, module, section, indicator, question, varlabel, timelength_sec, interview__id) %>%
  group_by(interview__id, section) %>% 
...
school_dta_preamble_id <- school_dta %>%
  mutate( school_code_preload=if_else(school_info_correct==1,as.double(school_code_preload), as.double(m1s0q2_emis))) %>% 
  select(interview__id, interview__key, school_code_preload, school_name_preload, school_province_preload, school_district_preload, lat, lon ) %>% 
  mutate(across(everything(), as.character))

school_modules_complete <- para_df_module %>%
  ungroup() %>%
  select(-c('timelength_sec')) %>%
  ...
  select(school_code_preload, M1, M2, M3, M4, M5, M6, M7, M8,  
...
# 
# para_df_tab <- para_df %>%
#   select( responsible, date, module, section, indicator, question, varlabel, timelength_sec, ?..interview__id,)
# 
...
#          datatable(linked_df, 
#                    colnames=c('Interview Code'='?..interview__id', 'Enumerator' = 'responsible', 'Date' = 'date', 'Module' = 'module', 'Section' = 'section',
