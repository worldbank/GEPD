...
school_map <- school_map %>%
  mutate(missings=(is.na(student_knowledge) | is.na(absence_rate) | is.na(student_attendance) | is.na(content_knowledge) | 
                   ...
                   | is.na(sch_monitoring) | is.na(sch_management_attraction) | is.na(sch_selection_deployment) 
                   | is.na(sch_support) | is.na(principal_evaluation))
  ...
  mutate(total_missing=is.na(student_knowledge) + is.na(absence_rate) + is.na(student_attendance) + is.na(content_knowledge) + 
         ...
         is.na(teacher_monitoring) + is.na(sch_monitoring) + is.na(sch_management_attraction) + is.na(sch_selection_deployment) +
         is.na(sch_support) + is.na(principal_evaluation))
...
map <- function(df_input) {    leaflet(df_input, height=1200, width=1020) %>% 
  ...
  addAwesomeMarkers(lng=~lon, lat= ~lat, icon=icons,
                    popup = paste("Name: ", school_map$school_name_preload, " <br>",
                                  ...
                                  "Inputs & Infrastructure Monitoring (De Facto): ", round(school_map$sch_monitoring, digits=2), "<br>",
                                  "School Management Attraction (De Facto): ", round(school_map$sch_management_attraction, digits=2), "<br>",
                                  "School Management Selection & Deployment: ", round(school_map$sch_selection_deployment, digits=2), "<br>",
                                  "School Management Support: ", round(school_map$sch_support, digits=2), "<br>",
...
#validate 
out <- confront(teacher_absence_final, EFFT_rules)
summary(out)
plot(out)
```
...
#validate 
out <- confront(assess_4th_grade_anon, LERN_rules)
summary(out)
plot(out)
```
