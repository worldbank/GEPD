...
file_name <- 'PolicySurvey - final.xlsx'
...
#start with teachers
##########################
  expert_dta_teachers <- readxl::read_xlsx(path=paste(expert_dir, file_name, sep="/"), sheet = 'Enseignants', .name_repair = 'universal') %>% 
  fill(Question..) %>% 
  group_by(Question..) %>%
...
# Inputs
##############################
expert_dta_inputs <- readxl::read_xlsx(path=paste(expert_dir, file_name, sep="/"), sheet = 'Intrants', .name_repair = 'universal') %>% 
  fill(`Question..`) %>%
  group_by(Question..) %>%
  fill(Scores, .direction='downup') %>%  
  filter(!is.na(Question)) 
...
# School Management
###############################
expert_dta_school_management <- readxl::read_xlsx(path=paste(expert_dir, file_name, sep="/"), sheet = 'Management ecole', .name_repair = 'universal') %>% 
  fill(Question..) %>% 
  group_by(Question..) %>%
  fill(Scores, .direction='downup') %>%
  filter(!is.na(Question))
...
# Learners 
################################
expert_dta_learners <- readxl::read_xlsx(path=paste(expert_dir, file_name, sep="/"), sheet = 'Eleves', .name_repair = 'universal') %>% 
fill(Question..) %>% 
  group_by(Question..) %>%
  fill(Scores, .direction='downup') %>%
  filter(!is.na(Question))
