...
final_indicator_data_PKNW <- school_data_PKNW %>%
  ...
  mutate(
  add_triple_digit_pknw_shr=rowSums(select(.,starts_with('m7sfq5_pknw')), na.rm=T),
           multiply_double_digit_pknw_shr=rowSums(select(.,starts_with('m7sfq6_pknw')), na.rm=T),
           complete_sentence_pknw_shr=rowSums(select(.,starts_with('m7sfq7_pknw')), na.rm=T),    
