...
public_officials_dta_short$hashed_office_id <-as.character(lapply(public_officials_dta_short$location, function(x) {digest(x, algo="xxhash64", seed=531254, serialize = T)}))
public_officials_dta_short$hashed_province <-as.character(lapply(public_officials_dta_short$province, function(x) {digest(x, algo="xxhash64", seed=531254, serialize = F)}))
public_officials_dta_short$hashed_district <-as.character(lapply(public_officials_dta_short$district, function(x) {digest(x, algo="xxhash64", seed=531254, serialize = F)}))

#save a hashed version of the dataset, to produce a link file
key<-public_officials_dta_short %>%
  select(id_code, position, location, province, district, hashed_position, hashed_office_id, hashed_province, hashed_district) 
...
for (i in data_list ) {
  if (exists(i)) {
    ...
    if ("position" %in% colnames(temp)) {
      temp <- temp %>%
        left_join(key) %>%
        select(id_code, hashed_position, hashed_office_id, hashed_province, hashed_district, everything())
    }
    ...
    temp <- temp %>%
      select(-starts_with('position'), -starts_with('office'), -one_of('region_code', 'district_code', 'district', 'province', 'location')) %>% # drop position names and address
      select(-one_of('survey_time', 'lat','lon')) %>% #drop geo-codes
      select(-contains('district')) %>%
      select(-contains('province')) %>%
...
save(list=anon_dta_list, file = file.path("C:/Users/wb631589/OneDrive - WBG/GEPD/CNT/PER/PER_2019_GEPD/PER_2019_GEPD_v02_M/Data/Public_Officials/public_officials_indicators_data_anon.RData"))
