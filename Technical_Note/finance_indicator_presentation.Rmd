...
wdi_df <- wb_data(
  indicator = c("SE.XPD.PRIM.PC.ZS", "SP.POP.TOTL", "NY.GDP.PCAP.PP.CD", "HD.HCI.HLOS"),
...
finance_df <- finance_sdmx_df %>%
  pivot_wider(
    id_cols = c("LOCATION", "obsTime"),
    names_from = NATMON_IND,
    values_from = obsValue
  ) %>%
  mutate(
    spending_per_sap = X_PPP_1_FSGOV * 1000000 / SAP_1,
...
income_df <- finance_df %>%
  ...
  summarise(
    spending_per_sap = wtd.mean(spending_per_sap, w = `SAP_1`),
