...
wdi_df <- wb_data(indicator=c('SE.XPD.PRIM.PC.ZS','SP.POP.TOTL','NY.GDP.PCAP.PP.CD'),
...
finance_df <- finance_sdmx_df %>%
  ...
  group_by(iso3c) %>%
  fill(spending_per_sap,spender_per_sap_gdp, spending_per_pupil, SE.XPD.PRIM.PC.ZS, .direction = 'downup') %>%
  group_by(iso3c) %>%
