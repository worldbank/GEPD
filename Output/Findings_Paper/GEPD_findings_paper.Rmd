...
TCD_data <- read_csv(paste0(indicators_dir, "/GEPD_Indicators_API_", 'TCD', ".csv")) %>%
  mutate(country="Chad",
         date='2023') %>%
  select(-year)

GAB_data <- read_csv(paste0(indicators_dir, "/GEPD_Indicators_API_", 'GAB', ".csv")) %>%
  mutate(country="Gabon",
         date='2023') %>%
  select(-year)

ICT_data <- read_csv(paste0(indicators_dir, "/GEPD_Indicators_API_", 'PAK_ICT', ".csv")) %>%
  mutate(country="Pakistan - ICT",
         date='2022') %>%
  select(-year)

KP_data <- read_csv(paste0(indicators_dir, "/GEPD_Indicators_API_", 'PAK_KP', ".csv")) %>%
  mutate(country="Pakistan - KP",
         date='2023') %>%
  select(-year)

JOR_23_data <- read_csv(paste0(indicators_dir, "/GEPD_Indicators_API_", 'JOR_2023', ".csv")) %>%
...
combined_df_raw <- bind_rows(PER_data,JOR_data, RWA_data, ETH_data,  MDG_data, SLE_data, NER_data,
                             TCD_data, GAB_data, ICT_data, KP_data, JOR_23_data)
...
Jordan_23_col <- tab_color_df$`Jordan - 2023`
Gabon_col <- tab_color_df$Gabon
Chad_col <- tab_color_df$Chad
ICT_col <- tab_color_df$`Pakistan - ICT`
KP_col <- tab_color_df$`Pakistan - KP`


#build table
tab_df <- tab_df  %>%
  select(`Indicator Name`, Peru, `Jordan - 2019`, Rwanda, Ethiopia, Madagascar, `Sierra Leone`, Niger, `Jordan - 2023`, Gabon, Chad, `Pakistan - ICT`, `Pakistan - KP`)
...
The countries in which data has been collected for the Global Education Policy Dashboard consist of low ad middle income countries with a range of development outcomes.  The countries range in population size from a population of around 10 million in Jordan to a population of more than 110 million in Ethiopia.  The demographic breakdowns of the countries vary as well.  The share of the population under 14 ranges from around 25% in Peru to close to 40% in Ethiopia, Madagascar, and Rwanda.  GNI per capita ranges from around \$1,400 in purchasing power parity adjusted terms in Madagascar to around \$11,000 in Peru.  The rates of extreme poverty range from nearly 79% in Madagscar to around 0.1% in Jordan.  The rate of stunting exceeds 30% of children under 5 in Ethiopia, Madagscar, and Rwanda.  Finally, when looking at the learning adjusted years of school, combining quantity and quality of schooling from the [Human Capital Index](https://www.worldbank.org/en/publication/human-capital), this ranges an average of 3.93 learning adjusted years of schooling in Rwanda to 8.63 years in Peru.

```{r}
#pull data on other outcomes, GNI per capita, Population, HCI, poverty, stunting

dev_out_df <- WDI::WDI(
    country=c('ET','RW','PE','JO','MG', 'SL', 'NE', "GA", "TD", "PK"),
    indicator=c("SP.POP.TOTL","SP.POP.0014.TO", "NY.GNP.PCAP.PP.KD","SI.POV.DDAY", "HD.HCI.LAYS", "SH.STA.STNT.ZS"  ),
    start=2010,
    end=2020,
    #latest=1,
    extra = TRUE
  ) %>%
  fill(c("SP.POP.TOTL", "SP.POP.0014.TO", "NY.GNP.PCAP.PP.KD","SI.POV.DDAY", "HD.HCI.LAYS", "SH.STA.STNT.ZS"  )) %>%
  group_by(iso3c) %>%
  arrange(year) %>%
  filter(row_number()==n()) %>%
  ungroup() %>%
  select(country, c("SP.POP.TOTL","SP.POP.0014.TO", "NY.GNP.PCAP.PP.KD","SI.POV.DDAY", "HD.HCI.LAYS", "SH.STA.STNT.ZS"  )) 


names <-  stack(sapply(dev_out_df, function(x) attr(x, "label")))

dev_out_df <- dev_out_df %>%
#  mutate(share_pop_14=SP.POP.0014.TO/SP.POP.TOTL) %>%
  mutate(across(c("SP.POP.TOTL","SP.POP.0014.TO", "NY.GNP.PCAP.PP.KD","SI.POV.DDAY", "HD.HCI.LAYS", "SH.STA.STNT.ZS"  ), scales::comma)) 
...
rownames(dev_out_tab) <- names$values

dev_out_tab %>%
  rownames_to_column(var = 'Outcome') %>%
  select("Outcome", "Peru", "Jordan", "Rwanda", "Ethiopia", "Madagascar", "Sierra Leone", "Niger", "Gabon", "Chad", "Pakistan") %>%
  flextable() %>%
...
Jordan_col_R <- tab_color_df_R$`Jordan - 2019`
Rwanda_col_R <- tab_color_df_R$Rwanda
Ethiopia_col_R <- tab_color_df_R$Ethiopia
Madagascar_col_R <- tab_color_df_R$Madagascar
SLE_col_R <- tab_color_df_R$`Sierra Leone`
Niger_col_R <- tab_color_df_R$Niger

Jordan_23_col_R <- tab_color_df_R$`Jordan - 2023`
Gabon_col_R <- tab_color_df_R$Gabon
Chad_col_R <- tab_color_df_R$Chad
ICT_col_R <- tab_color_df_R$`Pakistan - ICT`
KP_col_R <- tab_color_df_R$`Pakistan - KP`
#build table
tab_df_R <- tab_df_R  %>%
  select(`Indicator Name`, Peru, `Jordan - 2019`, Rwanda, Ethiopia, Madagascar, `Sierra Leone`, Niger, `Jordan - 2023`, Gabon, Chad, `Pakistan - ICT`, `Pakistan - KP`) %>%
  rename(Peru_R=Peru, 
         Jordan_R=`Jordan - 2019`, 
         Rwanda_R=Rwanda,
         Ethiopia_R=Ethiopia, 
         Madagascar_R=Madagascar,
         SLE_R=`Sierra Leone`,
         Niger_R=Niger,
         Jordan_23_R=`Jordan - 2023`, 
         Gabon_R=Gabon,
         Chad_R=Chad, 
         ICT_R=`Pakistan - ICT`,
         KP_R=`Pakistan - KP`
...
Jordan_col_U <- tab_color_df_U$`Jordan - 2019`
Rwanda_col_U <- tab_color_df_U$Rwanda
Ethiopia_col_U <- tab_color_df_U$Ethiopia
Madagascar_col_U <- tab_color_df_U$Madagascar
SLE_col_U <- tab_color_df_U$`Sierra Leone`
Niger_col_U <- tab_color_df_U$Niger

Jordan_23_col_U <- tab_color_df_U$`Jordan - 2023`
Gabon_col_U <- tab_color_df_U$Gabon
Chad_col_U <- tab_color_df_U$Chad
ICT_col_U <- tab_color_df_U$`Pakistan - ICT`
KP_col_U <- tab_color_df_U$`Pakistan - KP`

#build table
tab_df_U <- tab_df_U  %>%
  select(`Indicator Name`, Peru, `Jordan - 2019`, Rwanda, Ethiopia, Madagascar, `Sierra Leone`, Niger, `Jordan - 2023`, Gabon, Chad, `Pakistan - ICT`, `Pakistan - KP`) %>%
  rename(Peru_U=Peru, 
         Jordan_U=`Jordan - 2019`, 
         Rwanda_U=Rwanda,
         Ethiopia_U=Ethiopia, 
         Madagascar_U=Madagascar,
         SLE_U=`Sierra Leone`,
         Niger_U=Niger,
         Jordan_23_U=`Jordan - 2023`, 
         Gabon_U=Gabon,
         Chad_U=Chad, 
         ICT_U=`Pakistan - ICT`,
         KP_U=`Pakistan - KP`
...
tab_df_comb <- tab_df_R %>%
  left_join(tab_df_U) %>%
  select(`Indicator Name`, Peru_R, Peru_U, Jordan_R, Jordan_U, Rwanda_R, Rwanda_U, Ethiopia_R, Ethiopia_U, Madagascar_R, Madagascar_U, SLE_R, SLE_U, Niger_R, Niger_U, Jordan_23_R, Jordan_23_U, Gabon_R, Gabon_U, Chad_R, Chad_U, ICT_R, ICT_U, KP_R, KP_U)
...
gg_df <- tab_df_comb %>%
  ...
  mutate(Peru_R_gg_100=map(Peru_R,value_plotter_100),
         ...
         SLE_R_gg_100=map(SLE_R,value_plotter_100),
         SLE_U_gg_100=map(SLE_U,value_plotter_100),
         Niger_R_gg_100=map(Niger_R,value_plotter_100),
         Niger_U_gg_100=map(Niger_U,value_plotter_100),
         ...
         SLE_R_gg_5=map(SLE_R,value_plotter_5),
         SLE_U_gg_5=map(SLE_U,value_plotter_5),
         Niger_R_gg_5=map(Niger_R,value_plotter_5),
         Niger_U_gg_5=map(Niger_U,value_plotter_5)) %>%
  ungroup() %>%
  mutate(
    ...
    SLE_R=if_else(`Indicator Name` %in% list_100,SLE_R_gg_100, SLE_R_gg_5),
    SLE_U=if_else(`Indicator Name` %in% list_100, SLE_U_gg_100, SLE_U_gg_5) ,   
    Niger_R=if_else(`Indicator Name` %in% list_100,Niger_R_gg_100, Niger_R_gg_5),
    Niger_U=if_else(`Indicator Name` %in% list_100, Niger_U_gg_100, Niger_U_gg_5)
  ) %>%
...
flextable(gg_df) %>%
  compose(j=c("Peru_R", "Peru_U", "Jordan_R", "Jordan_U", "Rwanda_R", "Rwanda_U", "Ethiopia_R", "Ethiopia_U", "Madagascar_R", "Madagascar_U", "SLE_R", "SLE_U", "Niger_R", "Niger_U"),
...
ur_diff_df <- data.frame(
  ...
  `Sierra Leone`=tab_df_R$SLE_R-tab_df_U$SLE_U,
  Niger=tab_df_R$Niger_R-tab_df_U$Niger_U
)
...
**Comparison to Service Delivery Indicators Data**

The Service Delivery Indicators (SDI) project produces nationally representative indicators on education and health using surveys of health and education facilities.  The education SDI survey overlaps in many ways with the GEPD school survey. Both surveys check for teacher presence, and give teachers a content knowledge assessment that are nearly identical (except the GEPD assessment has a few slightly more difficult questions).  Finally, both measures contain several indicators on the availability of basic inputs and infrastructure that overlap.

```{r sdi}
#data pulled from databank on March 1, 2022

sdi_df <- read_csv(paste0(shared_loc, "/Data/SDI_data.csv")) %>%
  select(c("Country Name", "Year", "Absence from school (%) [SDI.E1.PCT]", "Teachers who pass at the 80% threshold (%) [SDI.E4.80.PCT]", "Classroom has a blackboard (%) [SDI.E5.BBD.PCT]",
           "Electricity (%) [SDI.H7.NRG.PCT]", "Students with writing instruments (%) [SDI.E5.PCLRAT]" )) %>%
  rename(
    `Absence from school (%)`=`Absence from school (%) [SDI.E1.PCT]`,
    `Teachers who pass at the 80% threshold (%)`=`Teachers who pass at the 80% threshold (%) [SDI.E4.80.PCT]`,
    `Classroom has a blackboard (%)`=`Classroom has a blackboard (%) [SDI.E5.BBD.PCT]`,
    `Electricity (%)`=`Electricity (%) [SDI.H7.NRG.PCT]`,
    `Students with writing instruments (%)`=`Students with writing instruments (%) [SDI.E5.PCLRAT]`
      ) %>%
  filter(!(`Absence from school (%)`==0 & `Teachers who pass at the 80% threshold (%)`==0 & `Classroom has a blackboard (%)`==0 & `Electricity (%)`==0 & `Students with writing instruments (%)`==0)) %>%
  mutate( #get rid of missing observations
    across(c(3:7), ~if_else(.==0,as.numeric(NA),.)),
    `Students with writing instruments (%)`=100*`Students with writing instruments (%)`, #fix formatting
    Survey="SDI",
    Year=as.character(Year)
    )

comp_ind <- c('SE.PRM.EFFT', 'SE.PRM.CONT.1', 'SE.PRM.INPT.2', 'SE.PRM.INFR.4', 'SE.PRM.INPT.3')
gepd_sdi_comp_df <- combined_df_raw %>%
  filter(Series %in% comp_ind) %>%
  select(country, date, Series, value) %>%
  pivot_wider(
    names_from=Series,
    values_from=value
  ) %>%
  mutate(SE.PRM.EFFT=100-SE.PRM.EFFT) %>%
  rename(
    `Country Name`=country,
    `Year`=date,
     `Absence from school (%)`=SE.PRM.EFFT, 
     `Teachers who pass at the 80% threshold (%)`=SE.PRM.CONT.1, 
     `Classroom has a blackboard (%)`=SE.PRM.INPT.2, 
     `Electricity (%)`=SE.PRM.INFR.4, 
     `Students with writing instruments (%)`=SE.PRM.INPT.3
    
  ) %>%
  mutate(Survey="GEPD") %>%
  bind_rows(sdi_df) %>%
  mutate(across(c(3:7),~round(.,3)),
         across(c(3:7), scales::percent,scale=1))
  
gepd_sdi_comp_df %>% 
  as_grouped_data(groups=c("Survey")) %>%
  flextable() 
...
defact_df <- combined_wide_df %>%
  ...
  pivot_longer(cols=c("Peru", "Jordan - 2019", "Ethiopia", "Rwanda", "Madagascar", "Sierra Leone", "Niger",
                      "Jordan - 2023", "Gabon", "Chad", "Pakistan - ICT", "Pakistan - KP"),
               names_to='country',
               values_to='defacto') %>%
  select(-Series)
...
PER_bur_df <- haven::read_dta(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/Public_Officials/data/public_officials_dta_clean_anon.dta", sep="/")) %>%
  ...
  select(iso3c, year,govt_tier, bur_indicators, everything()) %>%
  ...
  mutate(govt_tier=factor(govt_tier, levels=c(1,2,3), labels=c("Ministry of Education (or equivalent)","Regional office (or equivalent)", "District office (or equivalent)" )),
         education=factor(DEM1q11, levels=c(1,2,3,4,5,6,7, 97, 900, 998), labels=c('Primary school',
                                                                                   ...
                                                                                   'Refused to answer' )),
         age=as.numeric(age),
         years_current_position=as.numeric(years_current_position),
         years_current_org=as.numeric(years_current_org),
         years_civil_service=as.numeric(years_civil_service),
         num_pub_sector_org=as.numeric(num_pub_sector_org)
...
JOR_bur_df <- haven::read_dta(file=paste(anonymized_dir,"CNT", paste0(country,"-19"),paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/Public_Officials/data/public_officials_dta_clean_anon.dta", sep="/")) %>%
  mutate(iso3c=paste(country, year, sep="-"), 
         year=as.numeric(year)) %>%
  select(iso3c, year,govt_tier, bur_indicators, everything()) %>%
  select(-starts_with("ENUM")) %>%
  mutate(govt_tier=factor(govt_tier, levels=c(1,2,3), labels=c("Ministry of Education (or equivalent)","Regional office (or equivalent)", "District office (or equivalent)" )),
...
country <- "SLE"
year <- "2022"
SLE_bur_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/Public_Officials/data/public_officials_dta_clean_anon.csv", sep="/")) %>%
  mutate(iso3c=country, 
         year=as.numeric(year)) %>%
  select(iso3c, year,govt_tier, bur_indicators, everything()) %>%
  select(-starts_with("ENUM")) %>%
...
country <- "NER"
year <- "2022"
NER_bur_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/Public_Officials/data/public_officials_dta_clean_anon.csv", sep="/")) %>%
  mutate(iso3c=country, 
         year=as.numeric(year)) %>%
  select(iso3c, year, govt_tier, bur_indicators, everything()) %>%
  select(-starts_with("ENUM")) %>%
...
country <- "JOR"
year <- "2023"
JOR_23_bur_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/Public_Officials/data/public_officials_dta_clean_anon.csv", sep="/")) %>%
  mutate(iso3c=paste(country, year, sep="-"), 
         year=as.numeric(year)) %>%
  select(iso3c, year, govt_tier, bur_indicators, everything()) %>%
  select(-starts_with("ENUM")) %>%
...
country <- "GAB"
year <- "2023"
GAB_bur_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/Public_Officials/data/public_officials_dta_clean_anon.csv", sep="/")) %>%
  mutate(iso3c=country, 
         year=as.numeric(year)) %>%
  select(iso3c, year, govt_tier, bur_indicators, everything()) %>%
  select(-starts_with("ENUM")) %>%
...
country <- "TCD"
year <- "2023"
TCD_bur_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/Public_Officials/data/public_officials_dta_clean_anon.csv", sep="/")) %>%
  mutate(iso3c=country, 
         year=as.numeric(year)) %>%
  select(iso3c, year, govt_tier, bur_indicators, everything()) %>%
  select(-starts_with("ENUM")) %>%
...
province <- "ICT"
year <- "2022"
ICT_bur_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/",province,"/Public_Officials/data/public_officials_dta_clean_anon.csv", sep="/")) %>%
  mutate(iso3c=paste(country, province, sep="-"), 
         year=as.numeric(year)) %>%
  select(iso3c, year, govt_tier, bur_indicators, everything()) %>%
  select(-starts_with("ENUM")) %>%
  mutate(govt_tier=factor(govt_tier, levels=c("Ministry of Education (or equivalent)","Regional office (or equivalent)", "District office (or equivalent)" )),
...
province <- "KP"
year <- "2022"
KP_bur_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/",province,"/Public_Officials/data/public_officials_dta_clean_anon.csv", sep="/")) %>%
  mutate(iso3c=paste(country, province, sep="-"), 
         year=as.numeric(year)) %>%
  select(iso3c, year, govt_tier, bur_indicators, everything()) %>%
  select(-starts_with("ENUM")) %>%
...
combined_bur_df <- bind_rows(PER_bur_df, JOR_bur_df, RWA_bur_df, ETH_bur_df, MDG_bur_df,SLE_bur_df,NER_bur_df, 
                             JOR_23_bur_df, GAB_bur_df, TCD_bur_df, ICT_bur_df, KP_bur_df) %>%
  mutate(iso3c=factor(iso3c, levels=c("PAK-KP", "PAK-ICT", "TCD","GAB", "JOR-2023","NER","SLE","MDG", "ETH", "RWA", "JOR-2019", "PER")))


combined_bur_df %>%
  rename(pol_policy_implementation=politicized_policy_implementation,
         res_monitoring_perform=responsible_monitoring_performance,
         prop_reported_underperform=proportion_reported_underperformance
         ) %>%
  janitor::clean_names() %>%
...
#impute missing values with the mean within country
  mutate(across(practices, ~if_else(is.na(.) , 1,0), .names = "{.col}_impflag")) %>%
  mutate(across(policies, ~if_else(is.na(.), 1,0), .names = "{.col}_impflag")) %>%
...
MDG_school_office_link_df <- MDG_school_anon_df %>%
  ...
  group_by(country) %>%
#impute missing values with the mean within country
  mutate(across(c('presence_rate', 'content_knowledge', 'inputs', 'infrastructure', 'ecd_student_knowledge',  'operational_management', 'instructional_leadership', 'principal_knowledge_score', 'principal_management'), ~if_else(is.na(.) , 1,0), .names = "{.col}_impflag")) %>%
...
year <- "2023"
confidential_folder <- file.path(paste(confidential_dir,"CNT",country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v01_RAW", sep="_"),"Data/confidential/", sep="/"))
JOR_23_school_anon_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_dta_short_anon.csv", sep="/"))
JOR_23_school_office_linkages <- read_csv(file=paste0(confidential_folder, "/School/linked_po_school_data_",country,"_",year,".csv"))
#load gdp info
JOR_23_school_gdp <-read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_gdp_anon.csv", sep="/"))
#load school size info
JOR_23_school_size <-read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_dta_anon.csv", sep="/")) %>%
  select(hashed_school_code, students_enrolled) %>%
  filter(!is.na(students_enrolled))
JOR_23_school_office_link_df <- JOR_23_school_anon_df %>%
  left_join(JOR_23_school_gdp) %>%
  left_join(JOR_23_school_office_linkages) %>%
  left_join(JOR_23_school_size) %>%
  mutate(country="Jordan-2023") %>%
  mutate(dist=distHaversine(.[,c('school_lon', 'school_lat')], .[c('office_lon', 'office_lat')]),
         dist_km=dist/1000,
         dist_log=log(dist_km),
...
year <- "2023"
confidential_folder <- file.path(paste(confidential_dir,"CNT",country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v01_RAW", sep="_"),"Data/confidential/", sep="/"))
GAB_school_anon_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_dta_short_anon.csv", sep="/"))
GAB_school_office_linkages <- read_csv(file=paste0(confidential_folder, "/School/linked_po_school_data_",country,"_",year,".csv"))
#load gdp info
GAB_school_gdp <-read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_gdp_anon.csv", sep="/"))
#load school size info
GAB_school_size <-read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_dta_anon.csv", sep="/")) %>%
  select(hashed_school_code, students_enrolled) %>%
  filter(!is.na(students_enrolled)) %>%
  distinct()
GAB_school_office_link_df <- GAB_school_anon_df %>%
  left_join(GAB_school_gdp) %>%
  left_join(GAB_school_office_linkages) %>%
  left_join(GAB_school_size) %>%
  mutate(country="Gabon") %>%
  mutate(dist=distHaversine(.[,c('school_lon', 'school_lat')], .[c('office_lon', 'office_lat')]),
         dist_km=dist/1000,
         dist_log=log(dist_km),
...
#impute missing values with the mean within country
  ...
  ungroup() %>%
  rename(GDP=gdp) %>%
  mutate(private=if_else(private==1, "Private", "Public"))
...
year <- "2023"
confidential_folder <- file.path(paste(confidential_dir,"CNT",country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v01_RAW", sep="_"),"Data/confidential/", sep="/"))
TCD_school_anon_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_dta_short_anon.csv", sep="/"))
TCD_school_office_linkages <- read_csv(file=paste0(confidential_folder, "/School/linked_po_school_data_",country,"_",year,".csv"))
#load gdp info
TCD_school_gdp <-read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_gdp_anon.csv", sep="/"))
#load school size info
TCD_school_size <-read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_dta_anon.csv", sep="/")) %>%
  select(hashed_school_code, students_enrolled) %>%
  filter(!is.na(students_enrolled))
TCD_school_office_link_df <- TCD_school_anon_df %>%
  left_join(TCD_school_gdp) %>%
  left_join(TCD_school_office_linkages) %>%
  left_join(TCD_school_size) %>%
  mutate(country="Chad") %>%
  mutate(dist=distHaversine(.[,c('school_lon', 'school_lat')], .[c('office_lon', 'office_lat')]),
         dist_km=dist/1000,
         dist_log=log(dist_km),
...
province <- "ICT"
year <- "2022"
confidential_folder <- file.path(paste(confidential_dir,"CNT",country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v01_RAW", sep="_"),"Data/",province,"/confidential/", sep="/"))
ICT_school_anon_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/",province,"School/data/school_dta_short_anon.csv", sep="/"))
ICT_school_office_linkages <- read_csv(file=paste0(confidential_folder, "/School/linked_po_school_data_",country,"_",year,".csv"))
#load gdp info
ICT_school_gdp <-read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data",province,"/School/data/school_gdp_anon.csv", sep="/"))
#load school size info
ICT_school_size <-read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data",province,"/School/data/school_dta_anon.csv", sep="/")) %>%
  select(hashed_school_code, students_enrolled) %>%
  filter(!is.na(students_enrolled))
ICT_school_office_link_df <- ICT_school_anon_df %>%
  left_join(ICT_school_gdp) %>%
  left_join(ICT_school_office_linkages) %>%
  left_join(ICT_school_size) %>%
  mutate(country="Pakistan-ICT") %>%
  mutate(dist=distHaversine(.[,c('school_lon', 'school_lat')], .[c('office_lon', 'office_lat')]),
         dist_km=dist/1000,
         dist_log=log(dist_km),
...
year <- "2022"
confidential_folder <- file.path(paste(confidential_dir,"CNT",country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v01_RAW", sep="_"),"Data/",province,"/confidential/", sep="/"))
KP_school_anon_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/",province,"School/data/school_dta_short_anon.csv", sep="/"))
KP_school_office_linkages <- read_csv(file=paste0(confidential_folder, "/School/linked_po_school_data_",country,"_",year,".csv"))
#load gdp info
KP_school_gdp <-read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data",province,"/School/data/school_gdp_anon.csv", sep="/"))
#load school size info
KP_school_size <-read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data",province,"/School/data/school_dta_anon.csv", sep="/")) %>%
  select(hashed_school_code, students_enrolled) %>%
  filter(!is.na(students_enrolled))
KP_school_office_link_df <- KP_school_anon_df %>%
  left_join(KP_school_gdp) %>%
  left_join(KP_school_office_linkages) %>%
  left_join(KP_school_size) %>%
  mutate(country="Pakistan-KP") %>%
  mutate(dist=distHaversine(.[,c('school_lon', 'school_lat')], .[c('office_lon', 'office_lat')]),
         dist_km=dist/1000,
         dist_log=log(dist_km),
...
combined_school_office_df <- bind_rows(PER_school_office_link_df, JOR_school_office_link_df, RWA_school_office_link_df, ETH_school_office_link_df, MDG_school_office_link_df, SLE_school_office_link_df, NER_school_office_link_df,
                                       JOR_23_school_office_link_df, GAB_school_office_link_df, TCD_school_office_link_df,
                                       ICT_school_office_link_df, KP_school_office_link_df) 


combined_school_office_df %>%
  rename(pol_policy_implementation=politicized_policy_implementation,
         principal_know_score_impflag=principal_knowledge_score_impflag, 
         teacher_sel_deployment_impflag=teacher_selection_deployment_impflag, 
         sch_man_attraction_impflag=sch_management_attraction_impflag) %>%
...
'Availability of Basic Inputs' = feols(inputs ~   operational_management   + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'Availability of Functional Blackboard' = feols(blackboard_functional ~   operational_management   + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'Availability of Writing Utencils' = feols(pens_etc ~   operational_management   + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'Availability of Textbooks' = feols(textbooks ~   operational_management   + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'Availability of Desks' = feols(share_desk ~   operational_management   + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'Availability of ICT' = feols(access_ict ~   operational_management   + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero')
...
'Availability of Basic Infrastructure' = feols(infrastructure ~   operational_management   + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'Availability of Drinking Water' = feols(drinking_water ~   operational_management   + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'Availability of Functional Toilet' = feols(functioning_toilet ~   operational_management   + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'Availability of internet' = feols(internet ~   operational_management   + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'Availability of Electricity' = feols(class_electricity ~   operational_management   + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'Access for the Disabled' = feols(disability_accessibility ~   operational_management   + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero')
...
p2 <- scree_fun("Jordan-2019", indicators_list) 
...
country <- "SLE"
year <- "2022"
SLE_g4_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/assess_4th_grade_anon_anon.csv", sep="/")) %>%
  mutate(iso3c=country)
########
# Niger
#######
...
country <- "NER"
year <- "2022"
NER_g4_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/assess_4th_grade_anon_anon.csv", sep="/")) %>%
  mutate(iso3c=country) %>%
  select(-public)
...
country <- "JOR"
year <- "2023"
JOR_23_g4_df <- haven::read_dta(file=paste(anonymized_dir,"CNT", 'JOR',paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/assess_4th_grade_anon_anon.dta", sep="/")) %>%
  mutate(iso3c=paste(country,year, sep="-"))
...
country <- "GAB"
year <- "2023"
GAB_g4_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/assess_4th_grade_anon_anon.csv", sep="/")) %>%
  mutate(iso3c=country) %>%
  mutate(private=if_else(private==1, "Private", "Public"))
...
country <- "TCD"
year <- "2023"
TCD_g4_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/assess_4th_grade_anon_anon.csv", sep="/")) %>%
  mutate(iso3c=country)
...
province <- "ICT"
year <- "2022"
ICT_g4_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/",province,"/School/data/assess_4th_grade_anon_anon.csv", sep="/")) %>%
  mutate(iso3c=paste(country, province, sep="-"))
...
province <- "KP"
year <- "2022"
KP_g4_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/",province,"/School/data/assess_4th_grade_anon_anon.csv", sep="/")) %>%
  mutate(iso3c=paste(country, province, sep="-"))

########
#combined
########
combined_g4_df <- bind_rows(PER_g4_df, JOR_g4_df, RWA_g4_df, ETH_g4_df_2020, ETH_g4_df_2021, MDG_g4_df, SLE_g4_df, NER_g4_df,
                            JOR_23_g4_df, GAB_g4_df, TCD_g4_df, ICT_g4_df, KP_g4_df) %>%
  mutate(country_code=factor(iso3c, levels=c("PAK-KP","PAK-ICT", "GAB", "TCD","JOR-2023" ,"SLE", "NER","MDG", "ETH", "RWA", "JOR-2019", "PER")),
...
p1 <- ggplot(combined_g4_df, aes(x=iso3c, y=student_knowledge, color=`Student Proficient`)) +
  ...
  geom_segment(data=combined_g4_mn, aes( xend=..x..-.3, yend=..y..), color='black', size=1)  +
  geom_text(data=combined_g4_mn, aes(label=paste0(round(student_knowledge, 1), "%")), hjust=-.25, color='black', size=3) +
  coord_flip() +
  scale_color_manual(
...
p2 <- ggplot(combined_g4_df, aes(x=iso3c, y=math_student_knowledge, color=`Student Proficient Math`)) +
  ...
  geom_segment(data=combined_g4_mn, aes( xend=..x..+.3, yend=..y..), color='black', size=1)  +
  geom_segment(data=combined_g4_mn, aes( xend=..x..-.3, yend=..y..), color='black', size=1)  +
  geom_text(data=combined_g4_mn, aes(label=paste0(round(math_student_knowledge, 1), "%")), hjust=-.25, color='black', size=3) +
  coord_flip() +
  scale_color_manual(
    values=c("#ef476f", "#06d6a0")
  ) +
...
p3 <- ggplot(combined_g4_df, aes(x=iso3c, y=literacy_student_knowledge, color=`Student Proficient Literacy`)) +
  ...
  geom_segment(data=combined_g4_mn, aes( xend=..x..+.3, yend=..y..), color='black', size=1)  +
  geom_segment(data=combined_g4_mn, aes( xend=..x..-.3, yend=..y..), color='black', size=1)  +
  geom_text(data=combined_g4_mn, aes(label=paste0(round(literacy_student_knowledge, 1), "%")), hjust=-.25, color='black', size=3) +
  coord_flip() +
  scale_color_manual(
    values=c("#ef476f", "#06d6a0")
  ) +
   ...
   mutate(
     ...
     across(ends_with("country"),
            ...
            ~ case_when(str_detect(., "MDG") ~ "Madagascar",
                        ...
                        str_detect(., "JOR-2019") ~ "Jordan-2019",
                        str_detect(., "NER") ~ "Niger",
                        str_detect(., "SLE") ~ "Sierra Leone",
                        str_detect(., "JOR-2019") ~ "Jordan-2023",
                        str_detect(., "GAB") ~ "Gabon",
                        str_detect(., "TCD") ~ "Chad",
                        str_detect(., "PAK-ICT") ~ "Pakistan-ICT",
                        str_detect(., "PAK-KP") ~ "Pakistan-KP")),
...
modelsummary(models,
             ...
             coef_omit="GDP|rural|students_enrolled",
             gof_map = gm,
             notes="Regressions also include country fixed effects, urban rural status, and total school enrollment as covariates.",
             escape = FALSE
             )
```

Because data for teacher pedagogical skills and student attendance is missing in Madagascar, data for Madagascar are omitted from the previous regressions.  As a sensitivity check, the pedagogical skill and student attendance measures are dropped and data from Madagascar included in the regression results below.

```{r}
models <- list(
  
'4th Grade Student Knowledge' = feols(student_knowledge ~  presence_rate + content_knowledge  + inputs + infrastructure + ecd_student_knowledge  + operational_management + instructional_leadership + principal_knowledge_score + principal_management  + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'4th Grade Reading Score' = feols(literacy_student_knowledge ~ presence_rate + content_knowledge  + inputs + infrastructure + ecd_student_knowledge  + operational_management + instructional_leadership + principal_knowledge_score + principal_management  + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'4th Grade Math Score' = feols(math_student_knowledge ~  presence_rate + content_knowledge  + inputs + infrastructure + ecd_student_knowledge  + operational_management + instructional_leadership + principal_knowledge_score + principal_management  + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero')
)
...
'4th Grade Student Knowledge - Peru' = feols(student_knowledge ~  presence_rate + content_knowledge + teach_score + inputs + infrastructure + ecd_student_knowledge + student_attendance + operational_management + instructional_leadership + principal_knowledge_score + principal_management   + rural + students_enrolled, data=PER_school_office_link_df,vcov='hetero'),

'4th Grade Student Knowledge - Jordan' = feols(student_knowledge ~  presence_rate + content_knowledge + teach_score + inputs + infrastructure + ecd_student_knowledge + student_attendance + operational_management + instructional_leadership + principal_knowledge_score + principal_management   + rural + students_enrolled, data=JOR_school_office_link_df,vcov='hetero'),

'4th Grade Student Knowledge - Rwanda' = feols(student_knowledge ~  presence_rate + content_knowledge + teach_score + inputs + infrastructure + ecd_student_knowledge + student_attendance + operational_management + instructional_leadership + principal_knowledge_score + principal_management  + rural + students_enrolled, data=RWA_school_office_link_df,vcov='hetero'),

'4th Grade Student Knowledge - Ethiopia' = feols(student_knowledge ~ presence_rate + content_knowledge + teach_score + inputs + infrastructure + ecd_student_knowledge + student_attendance + operational_management + instructional_leadership + principal_knowledge_score + principal_management   + rural + students_enrolled, data=ETH_school_office_link_df,vcov='hetero'),

'4th Grade Student Knowledge - Madagascar' = feols(student_knowledge ~  presence_rate + content_knowledge  + inputs + infrastructure + ecd_student_knowledge + operational_management + instructional_leadership + principal_knowledge_score + principal_management   + rural + students_enrolled, data=MDG_school_office_link_df,vcov='hetero')
)
...
country <- "SLE"
year <- "2022"
SLE_g1_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/ecd_dta_anon_anon.csv", sep="/")) %>%
  mutate(iso3c=country) 
...
country <- "NER"
year <- "2022"
NER_g1_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/ecd_dta_anon_anon.csv", sep="/")) %>%
  mutate(iso3c=country) %>%
  select(-public)
...
country <- "JOR"
year <- "2023"
JOR_23_g1_df <- haven::read_dta(file=paste(anonymized_dir,"CNT", 'JOR',paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/ecd_dta_anon.dta", sep="/")) %>%
  mutate(iso3c=paste(country, year, sep="-"))
...
country <- "GAB"
year <- "2023"
GAB_g1_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/ecd_dta_anon_anon.csv", sep="/")) %>%
  mutate(iso3c=country,
         private=if_else(private==1,"Private", "Public")) %>%
  select(-language_g1)
...
country <- "TCD"
year <- "2023"
TCD_g1_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/ecd_dta_anon_anon.csv", sep="/")) %>%
  mutate(iso3c=country) %>%
  select(-language_g1)
...
province <- "ICT"
year <- "2022"
ICT_g1_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/",province,"/School/data/ecd_dta_anon_anon.csv", sep="/")) %>%
  mutate(iso3c=paste(country, province, sep="-") ) %>%
  select(-language_g1)
...
province <- "KP"
year <- "2022"
KP_g1_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/",province,"/School/data/ecd_dta_anon_anon.csv", sep="/")) %>%
  mutate(iso3c=paste(country, province, sep="-") ) %>%
  select(-language_g1)
...
#combined
########
combined_g1_df <- bind_rows(PER_g1_df, JOR_g1_df, RWA_g1_df, ETH_g1_df_2020, ETH_g1_df_2021, MDG_g1_df, SLE_g1_df, NER_g1_df,
                            JOR_23_g1_df, GAB_g1_df, TCD_g1_df, ICT_g1_df, KP_g1_df) %>%
  mutate(iso3c=factor(iso3c, levels=c("PAK-KP", "PAK-ICT", "TCD", "GAB", "JOR-2023", "NER", "SLE",  "MDG", "ETH", "RWA", "JOR-2019", "PER"))) %>%
...
p1 <- combined_g1_df %>%
  filter(!is.na(Gender)) %>%
  ggplot( aes(x=Gender, y=ecd_student_knowledge, color=`Student Proficient`)) +
  facet_wrap(~iso3c) +
  geom_quasirandom() +
  geom_segment(data=combined_g1_mn, aes( xend=..x..+.3, yend=..y..), color='black', size=1)  +
  geom_segment(data=combined_g1_mn, aes( xend=..x..-.3, yend=..y..), color='black', size=1)  +
  geom_text(data=combined_g1_mn, aes(label=paste0(round(ecd_student_knowledge, 1), "%")), hjust=-.15, color='black', size=3) +
  coord_flip() +
  scale_color_manual(
    values=c("#ef476f", "#06d6a0")
  ) +
...
reg_fun <- function(indicators) {
  ...
  models <- list(
   ...
   " " = feols(form, data=combined_school_office_df,vcov='hetero')
               ...
               notes="Regressions also include country fixed effects, the log GDP within 1km of school, urban rural status, and total school enrollment as covariates.  
                  *** significant at 0.1% level. 
                   ...
                   * significant at 5% level.  
                   + significant at 10% level.  ",
               escape = FALSE,
               title=paste0('Regression of ',out_lab,' practice outcome on related policies.  Data from all schools in all available countries.')
               ) 
...
combined_teachers_df <- read_dta(file=paste0(confidential_dir, "/General/LEGO_Teacher_Paper/5_output_data/GEPD_teacher_school_country_level.dta")) %>%
  mutate(iso3c=factor(country, levels=c("NER", "SLE", "ETH","RWA", "JOR", "PER")))  %>%
...
Teacher support has a positive relationship with teacher content knowledge at the 10% level.  A one unit increase in teacher support (on a 1-5 scale) is associated with a 1 percentage point improvement in teacher content knowledge.  
...
Teacher pedagogy has no statistically significant relationships to the underlying policy indicators.  Data from Madagascar are not included in this regression analysis, as data on teacher pedagogical skills were note collected in this country. The Madagascar survey was done in conjunction with a survey for the Service Deliverty Inidcators (SDI) project, and differences in methodology led to some indicators not being collected.
...
```{r}
 ...
 JOR_df <- haven::read_dta(file=paste(anonymized_dir,"CNT", 'JOR-19',paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_dta_short_anon.dta", sep="/")) %>%
    mutate(iso3c=paste(country, year, sep="-")) %>% mutate(across(.cols =  everything(), ~ as.character(.)))
 ########
 # Rwanda
 #######
 #read in anonymized school data
 ...
 country <- "SLE"
 year <- "2022"
 SLE_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_dta_short_anon.csv", sep="/")) %>%
    mutate(iso3c=country) %>%
 ...
 country <- "NER"
 year <- "2022"
 NER_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_dta_short_anon.csv", sep="/")) %>%
    mutate(iso3c=country) %>%
    mutate(across(.cols =  everything(), ~ as.character(.)))
 ...
 country <- "JOR"
 year <- "2023"
 JOR_23_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_dta_short_anon.csv", sep="/")) %>%
    mutate(iso3c=paste(country, year, sep="-")) %>%
    mutate(across(.cols =  everything(), ~ as.character(.))) 
 ...
 country <- "GAB"
 year <- "2023"
 GAB_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_dta_short_anon.csv", sep="/")) %>%
    mutate(iso3c=country) %>%
    mutate(across(.cols =  everything(), ~ as.character(.))) 
 ...
 country <- "TCD"
 year <- "2023"
 TCD_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/School/data/school_dta_short_anon.csv", sep="/")) %>%
    mutate(iso3c=country) %>%
    mutate(across(.cols =  everything(), ~ as.character(.)))  %>%
  select(-public)
 ...
 province <- "ICT"
 year <- "2022"
 ICT_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/",province,"/School/data/school_dta_short_anon.csv", sep="/")) %>%
    mutate(iso3c=paste(country, province, sep="-")) %>%
    mutate(across(.cols =  everything(), ~ as.character(.)))  
...
########
 ...
 province <- "KP"
 year <- "2022"
 KP_df <- read_csv(file=paste(anonymized_dir,"CNT", country,paste(country,year,"GEPD", sep="_"), paste(country,year,"GEPD_v01_M", sep="_"),"Data/",province,"/School/data/school_dta_short_anon.csv", sep="/")) %>%
    mutate(iso3c=paste(country, province, sep="-")) %>%
    mutate(across(.cols =  everything(), ~ as.character(.)))  
 ########
 ...
 #combined
 ########
 combined_school_df <- bind_rows(PER_df, JOR_df, RWA_df, ETH_df, MDG_df, SLE_df, NER_df, JOR_23_df, GAB_df, TCD_df, ICT_df, KP_df) %>%
    mutate(iso3c=factor(iso3c, levels=c("PAK-KP", "PAK-ICT", "TCD", "GAB", "JOR-2023", "NER", "SLE", "MDG", "ETH", "RWA", "JOR-2019", "PER")))
...
```{r}
 ...
 breaking_up_all_principal <- combined_school_df %>%
    ...
    ) %>% select(skim_variable, iso3c, numeric.mean) %>% rename(mean_country= iso3c, mean_var = numeric.mean) %>% arrange(skim_variable) %>% ungroup()
 ...
 breaking_up_principal <- combined_school_df %>%
    ...
    arrange(skim_variable) %>% ungroup() %>% bind_cols(breaking_up_all_principal %>% filter(!is.na(skim_variable)) %>% select(-skim_variable) ) %>% 
...
'4th Grade Student Knowledge' = feols(student_knowledge ~  bureau_index + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'Teacher Presence' =  feols(presence_rate ~   bureau_index  + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'Teacher Pedagogy' =  feols(teach_score ~   bureau_index + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'School Inputs' =  feols(inputs ~   bureau_index + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'School Infrastructure' =  feols(infrastructure ~   bureau_index + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'Operations Management' =  feols(operational_management ~  bureau_index  + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'Principal Management Skills' =  feols(principal_management ~  bureau_index  + log(GDP) + rural + students_enrolled   | country, data=combined_school_office_df,vcov='hetero')
...
'4th Grade Student Knowledge' = feols(student_knowledge ~  bureau_index:country + log(GDP) + rural + students_enrolled | country, data=combined_school_office_df,vcov='hetero'),

'Teacher Presence' =  feols(presence_rate ~   bureau_index:country  + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'Teacher Pedagogy' =  feols(teach_score ~   bureau_index:country + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'School Inputs' =  feols(inputs ~   bureau_index:country + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'School Infrastructure' =  feols(infrastructure ~   bureau_index:country + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'Operations Management' =  feols(operational_management ~  bureau_index:country  + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'Principal Management Skills' =  feols(principal_management ~  bureau_index:country  + log(GDP) + rural + students_enrolled   | country, data=combined_school_office_df,vcov='hetero')
...
'Student Knowledge' = feols(student_knowledge ~ quality_bureaucracy + national_learning_goals + impartial_decision_making + mandates_accountability  + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'Teacher Presence' =  feols(presence_rate ~ quality_bureaucracy + national_learning_goals + impartial_decision_making + mandates_accountability  + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'Teacher Pedagogy' =  feols(teach_score ~ quality_bureaucracy + national_learning_goals + impartial_decision_making + mandates_accountability  + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'School Inputs' =  feols(inputs ~ quality_bureaucracy + national_learning_goals + impartial_decision_making + mandates_accountability  + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'School Infrastructure' =  feols(infrastructure ~ quality_bureaucracy + national_learning_goals + impartial_decision_making + mandates_accountability  + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'Operations Management' =  feols(operational_management ~ quality_bureaucracy + national_learning_goals + impartial_decision_making + mandates_accountability  + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero'),

'Principal Management Skills' =  feols(principal_management ~ quality_bureaucracy + national_learning_goals + impartial_decision_making + mandates_accountability  + log(GDP) + rural + students_enrolled  | country, data=combined_school_office_df,vcov='hetero')
...
## Share of Variation Explained

The goal in this analysis to decompose how much of the between school variability in the outcome variable, 4th grade student learning, is explained by the GEPD practice indicators, policy indicators, and bureaucracy indicators.  As most of the indicators vary only at the school level, this analysis examines between school variation, rather than variation within schools between students, as the majority of the indicators provide no variation within a school (e.g. principal management only occurs at the school level). Analyzing the between school variation will be done using Analysis of Variance (ANOVA) methods. 

First, 4th grade student learning is expressed as a linear function of the GEPD practices, policy, and bureaucracy indicators, plus other controls such as the GDP in the vicinity of the school, the urban/rural status, the number of students enrolled, and country fixed effects.  All data is aggregated to the school level, so that the 4th grade achievement outcome is the school level average of 4th grade achievement for students.  Likewise, indicators such as teacher presence and content knowledge are aggregated to the school level.  

The linear model of student achievement is

$$ A_{ic} = Pract_{ic} + Policy_{ic} + Bureau_{ic} + Z_{ic} + \mu_c + \epsilon_{ic} $$

where $A_{ic}$ is average 4th grade student achievement at the school $i$ in country $c$, $Pract_{ic}$ are the practice indicators (teacher presence, content knowledge, pedagogical skill, inputs, infrastructure, 1st grade student knowledge, 4th grade attendance, operational management, instructional leadership, principal school knowledge, and management skills), $Policy_{ic}$ are the de facto policy indicators (teacher attraction, selection & deployment, support, evaluation, monitoring & accountability, intrinsic motivation, input & infrastructure monitoring, standards, school management clarity of functions, attraction, selection & deployment, support, and evaluation), and $Bureau_{ic}$ are the district level averages of the politics and bureaucracy capacity indicators (characteristics of bureaucracy, impartial decision-making, mandates & accountability, and national learning goals).  $Z_{ic}$ are other controls including the urban/rural status of the school, GDP in the vicinity of the school based on satellite data, and the student enrollment at the school. $\mu_{c}$ is a country specific intercept (or heterogeneity term) and $\epsilon_{ic}$ is an idiosyncratic error term.  This model is estimated using OLS using data from all schools that can be matched to a district education office of public officials.  

Next that linear model is decomposed into the share of variation explained by school practices, policies, bureaucracy, and other factors using ANOVA.  This is done by partitioning the total sum of squares $ \sum (A_i-\bar{A})^2 $ into the sum of squares explained by the model and the residual sum of squares.
...
Alternative without student attendance and teacher pedagogy to include Madagascar.


```{r anovamda}
#set controls
controls <- 'log(GDP) + rural + students_enrolled '
practices <- 'presence_rate + content_knowledge  + inputs + infrastructure + ecd_student_knowledge + operational_management + instructional_leadership + principal_knowledge_score + principal_management'
policies <- 'teacher_attraction + teacher_selection_deployment + teacher_support + teaching_evaluation + teacher_monitoring + intrinsic_motivation + standards_monitoring + sch_monitoring + sch_management_clarity + sch_management_attraction + sch_selection_deployment + sch_support + principal_evaluation'
politics <- 'quality_bureaucracy + national_learning_goals + impartial_decision_making + mandates_accountability'
...
anova_df <- combined_school_office_df %>%
  ...
  select(student_knowledge, GDP, rural, students_enrolled, country,
         presence_rate,  content_knowledge,  inputs,  infrastructure,  ecd_student_knowledge,  operational_management,  instructional_leadership,  principal_knowledge_score,  principal_management,
         teacher_attraction,  teacher_selection_deployment,  teacher_support,  teaching_evaluation,  teacher_monitoring,  intrinsic_motivation,  standards_monitoring,  sch_monitoring,  sch_management_clarity,  sch_management_attraction,  sch_selection_deployment,  sch_support,  principal_evaluation,
         quality_bureaucracy,  national_learning_goals,  impartial_decision_making,  mandates_accountability) %>%
  na.omit
...
'Student Knowledge' = feols(student_knowledge ~ teacher_attraction  + teaching_evaluation + teacher_selection_deployment + teacher_support + intrinsic_motivation + teacher_monitoring , data=defacto,vcov='hetero'),

'Teacher Presence' =  feols(presence_rate ~  teacher_attraction  + teaching_evaluation + teacher_selection_deployment + teacher_support + intrinsic_motivation + teacher_monitoring , data=defacto,vcov='hetero'),

'Teacher Pedagogy' =  feols(teach_score ~  teacher_attraction  + teaching_evaluation + teacher_selection_deployment + teacher_support + intrinsic_motivation + teacher_monitoring , data=defacto,vcov='hetero'),

'Content Knowledge' =  feols(content_knowledge ~  teacher_attraction  + teaching_evaluation + teacher_selection_deployment + teacher_support + intrinsic_motivation + teacher_monitoring , data=defacto,vcov='hetero')
...
'Student Knowledge' = feols(student_knowledge ~ teacher_attraction  + teaching_evaluation + teacher_selection_deployment + teacher_support + intrinsic_motivation + teacher_monitoring , data=deJure,vcov='hetero'),

'Teacher Presence' =  feols(presence_rate ~  teacher_attraction  + teaching_evaluation + teacher_selection_deployment + teacher_support + intrinsic_motivation + teacher_monitoring , data=deJure,vcov='hetero'),

'Teacher Pedagogy' =  feols(teach_score ~  teacher_attraction  + teaching_evaluation + teacher_selection_deployment + teacher_support + intrinsic_motivation + teacher_monitoring , data=deJure,vcov='hetero'),

'Content Knowledge' =  feols(content_knowledge ~  teacher_attraction  + teaching_evaluation + teacher_selection_deployment + teacher_support + intrinsic_motivation + teacher_monitoring , data=deJure,vcov='hetero')
...
'Basic Infrastructure' =  feols(infrastructure ~ standards_monitoring  + monitoring_infrastructure + monitoring_inputs + log(GDP) + rural| ADM0_NAME, data=combined_school_office_df,vcov='hetero'),
  
'Basic Input' = feols(inputs ~ standards_monitoring  + monitoring_infrastructure + monitoring_inputs + log(GDP) + rural| ADM0_NAME, data=combined_school_office_df,vcov='hetero'),

'Student Knowledge' =  feols(student_knowledge ~ standards_monitoring  + monitoring_infrastructure + monitoring_inputs + log(GDP) + rural| ADM0_NAME, data=combined_school_office_df,vcov='hetero'),

'School Knowledge' =  feols(principal_knowledge_score ~ standards_monitoring  + monitoring_infrastructure + monitoring_inputs + log(GDP) + rural| ADM0_NAME, data=combined_school_office_df,vcov='hetero')
...
'Student Knowledge' =  feols(student_knowledge ~ sch_management_clarity  + sch_selection_deployment + sch_management_attraction + sch_monitoring + sch_support + log(GDP) + rural| ADM0_NAME, data=combined_school_office_df,vcov='hetero'),
  
'Operational Management' = feols(operational_management ~ sch_management_clarity  + sch_selection_deployment + sch_management_attraction + sch_monitoring + sch_support  + log(GDP) + rural| ADM0_NAME, data=combined_school_office_df,vcov='hetero'),

'Instructional Leadership' =  feols(instructional_leadership ~ sch_management_clarity  + sch_selection_deployment + sch_management_attraction + sch_monitoring + sch_support   + log(GDP) + rural| ADM0_NAME, data=combined_school_office_df,vcov='hetero'),

'School Knowledge' =  feols(principal_knowledge_score ~ sch_management_clarity  + sch_selection_deployment + sch_management_attraction + sch_monitoring + sch_support + log(GDP) + rural| ADM0_NAME, data=combined_school_office_df,vcov='hetero')
