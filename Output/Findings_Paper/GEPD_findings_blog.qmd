...
#directories
  ...
  if (str_to_lower(Sys.info()["user"]) == "wb631589") {
    ...
    dir <- here()
    shared_loc <- 'C:/Users/wb631589/OneDrive - WBG/Dashboard (Team Folder)/Main_Documents/GEPD_FCDO_report"'
  } else if (str_to_lower(Sys.info()["user"]) == "wb577189") {
...
#path to confidential directory
  ...
  if (str_to_lower(Sys.info()["user"]) == "wb469649") {
    ...
    #directories
    confidential_dir<- "C:/Users/wb631589/OneDrive - WBG/GEPD-Confidential/"
    anonymized_dir <- "C:/Users/wb631589/OneDrive - WBG/GEPD/"
...
combined_df_raw <- bind_rows(PER_data,JOR_data, RWA_data, ETH_data,  MDG_data, SLE_data)
...
#build table
tab_df <- tab_df  %>%
  select(`Indicator Name`, Peru, Jordan, Rwanda, Ethiopia, Madagascar, `Sierra Leone`)
...
seg_df <- plot_df %>%
  ...
  'SE.PRM.ATTD',
  'SE.PRM.LERN')))  
write_excel_csv(plot_df, file=paste0(dir, "/Output/Findings_Paper/chart_data/chart2.csv"))
...
```{r}
#| , fig.width = 7, fig.height = 6, dpi=350
...
infra_df %>%
  ...
  write_excel_csv( file=paste0(dir, "/Output/Findings_Paper/chart_data/chart3.csv"))
...
```{r}
radar_ind <- c('SE.PRM.TATT.DF', 'SE.PRM.TEVL.DF', 'SE.PRM.TMNA.DF', 'SE.PRM.TSDP.DF','SE.PRM.TSUP.DF','SE.PRM.SATT.DF', 'SE.PRM.SEVL.DF', 'SE.PRM.SSLD.DF', 'SE.PRM.SSUP.DF',
               'SE.PRM.TATT.DJ', 'SE.PRM.TEVL.DJ', 'SE.PRM.TMNA.DJ', 'SE.PRM.TSDP.DJ','SE.PRM.TSUP.DJ','SE.PRM.SATT.DJ', 'SE.PRM.SEVL.DJ', 'SE.PRM.SSLD.DJ', 'SE.PRM.SSUP.DJ')

radar_df <- combined_df %>%
  filter(Series %in% radar_ind) %>%
  mutate(class=if_else(grepl("*.DF$", Series), "De Facto", "De Jure")) %>%
  mutate(`Indicator Name`=str_remove(`Indicator Name`, "\\(De Facto\\)"),
         `Indicator Name`=str_remove(`Indicator Name`, "\\(De Jure\\)"),
         `Indicator Name`=str_remove(`Indicator Name`, "\\("),
         `Indicator Name`=str_remove(`Indicator Name`, "\\)"),
         `Indicator Name`=str_remove(`Indicator Name`, "Policy Lever")) %>%
  select(country, `Indicator Name`, value, class) %>%
  pivot_wider(
    values_from = 'value',
    names_from = "Indicator Name"
  )

write_excel_csv(radar_df,  file=paste0(dir, "/Output/Findings_Paper/chart_data/chart4.csv"))
...
![Variation in 4th Grade Student Achievement Explained by School Practices, Policies, and Politics Indicators](./plots/cor_gif_updated.gif)
...
combined_school_office_df <- bind_rows(PER_school_office_link_df, JOR_school_office_link_df,RWA_school_office_link_df, ETH_school_office_link_df, MDG_school_office_link_df,  SLE_school_office_link_df) 
...
```{r}
#| label: tbl-models
#| tbl-cap: GEPD Regression Output


#set controls
controls <- 'i(country, log(GDP)) + i(country, rural) + i(country, students_enrolled) + factor(country)'
practices <- 'i(country, presence_rate) + i(country, content_knowledge) + i(country, teach_score) + i(country, inputs) + i(country, infrastructure) + i(country, ecd_student_knowledge) + i(country, student_attendance) + i(country, operational_management) + i(country, instructional_leadership) + i(country, principal_knowledge_score) + i(country, principal_management)'
policies <- 'i(country, teacher_attraction) + i(country, teacher_selection_deployment) + i(country, teacher_support) + i(country, teaching_evaluation) + i(country, teacher_monitoring) + i(country, intrinsic_motivation) + i(country, standards_monitoring) + i(country, sch_monitoring) + i(country, sch_management_clarity) + i(country, sch_management_attraction) + i(country, sch_selection_deployment) + i(country, sch_support) + i(country, principal_evaluation)'
politics <- 'i(country, quality_bureaucracy) + i(country, national_learning_goals) + i(country, impartial_decision_making) + i(country, mandates_accountability)'
...
anova_df <- combined_school_office_df %>%
  ...
  mutate(country=factor(country, levels=unique(combined_school_office_df$country))) %>%
  na.omit
...
models %>% modelsummary(estimate= "{estimate}{stars}")
...
ggplot(baseline_pract_df, aes(x=pred, y=student_knowledge, color=country)) +
  ...
  labs(caption = str_wrap('Fitted values produced using a pooled OLS regression across countries of 4th grade student knowledge scores on GEPD school practice indicators.  Interaction terms were included so that slopes were allowed to vary by country.  No other controls included in regressions.' ,100)) +
  theme_bw() 
```
## PP
...
ggplot(baseline_pract_pol_df, aes(x=pred, y=student_knowledge, color=country)) +
  ...
  ylab('4th-grade student knowledge') +
    ...
    labs(caption = str_wrap('Fitted values produced using a pooled OLS regression across countries of 4th grade student knowledge scores on GEPD school practice & policy indicators.  Interaction terms were included so that slopes were allowed to vary by country.  No other controls included in regressions' ,100)) +
  theme_bw() 
```
## PPP
...
ggplot(baseline_pract_pol_bur_df, aes(x=pred, y=student_knowledge, color=country)) +
  ...
  ggtitle('Explaining 4th-grade Student Outcomes using \nSchool Practices + Policies + Politics') +
    labs(caption = str_wrap('Fitted values produced using a pooled OLS regression across countries of 4th grade student knowledge scores on GEPD school practice, policy, and politics indicators.  Interaction terms were included so that slopes were allowed to vary by country.  No other controls included in regressions' ,100)) +
  theme_bw() 
```
...
student_df <- read_dta(paste0(confidential_dir, "General/combined_g4.dta"))

aov_df <- student_df  %>%
  group_by(hashed_school_code) %>%
    mutate(weights=ipw/n()) %>%
  group_by(iso3c) %>%
  nest() %>%
  mutate(
    aov=map(data, ~aov(.x$student_knowledge ~ factor(.x$hashed_school_code), weights=.x$weights) %>% broom::tidy())
  ) %>%
  select(-data) %>%
  unnest(aov) %>%
  group_by(iso3c) %>%
  mutate(share_explained=100*sumsq/sum(sumsq))
