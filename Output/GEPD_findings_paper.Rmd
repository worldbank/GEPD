...
pract_ind <- c("student_knowledge"="4th Grade Student Knowledge",
...
cor_df <- combined_school_office_df %>%
  select(country, rural, names(pract_ind)) %>%
  group_by(country,rural) %>%
  mutate(across(names(pract_ind),~if_else(is.na(.),mean(.,na.rm=T),.))) %>%#impute with mean by country and urban/rural
  ungroup() %>%
  select(names(pract_ind))
