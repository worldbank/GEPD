...
defacto <- data_policy_combined %>% filter(str_detect(group, "Facto")) %>% pivot_wider( id_cols = c("country"),names_from = indicators, values_from = value) %>% left_join(combined_school_df %>% select(iso3c, student_knowledge, presence_rate, teach_score, content_knowledge) %>% rename(country= iso3c) %>%  group_by(country) %>% mutate(
  student_knowledge = mean(as.numeric(student_knowledge), na.rm=T),
  presence_rate = mean(as.numeric(presence_rate), na.rm=T),
  teach_score = mean(as.numeric(teach_score), na.rm=T),
  content_knowledge = mean(as.numeric(content_knowledge), na.rm=T),
