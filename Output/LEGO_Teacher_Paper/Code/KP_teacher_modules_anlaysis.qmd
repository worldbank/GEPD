...
if (str_to_lower(Sys.getenv("USERNAME")) == "wb469649" ){
  ...
  download_folder <-file.path(paste(project_folder,"CNT",country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v01_RAW", sep="_"),"Data/",province,"/raw/School/", sep="/"))
  sampling_folder <-file.path(paste(project_folder,"CNT",country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v01_RAW", sep="_"),"Data/",province,"/sampling", sep="/"))
  confidential_folder <- file.path(paste(project_folder,"CNT",country,paste(country,year,"GEPD", sep="_"),paste(country,year,"GEPD_v01_RAW", sep="_"),"Data/",province,"/confidential/School", sep="/"))
...
data_set_updated <- read_csv(paste(sampling_folder, '/GEPD_KP_weights_200_', currentDate,  '.csv', sep="")
) %>%
...
teacher_combined_df %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country, "_",province,"/", country,"_teacher_combined_modules.dta"))


matches %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country, "_",province,"/", country,"_teacher_modules_key.dta"))

#save input files
school_micro %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country, "_",province,"/", country,"_school_modules.dta"))

teacher_absence_micro %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country, "_",province,"/", country,"_teacher_absence.dta"))

teacher_pedagogy_micro %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country, "_",province,"/", country,"_teacher_pedagogy.dta"))

teacher_assessment_micro %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country, "_",province,"/", country,"_teacher_assessment.dta"))

teacher_questionnaire_micro %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country, "_",province,"/", country,"_teacher_questionnaire.dta"))

assess_4th_grade_anon %>%
  select(-ends_with('nogiraffe')) %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country, "_",province,"/", country,"_fourth_grade_assessment.dta"))

ecd_dta_anon %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country, "_",province,"/", country,"_first_grade_assessment.dta"))
```
