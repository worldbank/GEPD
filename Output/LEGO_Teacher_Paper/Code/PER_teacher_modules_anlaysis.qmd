...
df_weights_function <- function(dataset,scode, snumber, prov) {
  ...
  mutate(school_code = codigo.modular)
  ...
  temp<-get(dataset) 
    ...
    if ("school_code" %in% colnames(temp)) {
      temp <- temp %>%
        ...
        select(hashed_school_code, hashed_school_province, hashed_school_district, school_code, everything())
    }
...
#   read_dta(file.path(download_folder, "EPDash.dta")) %>%
...
#   distinct(interview__key, school_code, .keep_all=TRUE)
...
school_micro_comb <- school_micro_raw %>%
  left_join(
    school_dta_short %>%
      rename(
        sch_goals_exist=school_goals_exist,
        sch_goals_clear=school_goals_clear,
        sch_goals_relevant=school_goals_relevant,
        sch_goals_measured=school_goals_measured
      ) %>%
...
#   distinct(interview__key, questionnaire_roster__id, .keep_all=TRUE)
...
#   distinct(interview__key, questionnaire_selected__id, .keep_all=TRUE)

teacher_absence_dta <- teacher_absence_final

#merge on derived indicators
teacher_absence_dta_comb <- teacher_absence_dta %>%
  mutate(teacher_number=questionnaire_selected__id) %>%
  # left_join(
  #   teacher_absence_final %>%
  #     select(school_code, teacher_number, contains(ind_list))
  # ) %>%
  rename(TEACHERS__id=questionnaire_selected__id)
...
#load(paste0(confidential_folder,"/dashboard_teacher_assessment_data.RData"))

teacher_assessment_dta_comb <- teacher_assessment_dta
...
#merge in teacher weigths 
teacher_assessment_weights <- school_weights %>%
  select(school_code, teacher_obs_weight_component)
...
#add the weights to g1 and g4 student assessments 
#add student weights to the g4 and g1 datasets
g4_weights <- school_weights %>%
  select(school_code, g4_stud_weight_component)

assess_4th_grade_anon <- assess_4th_grade_anon %>%
  left_join(g4_weights, by = "school_code")
...
ecd_dta_anon <- ecd_dta_anon %>%
  left_join(g1_weights, by = "school_code") %>%
  select(-starts_with("hashed"))
...
teacher_combined_df %>%
  ...
  write_excel_csv(path=paste0(save_input_folder,"/",country,"/", country,"_teacher_combined_modules.csv"))


matches %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country,"/", country,"_teacher_modules_key.dta")) %>%
  write_excel_csv(path=paste0(save_input_folder,"/",country,"/", country,"_teacher_modules_key.csv"))

#save input files
school_micro %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country,"/", country,"_school_modules.dta")) %>%
  write_excel_csv(path=paste0(save_input_folder,"/",country,"/", country,"_school_modules.csv"))


teacher_absence_micro %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country,"/", country,"_teacher_absence.dta")) %>%
  write_excel_csv(path=paste0(save_input_folder,"/",country,"/", country,"_teacher_absence.csv"))

teacher_pedagogy_micro %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country,"/", country,"_teacher_pedagogy.dta")) %>%
  write_excel_csv(path=paste0(save_input_folder,"/",country,"/", country,"_teacher_pedagogy.csv"))

teacher_assessment_micro %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country,"/", country,"_teacher_assessment.dta")) %>%
  write_excel_csv(path=paste0(save_input_folder,"/",country,"/", country,"_teacher_assessment.csv"))

teacher_questionnaire_micro %>%
  janitor::clean_names() %>%
  write_dta(path=paste0(save_input_folder,"/",country,"/", country,"_teacher_questionnaire.dta"))  %>%
  write_excel_csv(path=paste0(save_input_folder,"/",country,"/", country,"_teacher_questionnaire.csv"))
