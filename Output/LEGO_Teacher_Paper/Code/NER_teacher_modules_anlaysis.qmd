...
teacher_questionnaire<-read_dta(file.path(download_folder, "ID_questionnaire_roster.dta")) %>%
...
teacher_combined_df <- teacher_combined_df %>%
  left_join(teacher_pedagogy_micro %>% mutate(TEACHERS__id=pedagogy_id), na_matches='never') %>%
  group_by(school_code,TEACHERS__id) %>%
  slice(1) 


sum(!is.na(teacher_combined_df$m4saq1_number))
pct <- round(100*sum(!is.na(teacher_combined_df$pedagogy_id))/nrow(teacher_pedagogy_micro))
