...
school_weights <- bind_rows(
  ...
  select(c("hashed_school_code",
           ...
           "iso3c", 
           "date")) %>%
  #fix an issue where there were no eligible teachers
  mutate(teacher_obs_weight_component=case_when(
    teacher_obs_weight_component==-Inf ~ 1,
    teacher_obs_weight_component==0 ~ 1,
    is.na(teacher_obs_weight_component) ~ 1,
    TRUE ~ teacher_obs_weight_component
  )) %>%
  mutate(
    g4_stud_weight=ipw*g4_stud_weight_component,
    g1_stud_weight=ipw*g1_stud_weight_component,
    teacher_absence_weight=ipw*abs_weight_component,
    teacher_questionnaire_weight=ipw*teacher_weight_component,
    teacher_observation_weight=ipw*teacher_obs_weight_component
  )
...
school_weights %>% group_by(iso3c) %>% summarise(teacher_obs_weight_component=median(teacher_obs_weight_component))
```
