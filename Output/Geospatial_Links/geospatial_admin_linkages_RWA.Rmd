...
shared_folder <- 'C:\\Users\\wb469649\\WBG\\Daniel Rogger - Global Education Dashboard\\01_raw_data\\'
...
#create dummies for survey time, enumerator, ENUMq1
main_po_data <- main_po_data %>%
  mutate(
  po_survey_length=difftime(lubridate::ymd_hms(ENUMq1),lubridate::ymd_hms(survey_time),units="mins"),
  po_survey_length=case_when(
    as.numeric(po_survey_length)<20 ~ "under 20 min",
    between(as.numeric(po_survey_length), 20,60) ~ "20-60 min",
    as.numeric(po_survey_length)>60 ~ "Over 60 min"
  )) %>%
  # get hour of survey
  mutate(po_survey_time=lubridate::ymd_hms(survey_time),
         po_survey_time=lubridate::hour(po_survey_time),
         po_survey_time=case_when(
           between(po_survey_time,8,12) ~ "Morning",
           between(po_survey_time,12,17) ~ "Afternoon",
           between(po_survey_time,17,21) ~ "Evening",
           between(po_survey_time,21,24) ~ "Night",
           between(po_survey_time,0,8) ~ "Night"
         ))

main_po_data <- fastDummies::dummy_cols(main_po_data, select_columns = 'enumerator_number')
main_po_data <- main_po_data %>%
  rename_at(vars(starts_with('enumerator_number_')), function(x) paste0(x,"_",country,"_",year))
main_po_data <- fastDummies::dummy_cols(main_po_data, select_columns = 'po_survey_length')
main_po_data <- fastDummies::dummy_cols(main_po_data, select_columns = 'po_survey_time')
...
po_district_df <- main_po_data %>%
  ...
  select(one_of(po_ind_list,"office_lat","office_lon"), starts_with('enumerator_number_'), starts_with('po_survey_length_'), starts_with('po_survey_time_')) %>%
  summarise(across(everything(),mean, na.rm=TRUE)) %>%  as_tibble() %>%
...
linked_school_data_csv <- linked_school_data %>%
  ...
  select(-geometry) %>%
  left_join(keyfile) %>%
  select(hashed_school_code, ADM0_NAME, ADM1_NAME, ADM2_NAME, school_lat, school_lon, everything()) 

write_excel_csv(linked_school_data_csv, paste0(confidential_folder, "/School/linked_po_school_data_",country,"_",year,".csv"))
write_excel_csv(linked_school_data_csv, paste0(shared_folder, country,"/linked_po_school_data_",country,"_",year,".csv"))
...
write_excel_csv(linked_school_data_csv, paste0(shared_folder, country,"/linked_po_directors_school_data_",country,"_",year,".csv"))
...
po_district_df <- main_po_data %>%
  ...
  haven::zap_labels() %>%
  group_by(ADM2_NAME, ADM2_CODE) %>%
  select(one_of(po_ind_list,"office_lat","office_lon"), starts_with('enumerator_number_'), starts_with('po_survey_length_'), starts_with('po_survey_time_'), ENUMq4) %>%
  summarise(across(everything(),Hmisc::wtd.mean, weights=ENUMq4, na.rm=TRUE)) %>%  as_tibble() %>%
  filter(!is.na(ADM2_NAME))
...
write_excel_csv(linked_school_data_csv, paste0(shared_folder, country,"/linked_po_enum_score_weighted_school_data_",country,"_",year,".csv"))
