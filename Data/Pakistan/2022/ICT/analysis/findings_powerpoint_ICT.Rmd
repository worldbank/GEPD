---
title: "Global Education Policy Dashboard - Pakistan - ICT 2022"
...
Roadmap:  
- Below is a set of tables and charts containing findings for the Pakistan - ICT 2022 Global Education Policy Dashboard survey. - We will start with breakdowns of our Practice indicators  
...
#list countries
countries <- c("PER", "JOR", "RWA", "MDG", "ETH", "SLE", "NER", "PAK-ICT")
...
combined_df_raw <- bind_rows(PAK_ICT_data, NER_data,SLE_data,PER_data,JOR_data, ETH_data, RWA_data, MDG_data)
...
#build table
tab_df <- tab_df  %>%
  select(`Indicator Name`,`Pakistan-ICT`,`Niger`,`Sierra Leone`, Peru, Jordan, Rwanda, Ethiopia, Madagascar)
...
svymeantab <- function(data, strata) {
  ...
  get(data) %>%
    ...
    select(one_of(indicators_list), Tehsil, Gender, urban_rural, ipw ) %>%
    pivot_longer(cols=one_of(indicators_list),
                 names_to = 'indicators',
                 values_to='value') %>%
    as_survey_design(strata=strata,
                     weight=ipw) %>%
    select(-c( 'Tehsil', 'Gender', 'urban_rural', 'ipw')) %>%
    group_by(indicators) %>%
...
# School Survey
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Pakistan - ICT 2022 - Urban/Rural",
