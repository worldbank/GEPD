...
final <- read.csv(paste0(save_folder, "amplb_pak.csv"))
```
...
final %>% filter(!is.na(.)) %>%  summarise(
  ...
  `Number of schools covered` = n_distinct(`School.EMIS`),
  `Total booklets entered` = n(),
  `Number of booklet 1` = sum(type_booklet==1),
  `Number of booklet 2`= sum(type_booklet==2),
  `Average number of students` = round(mean(Number.of.students.for.which.the.assessment.was.entered), digits = 0),
...
perschool<- final %>% group_by(School.EMIS) %>%  summarise(
...
school_completed <- final %>% filter(`School.EMIS`!= "Blank interview") %>% pull(`School.EMIS`)
...
school_map <- read.csv("C:/Users/wb577189/OneDrive - WBG/My files/Dashboard (Team Folder)/Country_Work/Pakistan_all/2021/Data/01. Pilot/clean/final_complete_school_data.csv") %>% select(school_name_preload, school_province_preload, school_district_preload, school_emis_preload, lat, lon) %>% right_join(perschool %>% rename(school_emis_preload=`School.EMIS`)) %>% filter(!is.na(.))
