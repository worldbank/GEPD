...
```{r regional_weights, echo=TRUE}
 ...
 #form regional weights, based on the number of schools.
  region_weights<- data_set_updated %>% #read in school sample frame as basis for forming weights
    group_by(region) %>%
    summarise(n_schools=n() )  #count number of schools per region

  #select six regions
  regions_list <- region_weights %>%
    sample_n(6, weight=n_schools)
...
```{r woredas}
  ...
  ungroup() %>%
  mutate(N_woredas_to_sample=(20*n_schools/sum(n_schools))) #get number of woredas based on number of schools
  ...
  woreda_list<- sample_updated %>%
    ...
    filter(!is.na(n_schools)) %>% #drop woredas in regions not selected
    sample_n(N_woredas_to_sample) %>%
    dplyr::select(region, zone, woreda, region_code, zone_code, woreda_code)
  ...
  woreda_list <- woreda_list %>%
    mutate(
      ...
      zone_code=if_else(region=="Addis Ababa",as.numeric(NA),as.numeric(zone_code)),
      woreda_code=if_else(region=="Addis Ababa",as.numeric(NA),as.numeric(woreda_code))
    )
