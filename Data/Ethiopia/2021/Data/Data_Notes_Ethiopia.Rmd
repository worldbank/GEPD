...
```{r}
  ...
  sample_random_2020 <- school_dta_2020 %>% 
  sample_n(50, replace = F, weight = NULL)

  # Matching with the list from 2021

    # Keep only variables of interest
     random_2020 <- sample_random_2020 %>% 
       select(school_code, lat, lon) %>% 
       mutate(year = 2020)
     
     schools_2021 <- school_dta_2021 %>% 
       select(school_code, lat, lon) %>% 
       rename(school_id = school_code) %>% 
       mutate(year = 2021) %>% filter(!is.na(.)) #some do not have GPS points to match on

     
   ## Shortest path with distance this time---- 
      library(geosphere)
      
      # create distance matrix
      mat <- distm(random_2020[,c('lon','lat')], schools_2021[,c('lon','lat')], fun=distVincentyEllipsoid) 
      
      # assign the name to the point in list1 based on shortest distance in the matrix
      random_2020$Closest_school_Post_covid <- schools_2021$school_id[max.col(-mat)]
      random_2020$Distance_btw_schools <- apply(mat, 1, min)
