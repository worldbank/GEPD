---
title: "Rwanda Public Officials Analysis"
author: "Brian Stacy"
date: "`r Sys.Date()`"
output:
...
#set directory to bring in data
 ...
 if(Sys.info()["user"] == "wb577189"){
    ...
    work_dir<- "C:/Users/wb577189/OneDrive - WBG/CNT/ETH/ETH_2020_GEPD/ETH_2020_GEPD_v01_RAW/Data/anonymized/"


    
  } else if (Sys.info()["user"] == "wb469649") {
    
work_dir<- "//wbgfscifs01/GEDEDU/datalib-edu/projects/GEPD-Confidential/CNT/RWA/RWA_2020_GEPD/RWA_2020_GEPD_v01_RAW/Data/confidential/"
...
office_list <- read_csv("//wbgfscifs01/GEDEDU/datalib-edu/projects/GEPD-Confidential/CNT/RWA/RWA_2020_GEPD/RWA_2020_GEPD_v01_RAW/Data/sampling/office_list.csv")

auxillary_frame_info<-read_csv(file.path(paste("//wbgfscifs01/GEDEDU/datalib-edu/projects/GEPD-Confidential/CNT/RWA/RWA_2020_GEPD/RWA_2020_GEPD_v01_RAW/Data/raw/School/pupil_counts_district/primary_pupil_enrollment.csv",sep=""))) %>%
...
# 
# po_df <- read_dta("C:/Users/wb469649/WBG/Daniel Rogger - DataWork/EncryptedData/Master public_officials Encrypted/DataSet/A_Confidential_Rwanda+PO.dta" ) %>%
...
public_officials_agg <- public_officials_dta_clean %>%
  ...
  mutate(office_preload=str_to_upper(office_preload)) %>%
...
#rename offices to match those in original district list
#first form regular expression with list of districts
if (exists('district_regex')) {
  rm('district_regex')
}


for (i in office_list$district) {
  if (!exists('district_regex')) {
  district_regex<-i
  } else {
  district_regex<-paste(district_regex, i, sep="|")
  }
}

#rename offices to match those in original sector list
#first form regular expression with list of sectors
if (exists('sector_regex')) {
  rm('sector_regex')
}


for (i in office_list$sector) {
  if (!exists('sector_regex')) {
  sector_regex<-i
  } else {
  sector_regex<-paste(sector_regex, i, sep="|")
  }
}

public_officials_agg <- public_officials_agg %>%
  left_join(office_list,  by=c('office_preload'='sector')) %>%
  mutate(school_district_preload=if_else(govt_tier!='Regional office (or equivalent)',
                          district,office_preload)) %>%
  # mutate(school_district_preload=if_else(govt_tier=='Regional office (or equivalent)',
  # str_trim(str_extract(office_preload,paste('\\b(',district_regex,')\\b', sep=""))),
  # str_trim(str_extract(office_preload,paste('\\b(',sector_regex,')\\b', sep="")))
  # )) %>%
...
public_offical_link_key <- public_officials_agg %>%
  select(office_preload, school_district_preload)
...
- The score is the average of all public officials in the region, including at the district level
...
#select data just for Rwanda
fc_Rwanda <- fc[fc@data$ADM0_NAME=="Rwanda",]



#plot base map of Rwanda with labels for subnational units.  http://rstudio.github.io/leaflet/popups.html

region_regex<-c('North', 'South', 'East','West', 'Kigali City')

maps <- fortify(fc_Rwanda, region="ADM1_NAME") %>%
  mutate(id=str_trim(str_extract(id,region_regex))) %>%
  group_by(group) %>%
...
#collapse to district level for mapping
po_df_collapse <- public_officials_agg %>%
  mutate(district=if_else(govt_tier!='Regional office (or equivalent)',
                          district,school_district_preload)) %>%
  left_join(auxillary_frame_info) %>%
  mutate(id=region) %>%
  mutate(id=if_else(id=="Kigali city", "Kigali City", id)) %>%
  group_by(id) %>%
  mutate(n_interviews=sum(n_interviews)) %>%
  summarise_all(list(~if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
...
school_df_collapse <- school_dta_matched %>%
  mutate(n_interviews=n()) %>%
  left_join(auxillary_frame_info,by=c('school_district_preload'='district')) %>%
  mutate(id=region) %>%
  mutate(id=if_else(id=="Kigali city", "Kigali City", id)) %>%
  group_by(id) %>%
...
acm<-  ggplot() +
  ...
  theme_bw() +
    xlab('Longitude') + 
    ylab('Latitude') + 
    # ggtitle(str_wrap('Map Survey of Public Officials Scores for Mandates and Accountability by Rwandan Districs', width=50)) +
...
achieve <- ggplot() +
    ...
    expand_limits(x = maps$long, y = maps$lat) +
    scale_fill_viridis_c(limits=c(0,65),
...
patch_plot + plot_annotation(
  ...
  caption = 'Regional shading shows average of all public officials in that district including officials at Sector level'
)
```
...
```{r table_output, echo=FALSE}
  ...
  kable_styling( bootstrap_options = c("striped", "hover")) %>%
    pack_rows("East", 1, 5, indent=F) %>%
    pack_rows("Kigal City", 5, 9, indent=F) %>%
    pack_rows("North", 9, 13, indent=F) %>%
    pack_rows("South", 13, 17, indent=F) %>%
    pack_rows("West", 17, 20, indent=F)
...
merge_df_collapse<-school_dta_matched %>%
  mutate(district=school_district_preload) %>%
  group_by(district) %>%
  select(district,ind_list, mandates_accountability, quality_bureaucracy, national_learning_goals, impartial_decision_making , total_enrolled) %>%
  summarise_all(list(~if(is.numeric(.)) wtd.mean(.,w=total_enrolled, na.rm = TRUE) else first(.)))




acm_scatter<-  ggplot(merge_df_collapse, aes(x=mandates_accountability, y=student_knowledge)) +
        geom_point() +
...
qb_scatter<-  ggplot(merge_df_collapse, aes(x=quality_bureaucracy, y=student_knowledge)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
nlg_scatter<-  ggplot(merge_df_collapse, aes(x=national_learning_goals, y=student_knowledge)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
idm_scatter <- ggplot(merge_df_collapse, aes(x=impartial_decision_making, y=student_knowledge)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
patch_scatter + plot_annotation(
  ...
  caption = str_wrap('Regional average of all public officials in that district including officials at Sector level',150)
)
...
acm_scatter<-  ggplot(merge_df_collapse, aes(x=mandates_accountability, y=ecd_student_knowledge)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
qb_scatter<-  ggplot(merge_df_collapse, aes(x=quality_bureaucracy, y=ecd_student_knowledge)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
nlg_scatter<-  ggplot(merge_df_collapse, aes(x=national_learning_goals, y=ecd_student_knowledge)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
idm_scatter <- ggplot(merge_df_collapse, aes(x=impartial_decision_making, y=ecd_student_knowledge)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
acm_scatter<-  ggplot(merge_df_collapse, aes(x=mandates_accountability, y=presence_rate)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
qb_scatter<-  ggplot(merge_df_collapse, aes(x=quality_bureaucracy, y=presence_rate)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
nlg_scatter<-  ggplot(merge_df_collapse, aes(x=national_learning_goals, y=presence_rate)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
idm_scatter <- ggplot(merge_df_collapse, aes(x=impartial_decision_making, y=presence_rate)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
idm_scatter2 <- ggplot(merge_df_collapse, aes(x=politicized_personnel_management, y=presence_rate)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
        ...
        ylab("Presence Rate") +
        xlab("Politicized Personnel Management") +
        stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
                     label.x.npc = "right", label.y.npc = -0.1,
                     formula = 'y~x', parse = TRUE, size = 5)   
...
patch_scatter3 <- idm_scatter + idm_scatter2 
...
```{r scatters_content, echo=FALSE, fig.height=6, fig.width=9, warning=FALSE}





acm_scatter<-  ggplot(merge_df_collapse, aes(x=mandates_accountability, y=content_knowledge)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
qb_scatter<-  ggplot(merge_df_collapse, aes(x=quality_bureaucracy, y=content_knowledge)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
nlg_scatter<-  ggplot(merge_df_collapse, aes(x=national_learning_goals, y=content_knowledge)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
idm_scatter <- ggplot(merge_df_collapse, aes(x=impartial_decision_making, y=content_knowledge)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
...
idm_scatter2 <- ggplot(merge_df_collapse, aes(x=politicized_personnel_management, y=content_knowledge)) +
        geom_point() +
        geom_smooth(method='lm', mapping = aes()) +
        theme_bw() +
        theme(
        ...
        ylab("Teacher Content Knowledge") +
        xlab("Politicized Policy Implementation") +
        stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
                     label.x.npc = "right", label.y.npc = -0.1,
                     formula = 'y~x', parse = TRUE, size = 5)   
...
patch_scatter + plot_annotation(
  title = 'Plot of Teacher Content Knowledge on Bureaucracy Indicators, by Region',
  caption = str_wrap('Regional average of all public officials in that District including officials at Sector level',150)
)
...
patch_scatter3 + plot_annotation(
  title = 'Plot of Teacher Content Knowledge on Impartial Decision Making Bureaucracy Indicators, by Region',
  caption = str_wrap('Regional average of all public officials in that District including officials at Sector level',150)
)
...
write_dta(merge_df_collapse, path="C:/Users/wb469649/OneDrive - WBG/Dashboard - School - Public Officials - Confidential/Countries/Rwanda/Data/public_officials_school_merged_df_collapsed.dta")

#rename some data frames for saving  
merge_df_collapse_Rwanda<-  merge_df_collapse
top_50_table_Rwanda<-top_50_table
sumstats_df_Rwanda<-sumstats_df
maps_Rwanda<-maps 
school_df_collapse_Rwanda<-school_df_collapse 
po_df_collapse_Rwanda<-po_df_collapse
public_officials_dta_clean_Rwanda <- public_officials_dta_clean
school_dta_matched_Rwanda <- school_dta_matched


save(list=c('merge_df_collapse_Rwanda', 'top_50_table_Rwanda', 'sumstats_df_Rwanda','maps_Rwanda','school_df_collapse_Rwanda','po_df_collapse_Rwanda',
            'public_officials_dta_clean_Rwanda','school_dta_matched_Rwanda' ), 
     file = file.path("C:/Users/wb469649/OneDrive - WBG/Dashboard - School - Public Officials - Confidential/Countries/Rwanda/Data/public_officials_school_merged_files.RData"))
