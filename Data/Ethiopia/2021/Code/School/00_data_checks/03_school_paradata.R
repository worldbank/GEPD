...
# POST(paste(server_add,"/api/v1/export/paradata/f09adb3305a440bca53462755afc4fa7$",quest_version,"/start", sep=""),
#      authenticate(user, password))
# 
# 
...
# Sys.sleep(10)
# 
# dataDownload <- GET(paste(server_add,"/api/v1/export/paradata/f09adb3305a440bca53462755afc4fa7$", quest_version,"/",sep=""),
#                     authenticate(user, password))
# 
...
var.labels=c(
            ...
            timestamp_utc = "Date and time when the event happened",
...
######################################
#clean up timestamp_utc
para_df <- para_df %>% 
  mutate(timestamp_utc= ymd_hms(timestamp_utc))
...
para_df_tab <- para_df %>%
  select( responsible, date, timestamp_utc, module, section, indicator, question, varlabel, timelength_sec, ï..interview__id) %>%
  group_by(ï..interview__id, section) %>% 
  summarise(responsible=first(responsible), date=first(date), module=first(module), timestamp_utc=last(timestamp_utc))
...
school_modules_complete <- para_df_module %>%
  mutate(interview__id=as.character(ï..interview__id)) %>%
  ungroup() %>%
  zap_labels() %>%
...
# bscols(widths=c(3,NA),
#        list(
#          filter_slider("Time", "Time Question Completed", linked_df, ~timestamp_utc),
#          filter_checkbox("enumerator", "Enumerator", linked_df, ~responsible),
#          filter_checkbox("date", "Date of Survey", linked_df, ~as.character(date), inline=FALSE),
#          filter_checkbox("section", "Section", linked_df, ~section, inline=FALSE),
#          filter_checkbox("module", "Module", linked_df, ~module, inline=FALSE)
...
#        
#        list (
#          plot_ly(linked_df, x=~question, y=~timestamp_utc, type='scatter', mode='markers', color=~responsible) %>%
#            layout(title='Time Question Completed by Enumerator',yaxis=list(title='Time'), xaxis=list(title='Question ID')),
#          datatable(linked_df, 
#                    colnames=c('Interview Code'='ï..interview__id', 'Enumerator' = 'responsible', 'Date' = 'date', 'Module' = 'module', 'Section' = 'section',
#                               'Indicator' = 'indicator', 'Question ID' = 'question', 'Question'='varlabel', 'Time' = 'timestamp_utc'),
#                    extensions="Scroller", style="bootstrap", class="compact", width="100%",
#                    options=list(deferRender=TRUE, scrollY=300, scroller=TRUE))
#          
#        )
