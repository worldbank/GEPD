...
if(Sys.info()["user"] == "wb577189"){
  ...
  expert_dir<- "C:/Users/wb577189/OneDrive - WBG/My files/Dashboard (Team Folder)/Country_Work/Ethiopia/2021/Data/clean/Policy_Survey/"
  
} else {
  expert_dir <- choose.dir(default = "", caption = "Choose folder with expert data")
...
expert_dta_teachers_final <- expert_dta_teachers_final %>%
  mutate(teacher_attraction=read_var(A4),
         teacher_salary=(read_var(A4)/936.34)) # GDP per capita https://data.worldbank.org/indicator/NY.GDP.PCAP.CD?locations=MG
...
expert_dta_school_management_final <- expert_dta_school_management_final %>%
  ...
  mutate(sch_selection_deployment=principal_rubric+principal_factors)
...
expert_dta_learners_final <- expert_dta_learners_final %>%
  ...
  mutate(nutrition_programs=1+iodization/4 + iron_fortification/4 + breastfeeding + school_feeding)
...
expert_dta_learners_final <- expert_dta_learners_final %>%
  ...
  mutate(health_programs=1+immunization + healthcare_young_children + antenatal_skilled_delivery/3)
