---
title: "Global Education Policy Dashboard - Ethiopia 2020"
...
Roadmap:  
- Below is a set of tables and charts containing findings for the Ethiopia 2020 Global Education Policy Dashboard survey. - We will start with breakdowns of our Practice indicators  
...
work_dir<- "C:/Users/wb469649/WBG/HEDGE Files - HEDGE Documents/GEPD-Confidential/CNT/ETH/ETH_2020_GEPD/ETH_2020_GEPD_v01_RAW/Data/anonymized/"
...
# School Survey
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Ethiopia 2020 - Urban/Rural",
...
Location1<- ggplot(data=filter(sumstats_dumbell, grepl("Pedagogical|Effort|Knowledge|Attendance|Proficiency|Capacity", varlabel)), aes( y=varlabel)) +
...
Location2<- ggplot(data=filter(sumstats_dumbell, !grepl("Pedagogical|Effort|Knowledge|Attendance|Proficiency|Capacity", varlabel)), aes( y=varlabel)) +
  geom_point(data=filter(dumbbell_df, !grepl("Pedagogical|Effort|Knowledge|Attendance|Proficiency|Capacity", varlabel)), aes(x=mean, color=group)) +
  geom_dumbbell(aes(x=mean_Urban, xend=mean_Rural),
    size=3, color="#e3e2e1",
                colour_x = "#5b8124", colour_xend = "#bad744",
...
#public/private
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Ethiopia 2020 - Public/Private Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
...
Location <- school_dta_short_anon %>%
  mutate(Type=factor(Location, levels=c("Rural", "Urban"), labels = c("Rural", "Urban"))) %>%
  filter(!is.na(Location))



infr_Location <- Location %>%
  group_by(Type) %>%
...
```{r Location_input, echo=FALSE}


Location_input <- school_dta_short_anon %>%
  mutate(Type=factor(Location, levels=c("Rural", "Urban"), labels = c("Rural", "Urban"))) %>%
  filter(!is.na(Location))



inpt_Location <- Location %>%
  group_by(Type) %>%
  mutate(weight=ipw/sum(ipw, na.rm=T)) %>%
  ungroup() %>%
  ggplot(aes(x=inputs))  +
...
textbooks <- final_indicator_data_PKNW_anon %>%
  ...
  mutate(Type=factor(Location.x, levels=c("Rural", "Urban"), labels = c("Rural", "Urban"))) %>%
  filter(!is.na(Type)) %>%
