...
sumstats_df<-my_skim(sumstats) %>%
  ...
  mutate(ci_low=as.numeric(numeric.mean)-1.96*(as.numeric(numeric.sd)/sqrt(as.numeric(numeric.complete_count))),
         ci_high=as.numeric(numeric.mean)+1.96*(as.numeric(numeric.sd)/sqrt(as.numeric(numeric.complete_count)))) %>%
  mutate(ci=paste("[",round(ci_low,2),", ", round(ci_high,2),"]", sep="")) %>%
  select(variable, numeric.mean, ci, numeric.sd, numeric.p0, numeric.p25, numeric.p50, numeric.p75, numeric.p100, numeric.complete_count, n_missing, numeric.hist) %>%
