---
title: "Item Analysis of GEPD Teacher Knowledge Module"
author: Brian Stacy
date: "`r Sys.Date()`"
output:
  html_document: 
...
Using GEPD data from Peru, this paper examines the psychometric properties of the mathematics portion of the teacher assessment module.  We estimate a two parameter logistic (3PL) item response theory (IRT) model.  We produce item characteristic curves (ICC) and test characteristic curves from this model.  We then compare our results to data from a similar assessment used in the Service Delivery Indicators project in several Sub-Sahara African countries.
...
Before estimating the model, we begin by examining summary statistics and missing values in our item response data.
...
Next I will display inter-item correlations
...
estc_math <- list(est_sdi_math$coef, est_dash_math$coef)
estc_lang <- list(est_sdi_lang$coef, est_dash_lang$coef)

#bring in variance-covariance matrix
estv_math <- list(est_sdi_math$var, est_dash_math$var)
estv_lang <- list(est_sdi_lang$var, est_dash_lang$var)

#equate two models math
mod2pl_irt_math <- modIRT(coef = estc_math, var = estv_math, display = FALSE)

direclist2pl_math <- direc(mod1 = mod2pl_irt_math[1], mod2 = mod2pl_irt_math[2], method = "Stocking-Lord")
math_peru_scores_convert <- convert(A=direclist2pl_math$A, B=direclist2pl_math$B, person.par=math_teach_scores$F1)
...
direclist2pl_lang <- direc(mod1 = mod2pl_irt_lang[1], mod2 = mod2pl_irt_lang[2], method = "Stocking-Lord")
lang_peru_scores_convert <- convert(A=direclist2pl_lang$A, B=direclist2pl_lang$B, person.par=lang_teach_scores$F1)
...
math_peru_scores_convert <- math_peru_scores_convert %>%
  transmute(F1=math_peru_scores_convert) %>%
  mutate(countrycode="PER",
         countryname="Peru")

lang_peru_scores_convert <- lang_peru_scores_convert %>%
...
Assessment_math_scores<-merge(df, sdi_math_scores, by="row.names") %>%
  ...
  bind_rows(math_peru_scores_convert)


ggplot(Assessment_math_scores[Assessment_math_scores$countrycode!="PAK",], aes(F1, color=countryname)) +
  geom_histogram(position="identity", binwidth=0.05, aes(y=..density.., fill=countryname),  alpha=0.25) +
  geom_density(size=1) +
  expand_limits(x=c(-3,3)) +
  #facet_wrap(~countryname, ncol=2)+
  labs(x=expression(theta))+
  ggtitle(wrapper("Distributions of Estimated Math Teacher Ability by Country", width=35))


Assessment_math_scores %>%
  group_by(countryname) %>%
  summarise(math_ability_scores=mean(F1),
            median_math_ability_scores=median(F1))


#now bring all the data together
Assessment_lang_scores<-merge(df, sdi_lang_scores, by="row.names") %>%
  dplyr::select(countryname, countrycode, F1) %>%
  bind_rows(lang_peru_scores_convert)


ggplot(Assessment_lang_scores[Assessment_lang_scores$countrycode!="PAK",], aes(F1, color=countryname)) +
  geom_histogram(position="identity", binwidth=0.05, aes(y=..density.., fill=countryname),  alpha=0.25) +
  geom_density(size=1) +
  expand_limits(x=c(-3,3)) +
  #facet_wrap(~countryname, ncol=2)+
  labs(x=expression(theta))+
  ggtitle(wrapper("Distributions of Estimated Language Teacher Ability by Country", width=35))


Assessment_lang_scores %>%
  group_by(countryname) %>%
  summarise(mean_lang_ability_scores=mean(F1),
            median_lang_ability_scores=median(F1))
