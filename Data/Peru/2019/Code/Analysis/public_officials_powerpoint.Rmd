...
main_indicator_labels<-c( 
    ...
    #start with teachers
    expert_dta_teachers <- readxl::read_xlsx(path=paste(expert_dir, 'PolicySurvey_Peru_VD.xlsx', sep="/"), sheet = 'Teachers', .name_repair = 'universal') 
    ...
    expert_dta_teachers_final <- expert_dta_teachers_final %>%
      mutate(initial_teacher_salary=2800.4/6947.3) %>% # based on A4 and GDP per capita from 2018 World Bank  https://data.worldbank.org/indicator/NY.GDP.PCAP.CD?locations=PE.  Also see https://www.cgdev.org/blog/chart-week-teacher-pay-around-world-beyond-disruption-and-deskilling
      mutate(incentives_hard_to_staff = as.numeric(NA),
             career_path_teachers = as.numeric(NA) ) %>%
      mutate(teacher_attraction=)
    
    
    expert_dta_inputs <- readxl::read_xlsx(path=paste(expert_dir, 'PolicySurvey_Peru_VD.xlsx', sep="/"), sheet = 'Inputs')
    expert_dta_school_management <- readxl::read_xlsx(path=paste(expert_dir, 'PolicySurvey_Peru_VD.xlsx', sep="/"), sheet = 'School_Management')
    expert_dta_learners <- readxl::read_xlsx(path=paste(expert_dir, 'PolicySurvey_Peru_VD.xlsx', sep="/"), sheet = 'Learners')
...
data_defacto <- school_dta_short %>%
  ...
  ggplot(data=data_defacto, aes(x=indicators, y=indicator_values, fill='#ff0000')) + #now plot the output in a bar graph
    geom_boxplot(position="dodge") +
    scale_x_discrete(labels = str_wrap(main_indicator_labels, width = 25)) + 
...
data_plot <- public_officials_dta_clean %>%
  ...
  mutate(govt_tier=str_remove(govt_tier, "\\)")) %>%
    mutate(govt_tier=str_remove(govt_tier, "\\(")) %>%
  group_by(govt_tier) %>%
  select(interview__id,  govt_tier, national_learning_goals, mandates_accountability, quality_bureaucracy,
         impartial_decision_making) %>%
  pivot_longer(cols=c("national_learning_goals", "mandates_accountability", 
                      "quality_bureaucracy", "impartial_decision_making"),
               names_to = "indicators",
               values_to = "indicator_values") 
...
dataMedian <- summarise(group_by(data_plot, indicators , govt_tier), MD = round(median(indicator_values),1))
  ...
  ggplot(data_plot, aes(x=indicators, y=indicator_values, fill=govt_tier)) + #now plot the output in a bar graph
    ...
    scale_x_discrete(labels = str_wrap(c("National Learning Goals",
                                         ...
                                         "Impartial Decision Making"), width = 25)) + 
