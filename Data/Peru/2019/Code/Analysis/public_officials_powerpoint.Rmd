...
learning_dataMedian <- summarise(group_by(learning_data_plot, Gender , Type), MD = round(weighted.median(student_knowledge, school_ipw, na.rm=T),2))
  ...
  ggplot(learning_data_plot, aes(x=Type, y=student_knowledge, fill=Gender)) + #now plot the output in a bar graph
    ...
    geom_text(data = learning_dataMedian, aes(x=Type, y=MD, label=MD), 
              position = position_dodge(width = 0.8), size = 3, vjust = -0.65) +
  xlab(str_wrap("Urban/Rural Status", width=75))+
  ylab("Fraction Correct on 4th Grade Assessment") +
  ggtitle(str_wrap("Boxplot of Student Math and Language Learning by Gender and Urban/Rural", width=75)) +
...
infr_urban_rural <- urban_rural %>%
  ...
  ggplot(aes(x=infrastructure))  +
    geom_density(aes(fill=Type , weight=weight), alpha=0.4) +
    theme_bw() +
  labs(x='Infrastructure Index')+
  ggtitle("Infrastructure Across Urban/Rural")
