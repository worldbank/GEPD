...
main_indicator_labels<-c("4th Grade Student Proficiency", 
  ...
  tables_public_officials <- function(po_var) {
    ...
    sumstats_merged_df<-skim(sumstats_merged) %>%
      select(-level, -type, -value) %>%
      spread(stat, formatted) %>%
...
patch_scatter + plot_annotation(
  title = 'Scatterplot of Student 4th Grade Proficiency on Bureaucracy Indicator Scores by Region',
  caption = 'Regional average of all public officials in that departamento including officials at UGEL level is used for regional score'
)
