...
school_dta_indicators_pred<- completeData %>%
  ...
  select(student_knowledge, pred_bagged, pred_linear, pred_logit, everything())

#plot
pred_plot_dta <- school_dta_indicators_pred %>%
  select(student_knowledge, pred_bagged, pred_linear, pred_logit) %>%
  pivot_longer(
    cols=c('pred_bagged', 'pred_linear', 'pred_logit'), 
    names_to = "type",
    values_to="predictions"
  )

ggplot(pred_plot_dta, aes(y=student_knowledge, x=predictions, colour=type)) +
  geom_point() +
