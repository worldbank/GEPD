...
school_dta_indicators_pred<- completeData %>%
  ...
  select(student_knowledge, pred_neural, pred_bagged, pred_linear, pred_logit, pred_lasso, everything())

#plot
pred_plot_dta <- school_dta_indicators_pred %>%
  select(student_knowledge, pred_neural, pred_bagged, pred_linear, pred_logit, pred_lasso) %>%
  pivot_longer(
    cols=c('pred_neural' , 'pred_bagged', 'pred_linear', 'pred_logit', 'pred_lasso'), 
    names_to = "type",
