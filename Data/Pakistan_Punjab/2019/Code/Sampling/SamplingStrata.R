...
#########################################################################
# Focus on Public School Frame first
#########################################################################
...
#take a look at data
#number by school type
        # Numeric  Label
        # 0  
        # 1  ECE school with QS
        # 2  ECE school without QS
        # 3  School with katchi only

ggplot(df_public_final, aes(x=public_strata, fill='blue')) +
  geom_histogram( alpha=0.2, aes(y=..density..)) +
  geom_histogram(data=public_df, aes(y=..density..,x=public_strata, fill='red'), alpha=0.2) +
  theme_bw()


#urban/rural
ggplot(df_public_final, aes(x=school_location, fill='blue')) +
  geom_bar()

#Enrollment
ggplot(df_public_final, aes(x=enrollment, fill="blue")) +
  geom_density()+
  geom_density(data=public_df, aes(x=enrollment, fill="red")) +
  scale_fill_discrete(name='Data Set', labels=c('6 District Sample', 'Full Data')) +
  theme_bw()

#Enrollment 1st grade
ggplot(df_public_final, aes(x=total_cls1_enrollment, fill="blue")) +
  geom_density()+
  geom_density(data=public_df, aes(x=total_cls1_enrollment, fill="red")) +
  scale_fill_discrete(name='Data Set', labels=c('6 District Sample', 'Full Data')) +
  theme_bw()
...
domains<-df_public_final %>% group_by(governorate, supervisory_authority ) %>% summarise(n=n())
...
province_list<-as.vector(provinces$district)

#produce table with randomly replacement selected schools
sample_replacement1<- data_set_updated %>%
  left_join(provinces) %>%
  filter(district %in% province_list) %>%
  filter(sample_dashboard==0) %>%
  group_by(district) %>%
  sample_n(n_prov, weight=total_students_grade_4 ) %>%
