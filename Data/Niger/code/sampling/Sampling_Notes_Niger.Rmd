...
# Same for G3 and G4 (CE2 and CM1).
################################################################
...
df <- df_raw %>%
  mutate(
    ...
    total_students_grade_1=as.numeric(NB_G_CP) + as.numeric(NB_F_CP),
    total_students_grade_1_female= as.numeric(NB_F_CP),
    total_students_grade_1_male=as.numeric(NB_G_CP)    
    ) 
...
lire_matchable <- lire_df %>%
  left_join(df_filtered %>% select(REGION, DEPARTEMENT, COMMUNE, village, NOM_ETABLISSEMENT, CODE_ADMINISTRATIF,CODE_ETABLISSEMENT, total_students ))
...
```
### Merge to LIRE Schools

There is a World Bank project targeting schools (LIRE) schools that will be sampled from
...
sumstats_rand %>%
  kable( caption="Counts of Schools Sampled per Region and Urban/Rural Status") %>%
  kable_styling()
