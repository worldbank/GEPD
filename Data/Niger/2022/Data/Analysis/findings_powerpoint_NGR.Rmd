...
Roadmap:  
- Below is a set of tables and charts containing findings for the NGRnda 2022 Global Education Policy Dashboard survey. - We will start with breakdowns of our Practice indicators  
...
# School Survey
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - NGRnda 2022 - Urban/Rural",
...
#public/private
      ...
      sumstats_df <- sumstats_school_df_final %>%
        bind_rows(sumstats_public_officials_df %>%  mutate(ci_urban= as.character(ci_urban), ci_rural = as.character(ci_rural))) %>%
        arrange(factor(varlabel, levels=main_indicator_labels2))
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - NGRnda 2022 - Public/Private Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
...
#Zinder
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Niger 2022 - Region",
          ...
          escape=F) %>%
    kable_styling("striped") %>%
    add_header_above(c(" "=1, "Overall"=2,"maradi"=2,"tahoua"=2,"niamey"=2 ,"zinder"=2, "tillabery"=2))
...
#     plot_annotation(title = str_wrap('Differences in Dashboard Indicators Between Urban/Rural Schools',80))
      ...
      sch_ipw<-sumstats_school_$ipw

      sumstats_school_ <- sumstats_school_ %>%
        select(one_of(indicators_list))



      sumstats_school__df<-my_skim(sumstats_school_) %>%
        yank("numeric") %>%
        mutate(variable=skim_variable) %>%
        select(variable, mean, sd, p0, p25, p50, p75, p100, complete,  hist)
      ...
      #add variable label
      sumstats_school__df <- sumstats_school__df %>%
        mutate(name=variable,
               indicators=variable) %>%
        left_join(labels_df_2) %>%
        mutate(varlabel=indicator_labels) %>%
...
#tahoua
      ...
      sumstats_public_officials_df <- sumstats_public_officials_df %>%
        ...
        mutate(mean_urban=as.numeric(NA),
               ...
               ci_rural=as.numeric(NA),
               mean_maradi=as.numeric(NA),
               ci_maradi=as.numeric(NA),
               mean_tahoua=as.numeric(NA),
               ci_tahoua=as.numeric(NA)) %>%
        select(varlabel, mean, ci, mean_urban, ci_urban, mean_rural, ci_rural, mean_maradi, ci_maradi,  mean_tahoua, ci_tahoua )
  ...
  sumstats_df %>%
    ...
    # )
    # ) %>%
    kable(caption = "Summary Statistics of Dashboard Indicators - NGRnda 2022 - Region",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
          , "Mean", "95% Confident Interval", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"),
          digits = 1,
          escape=F) %>%
