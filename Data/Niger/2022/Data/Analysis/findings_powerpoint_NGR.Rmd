---
title: "Global Education Policy Dashboard - Niger 2022"
...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, fig.height=8, fig.width = 10)
#include packages
library(tidyverse)
library(haven)
library(kableExtra)
...
Roadmap:  
- Below is a set of tables and charts containing findings for the Niger 2022 Global Education Policy Dashboard survey. - We will start with breakdowns of our Practice indicators  
...
work_dir<- "C:/Users/wb577189/OneDrive - WBG/GEPD-Confidential/CNT/NGR/NGR_2022_GEPD/NGR_2022_GEPD_v01_RAW/Data/anonymized/"
...
## If student knowledge is absent, we can assume the school was not covered but replaced

school_dta_short_anon  <- school_dta_short_anon %>% group_by(hashed_school_code) %>%     summarise_all(coalesce_by_column) 

school_dta_short_anon[sapply(school_dta_short_anon, is.nan)] <- NA
...
# School Survey
    ...
    sumstats_school_urban <- school_dta_short_anon %>%
      filter(str_detect(urban_rural, "Urban"))
    ...
    #rural
      sumstats_school_rural <- school_dta_short_anon  %>%
        filter(urban_rural=="Rural") %>% filter(!is.na(student_attendance))
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Niger 2022 - Urban/Rural",
...
#public/private
      ...
      sumstats_school_nonprivate_df <- sumstats_school_nonprivate_df %>%
        ...
        mutate(ci_low=as.numeric(mean)-1.96*(as.numeric(sd)/sqrt(as.numeric(complete))),
               ci_high=as.numeric(mean)+1.96*(as.numeric(sd)/sqrt(as.numeric(complete)))) %>%
        mutate(ci=paste("[",round(ci_low,2),", ", round(ci_high,2),"]", sep="")) %>%
        mutate(mean_public=mean,
               ci_public=ci) %>%
        select(varlabel, mean_public, ci_public)
      ...
      sumstats_school_private_df <- sumstats_school_private_df %>%
        ...
        mutate(ci_low=as.numeric(mean)-1.96*(as.numeric(sd)/sqrt(as.numeric(complete))),
               ci_high=as.numeric(mean)+1.96*(as.numeric(sd)/sqrt(as.numeric(complete)))) %>%
        mutate(ci=paste("[",round(ci_low,2),", ", round(ci_high,2),"]", sep="")) %>%
        mutate(mean_private=mean,
               ci_private=ci) %>%
        select(varlabel, mean_private, ci_private)
      
      #now bind public/rural with the main results
      sumstats_school_df_final <- sumstats_school_df %>%
      ...
      #          ci_high=as.numeric(mean)+1.96*(as.numeric(sd)/sqrt(as.numeric(complete)))) %>%
      #   mutate(ci=paste("[",round(ci_low,2),", ", round(ci_high,2),"]", sep="")) %>%
      #   mutate(mean_public=as.numeric(NA),
      #          ci_public=as.numeric(NA),
      #          mean_private=as.numeric(NA),
      #          ci_private=as.numeric(NA)) %>%
      #   select(varlabel, mean, ci, mean_public, ci_public, mean_private, ci_private)
      # 
      # 
      # sumstats_df <- sumstats_school_df_final %>%
      #   bind_rows(sumstats_public_officials_df %>%  mutate(ci_public= as.character(ci_public), ci_private = as.character(ci_private))) %>%
      #   arrange(factor(varlabel, levels=main_indicator_labels2))
      ...
      sumstats_df <- sumstats_df %>%
        mutate(ratio=(as.numeric(mean_private))/as.numeric(mean_public))
      
  sumstats_df %>%
    mutate(ratio=cell_spec(
      round(ratio,2), background = spec_color(ratio,  alpha=0.4, option='cividis', direction=-1)
    )) %>%
    kable(caption = "Summary Statistics of Dashboard Indicators - Niger 2022 - Public/Private Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
    ...
    add_header_above(c(" "=1, "Overall"=2,"Public"=2,"Private"=2, " "=1))
      ...
      #
      sumstats_school_agadez <- school_dta_short_anon %>%
        filter(region=="AGADEZ")


      sch_ipw<-sumstats_school_df$ipw

      sumstats_school_agadez <- sumstats_school_agadez %>%
        select(one_of(indicators_list))



      sumstats_school_agadez_df<-my_skim(sumstats_school_agadez) %>%
        yank("numeric") %>%
        mutate(variable=skim_variable) %>%
        select(variable, mean, sd, p0, p25, p50, p75, p100, complete,  hist)
      ...
      #add variable label
      sumstats_school_agadez_df <- sumstats_school_agadez_df %>%
        mutate(name=variable,
               indicators=variable) %>%
        left_join(labels_df_2) %>%
        mutate(varlabel=indicator_labels) %>%
        mutate(ci_low=as.numeric(mean)-1.96*(as.numeric(sd)/sqrt(as.numeric(complete))),
               ci_high=as.numeric(mean)+1.96*(as.numeric(sd)/sqrt(as.numeric(complete)))) %>%
        mutate(ci=paste("[",round(ci_low,2),", ", round(ci_high,2),"]", sep="")) %>%
        mutate(mean_agadez=mean,
               ci_agadez=ci) %>%
        select(varlabel, mean_agadez, ci_agadez)

      #middle
      sumstats_school_dosso <- school_dta_short_anon  %>%
        filter(region=="DOSSO") 


      sch_ipw<-sumstats_school_dosso$ipw

      sumstats_school_dosso <- sumstats_school_dosso %>%
        select(one_of(indicators_list))%>% 
        filter(rowSums(is.na(.)) != ncol(.))


      sumstats_school_dosso_df<-my_skim(sumstats_school_dosso) #%>%
        yank("numeric") %>%
        mutate(variable=skim_variable) %>%
        select(variable, mean, sd, p0, p25, p50, p75, p100, complete,  hist)
      ...
      #add variable label
      sumstats_school_dosso_df <- sumstats_school_dosso_df %>%
        mutate(name=variable,
               indicators=variable) %>%
        left_join(labels_df_2) %>%
        mutate(varlabel=indicator_labels) %>%
        mutate(ci_low=as.numeric(mean)-1.96*(as.numeric(sd)/sqrt(as.numeric(complete))),
               ci_high=as.numeric(mean)+1.96*(as.numeric(sd)/sqrt(as.numeric(complete)))) %>%
        mutate(ci=paste("[",round(ci_low,2),", ", round(ci_high,2),"]", sep="")) %>%
        mutate(mean_dosso=mean,
               ci_dosso=ci) %>%
        select(varlabel, mean_dosso, ci_dosso)

      #maradi
      sumstats_school_maradi <- school_dta_short_anon  %>%
        filter(region=="MARADI")


      sch_ipw<-sumstats_school_maradi$ipw

      sumstats_school_maradi <- sumstats_school_maradi %>%
        select(one_of(indicators_list))


      sumstats_school_maradi_df<-my_skim(sumstats_school_maradi) %>%
        yank("numeric") %>%
        mutate(variable=skim_variable) %>%
        select(variable, mean, sd, p0, p25, p50, p75, p100, complete,  hist)
      ...
      #add variable label
      sumstats_school_maradi_df <- sumstats_school_maradi_df %>%
        mutate(name=variable,
               indicators=variable) %>%
        left_join(labels_df_2) %>%
        mutate(varlabel=indicator_labels) %>%
        mutate(ci_low=as.numeric(mean)-1.96*(as.numeric(sd)/sqrt(as.numeric(complete))),
               ci_high=as.numeric(mean)+1.96*(as.numeric(sd)/sqrt(as.numeric(complete)))) %>%
        mutate(ci=paste("[",round(ci_low,2),", ", round(ci_high,2),"]", sep="")) %>%
        mutate(mean_maradi=mean,
               ci_maradi=ci) %>%
        select(varlabel, mean_maradi, ci_maradi)

#tahoua
      sumstats_school_tahoua <- school_dta_short_anon  %>%
        filter(region=="TAHOUA")


      sch_ipw<-sumstats_school_tahoua$ipw

      sumstats_school_tahoua <- sumstats_school_tahoua %>%
        select(one_of(indicators_list))


      sumstats_school_tahoua_df<-my_skim(sumstats_school_tahoua) %>%
        yank("numeric") %>%
        mutate(variable=skim_variable) %>%
        select(variable, mean, sd, p0, p25, p50, p75, p100, complete,  hist)
      ...
      #add variable label
      sumstats_school_tahoua_df <- sumstats_school_tahoua_df %>%
        mutate(name=variable,
               indicators=variable) %>%
        left_join(labels_df_2) %>%
        mutate(varlabel=indicator_labels) %>%
        mutate(ci_low=as.numeric(mean)-1.96*(as.numeric(sd)/sqrt(as.numeric(complete))),
               ci_high=as.numeric(mean)+1.96*(as.numeric(sd)/sqrt(as.numeric(complete)))) %>%
        mutate(ci=paste("[",round(ci_low,2),", ", round(ci_high,2),"]", sep="")) %>%
        mutate(mean_tahoua=mean,
               ci_tahoua=ci) %>%
        select(varlabel, mean_tahoua, ci_tahoua)


#Nianmey
      sumstats_school_niamey <- school_dta_short_anon  %>%
        filter(region=="NIAMEY")


      sch_ipw<-sumstats_school_niamey$ipw

      sumstats_school_niamey <- sumstats_school_niamey %>%
        select(one_of(indicators_list))


      sumstats_school_niamey_df<-my_skim(sumstats_school_niamey) %>%
        yank("numeric") %>%
        mutate(variable=skim_variable) %>%
        select(variable, mean, sd, p0, p25, p50, p75, p100, complete,  hist)
      ...
      #add variable label
      sumstats_school_niamey_df <- sumstats_school_niamey_df %>%
        mutate(name=variable,
               indicators=variable) %>%
        left_join(labels_df_2) %>%
        mutate(varlabel=indicator_labels) %>%
        mutate(ci_low=as.numeric(mean)-1.96*(as.numeric(sd)/sqrt(as.numeric(complete))),
               ci_high=as.numeric(mean)+1.96*(as.numeric(sd)/sqrt(as.numeric(complete)))) %>%
        mutate(ci=paste("[",round(ci_low,2),", ", round(ci_high,2),"]", sep="")) %>%
        mutate(mean_niamey=mean,
               ci_niamey=ci) %>%
        select(varlabel, mean_niamey, ci_niamey)

#Tillabery
      sumstats_school_tillabery <- school_dta_short_anon  %>%
        filter(region=="TILLABERY")


      sch_ipw<-sumstats_school_tillabery$ipw

      sumstats_school_tillabery <- sumstats_school_tillabery %>%
        select(one_of(indicators_list))


      sumstats_school_tillabery_df<-my_skim(sumstats_school_tillabery) %>%
        yank("numeric") %>%
        mutate(variable=skim_variable) %>%
        select(variable, mean, sd, p0, p25, p50, p75, p100, complete,  hist)
      ...
      #add variable label
      sumstats_school_tillabery_df <- sumstats_school_tillabery_df %>%
        mutate(name=variable,
               indicators=variable) %>%
        left_join(labels_df_2) %>%
        mutate(varlabel=indicator_labels) %>%
        mutate(ci_low=as.numeric(mean)-1.96*(as.numeric(sd)/sqrt(as.numeric(complete))),
               ci_high=as.numeric(mean)+1.96*(as.numeric(sd)/sqrt(as.numeric(complete)))) %>%
        mutate(ci=paste("[",round(ci_low,2),", ", round(ci_high,2),"]", sep="")) %>%
        mutate(mean_tillabery=mean,
               ci_tillabery=ci) %>%
        select(varlabel, mean_tillabery, ci_tillabery)


#Zinder
      sumstats_school_zinder <- school_dta_short_anon  %>%
        filter(region=="ZINDER")


      sch_ipw<-sumstats_school_zinder$ipw

      sumstats_school_zinder <- sumstats_school_zinder %>%
        select(one_of(indicators_list))


      sumstats_school_zinder_df<-my_skim(sumstats_school_zinder) %>%
        yank("numeric") %>%
        mutate(variable=skim_variable) %>%
        select(variable, mean, sd, p0, p25, p50, p75, p100, complete,  hist)
      ...
      #add variable label
      sumstats_school_zinder_df <- sumstats_school_zinder_df %>%
        mutate(name=variable,
               indicators=variable) %>%
        left_join(labels_df_2) %>%
        mutate(varlabel=indicator_labels) %>%
        mutate(ci_low=as.numeric(mean)-1.96*(as.numeric(sd)/sqrt(as.numeric(complete))),
               ci_high=as.numeric(mean)+1.96*(as.numeric(sd)/sqrt(as.numeric(complete)))) %>%
        mutate(ci=paste("[",round(ci_low,2),", ", round(ci_high,2),"]", sep="")) %>%
        mutate(mean_zinder=mean,
               ci_zinder=ci) %>%
        select(varlabel, mean_zinder, ci_zinder)

      #now bind urban/rural with the main results
      sumstats_school_df_final <- sumstats_school_df %>%
        left_join(sumstats_school_agadez_df) %>%
        left_join(sumstats_school_dosso_df) %>%
        left_join(sumstats_school_maradi_df) %>%
        left_join(sumstats_school_tahoua_df)%>%
        left_join(sumstats_school_niamey_df)%>%
        left_join(sumstats_school_zinder_df)%>%
        left_join(sumstats_school_tillabery_df)



#NOTE - THIS NEEDS TO BE BROKEN DOWN BY REGION TO MATCH THE PREVIOUS ANALYSIS
      # ###Survey of Public Officials
      # metadata<-public_officials_metadata
      # 
      # #add function to produce weighted summary stats
      ...
      #          mean_rural=as.numeric(NA),
      #          ci_rural=as.character(NA),
      #          mean_maradi=as.numeric(NA),
      #          ci_maradi=as.character(NA),
      #          mean_tahoua=as.numeric(NA),
      #          ci_tahoua=as.character(NA)) %>%
      #   select(varlabel, mean, ci, mean_urban, ci_urban, mean_rural, ci_rural, mean_maradi, ci_maradi,  mean_tahoua, ci_tahoua )
      ...
      # sumstats_df <- sumstats_df %>%
      #   select(varlabel,  mean, ci, mean_urban, ci_urban, mean_rural, ci_rural, mean_maradi, ci_maradi, mean_tahoua, ci_tahoua, mean_)


  #create table
  sumstats_df %>%
    kable(caption = "Summary Statistics of Dashboard Indicators - Niger 2022 - Region",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
          , "Mean", "95% Confident Interval", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval",
          "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"),
          digits = 1,
          escape=F) %>%
    kable_styling("striped") %>%
    add_header_above(c(" "=1, "Overall"=2,"Agadez"=2, "Dosso"=2 ,"Maradi"=2,"Tahoua"=2,"Niamey"=2 ,"Zinder"=2, "Tillabery"=2))
...
learning_data_plot <- final_indicator_data_LERN_anon %>%
  ...
  select(-Female, -Male, -urban_rural, -ipw) %>%
  left_join(school_dta_short_anon %>% select(hashed_school_code, urban_rural)) %>% 
  left_join(key) %>% rename(ipw = g4_stud_weight_component) %>% 
  mutate(Type=urban_rural)


learning_dataMean <- summarise(group_by(learning_data_plot, Gender , Type), MD = round(wtd.mean(student_knowledge, ipw, na.rm=T),1))
...
- Regions (, dosso, maradi, tahoua) differ in terms of their service delivery and learning outcomes.  
...
```{r radar, echo=FALSE, warning=FALSE, eval=FALSE}
...
main_indicator_labels<-c( 
    ...
    expert_dir <- "//wbgfscifs01/GEDEDU/datalib-edu/projects/GEPD/CNT//NGR/NGR_2022_GEPD/NGR_2022_GEPD_v01_M/Data/Expert_Survey"
...
- Fraction of public officials reporting as motivated or more motivated: `r mean(public_officials_dta_clean_anon$motivation_relative_start>=100, na.rm=T)`


```{r overall, echo=FALSE}

public_officials_dta_clean_anon_clean <- public_officials_dta_clean_anon %>%
    filter(!is.na(govt_tier)) %>% 
  distinct(interview__id, interview__key, .keep_all=T)
...
data_plot <- public_officials_dta_clean_anon %>%
  ...
  mutate(govt_tier=case_when(
    govt_tier=='District office ' ~ 'Commune Office',
    govt_tier=='Regional office ' ~ 'Regional Office',
    govt_tier=='Ministry of Education '  ~ 'Ministry of Education'
  )) %>%
  group_by(govt_tier) %>%
...
#     plot_annotation(title = str_wrap('Differences in Dashboard Indicators Between Urban/Rural Schools',80))
      ...
      sch_ipw<-sumstats_school_agadez$ipw
...
#tahoua
  ...
  #   # ),
  #   # mean_maradi=cell_spec(
  #   #   round(mean_maradi,1), background = spec_color(mean_maradi/mean,  alpha=0.4, option='cividis', direction=-1)
  #   # )
  #   # ) %>%
  #   kable(caption = "Summary Statistics of Dashboard Indicators - Niger 2022 - Region",
  #         col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
  ...
  #         escape=F) %>%
  #   kable_styling("striped") %>%
  #   add_header_above(c(" "=1, "Overall"=2,""=2,"dosso"=2,"maradi"=2 ,"tahoua"=2))
