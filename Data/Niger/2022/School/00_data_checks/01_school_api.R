...
if (file.exists(pw_file)) {
  ...
  workspace <- rstudioapi::askForPassword(prompt = 'Please enter the workspace:')
}
...
q<-GET(paste(server_add,"/api/v1/workspaces", sep=""),
...
POST(paste(server_add,"/api/v2/export/stata/26db9325-f5b2-4098-bc81-191b554874b8$",quest_version,"/start", sep=""),
         authenticate(user, password))


POST(paste(server_add, "/api/v1/questionnaires", sep = ""))
#sleep for 10 seconds to wait for stata file to compile
Sys.sleep(10)

dataDownload <- GET(paste(server_add,"/api/v6/export/stata/15a17cf7-4090-48a3-b8ab-ecc9e4a951b9$", quest_version,"/",sep=""),
                    authenticate(user, password))
