...
school_dta<- school_dta %>%
  ...
  select(-starts_with("enumerators_preload")) %>% #bring in school codes that were created in separate stata program
    left_join((read_csv(file.path(download_folder, "emis_fixes.csv")) %>% select(-m1s0q2_emis))) %>%
...
#############################################
  ...
  school_data_SATT <- school_data_SATT %>%
    mutate(principal_satisfaction=attitude_fun_rev(m7shq1_satt),
           principal_salary=if_else(m7shq2_satt==999, as.numeric(NA),12*m7shq2_satt/1727.11	)) %>%
    mutate(
