...
work_dir <- "C:/Users/wb577189/OneDrive - WBG/GEPD-Confidential/CNT/SLE/SLE_2022_GEPD/SLE_2022_GEPD_v01_Analysis/Data/anonymized/"
...
learning_dataMedian <- summarise(group_by(learning_data_plot2, Gender), MD = round(wtd.mean(literacy_student_knowledge, ipw, na.rm=T),1))
  ...
  ggplot(learning_data_plot2, aes(x=Type, y=literacy_student_knowledge, fill=Gender)) + #now plot the output in a bar graph
    ...
    geom_text(data = learning_dataMedian, aes(x=Type, y=MD, label=MD), 
              position = position_dodge(width = 0.9), size = 5, vjust = -1) +
    geom_point(data = learning_dataMedian, aes(x=Type, y=MD),
               position = position_dodge(width = 0.9)) +
  ggtitle(str_wrap("4th Grade Literacy Student Assessment Scores by Gender and Whether Tested in Kinyarwanda",100)) +
...
ggplot(learning_dataMedian, aes(x=Type, y=MD, fill=Gender)) + #now plot the output in a bar graph
    geom_bar(stat='identity', position="dodge") +
    geom_text(data = learning_dataMedian, aes(x=Type, y=MD, label=MD), 
              position = position_dodge(width = 0.9), size = 5, vjust = -1) +
    geom_point(data = learning_dataMedian, aes(x=Type, y=MD),
               position = position_dodge(width = 0.9)) +
  ggtitle(str_wrap("4th Grade Literacy Assessment Scores by Gender and Whether Tested in Kinyarwanda",100)) +
  xlab(str_wrap("Language Tested", width=75))+
  ylab(str_wrap("Fraction Correcton on 4th Grade Literacy Assessment",35)) +
  scale_fill_discrete(name="Gender", 
                      labels=c('All Students','Girls','Boys' )) +
...
learning_dataMedian <- summarise(group_by(learning_data_plot2, Gender , Type), MD = round(wtd.mean(math_student_knowledge, ipw, na.rm=T),1))
...
ggplot(learning_dataMedian, aes(x=Type, y=MD, fill=Gender)) + #now plot the output in a bar graph
    ...
    geom_point(data = learning_dataMedian, aes(x=Type, y=MD),
               position = position_dodge(width = 0.9)) +
  ggtitle(str_wrap("4th Grade Math Assessment Scores by Gender and Whether Tested in Kinyarwanda",100)) +
  xlab(str_wrap("Language Tested", width=75))+
  ylab(str_wrap("Fraction Correcton on 4th Grade Math Assessment",35)) +
  scale_fill_discrete(name="Gender", 
                      labels=c('All Students','Girls','Boys' )) +
  coord_flip() +
  expand_limits(y=c(0,50)) + 
...
urban_rural <- school_dta_short_anon %>%
  rename(rural = urban_rural) %>% 
  mutate(Type=factor(rural, levels=c("Rural", "Urban"), labels = c("Rural", "Urban"))) %>%
  filter(!is.na(rural))
...
data_plot <- public_officials_dta_clean_anon %>%
  ...
  mutate(govt_tier=case_when(
    ...
    govt_tier=='Ministry of Education '  ~ 'MBSSE'
  )) %>%
  group_by(govt_tier) %>%
...
dataMedian <- summarise(group_by(data_plot, indicators , govt_tier), MD = round(median(indicator_values, na.rm=T),1))
  ...
  ggplot(data_plot, aes(x=indicators, y=indicator_values, fill=govt_tier)) + #now plot the output in a bar graph
    ...
    scale_x_discrete(labels = str_wrap(c("Impartial Decision Making",
                                         ...
                                         "Characteristics of Bureaucracy"), width = 25)) + 
