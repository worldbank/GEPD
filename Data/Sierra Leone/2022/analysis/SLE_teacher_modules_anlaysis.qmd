...
#add weights and anonymize
teacher_pedagogy_micro_g2 <- df_add_weights('teacher_pedagogy_micro_g2') %>%
  select(-starts_with('interview__')) %>%
...
teacher_combined_df %>%
  select(-...4) %>%
  write_dta(path=paste0(confidential_folder,"/teacher_combined_modules.dta"))

matches %>%
  write_dta(path=paste0(confidential_folder,"/teacher_modules_key.dta"))

school_micro %>%
  write_dta(path=paste0(confidential_folder,"/school_modules.dta"))
```
...
Start with teacher pedagogy

```{r}
frame_matches <- matches %>%
  filter(!is.na(m2saq2))

pedagogy_matches <- teacher_pedagogy_micro %>%
  left_join(frame_matches ) %>%
  mutate(matched=if_else(is.na(m2saq2), "Not matched", "Matched")) %>%
  select(school_code, TEACHERS__id, m2saq2,m4saq1_number, m4saq1, matched)
...
pedagogy_matches_g2 <- teacher_pedagogy_micro_g2 %>%
  left_join(frame_matches ) %>%
  mutate(matched=if_else(is.na(m2saq2), "Not matched", "Matched")) %>%
  select(school_code, TEACHERS__id, m2saq2,m4saq1_number_g2, m4saq1_g2, matched)
