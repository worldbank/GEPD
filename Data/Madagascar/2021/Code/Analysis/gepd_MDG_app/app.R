                              ...
                              mainPanel(
                                ...
                                tabsetPanel(type = "tabs",
                                            ...
                                            tabPanel("Graphique de distribution", value=1, 
                                                     selectInput("stud_level", "Utiliser les données au niveau des élevé ?",
                                                                 choices=c('Non', 'Oui'),
                                                                 selected='Non'),
                                   ...
                                   d'une simple régression OLS bivariée du premier indicateur sur le second."),
                                                     ...
                                                     selectizeInput("reg_choices", "Choisissez la variable de résultat pour les régression : (Par défaut : Apprentissage en 4ème année)", 
                                                                    choices=NULL)   ,
                                            ...
                                            tabPanel("Outil de régression multivariable", value=7, 
                                                     p("Cet outil permet à l'utilisateur de produire des tableaux de régression MCO multivariés en sélectionnant une variable de résultat ainsi que l'ensemble des variables X dans la régression. Cet outil peut donner certaines informations sur la persistance des relations entre les variables après le contrôle d'autres covariables. Ces estimations, même si elles contrôlent certaines autres covariables, ne sont pas nécessairement causales. Les variables X comprennent nos indicateurs du tableau de bord, ainsi que l'option d'inclure le statut rural de l'école, ainsi que le PIB par kilomètre carré dans un rayon d'un kilomètre carré autour de l'école. Les données relatives au PIB par kilomètre carré sont produites par le personnel de la Banque mondiale, à l'origine pour le Rapport d'évaluation globale de la réduction des risques (GAR) pour l'année 2010. Ces données sont disponibles sur le site https://datacatalog.worldbank.org/dataset/gross-domestic-product-2010. En outre, l'utilisateur a la possibilité d'inclure des effets fixes régionaux. Dans le cas de Madagascar, il s'agit des effets fixes de la région de Madagascar.", 
                                                                    choices=NULL)   ,
                                                     ...
                                                     selectizeInput("province_dummies", "Inclure les effets fixes de province dans la régression?", 
                                                                    choices=c("Non", "Oui"),
                                                                    selected="Non")   ,
                                                     selectizeInput("imputed", "Utiliser un ensemble de données qui utilise l'imputation pour les valeurs manquantes, plut?t qu'un ensemble de données brutes ?", 
                                                                    choices=c("Non", "Oui"),
                                                                    selected="Non")   ,
                                            ...
                                            tabPanel("Analyse de régression des sous-indicateurs", value=6,
                                                     p("Cet outil permet à l'utilisateur de produire des tableaux de régression MCO multivariés en sélectionnant une variable de résultat ainsi que l'ensemble des sous-indicateurs de la régression. L'objectif est de fournir des informations sur la manière dont chaque sous-indicateur est lié à certains résultats."),
                                                     selectizeInput("sub_reg_choices", "Choisissez la variable de résultat pour les régression : (Par défaut : Apprentissage en 4ème année)", 
                                                                    choices=NULL)   ,
                                            ...
                                            tabPanel("Analyse factorielle des sous-indicateurs", value=4, 
                                                     ...
                                                     p( "Dans cette analyse, nous utilisons l'analyse factorielle pour produire un facteur unique, qui maximise la variation expliquée dans nos sous-indicateurs. Ce facteur est ensuite comparé à notre indicateur pour vérifier si la notation basée sur notre approche ou sur l'analyse factorielle produit des résultats similaires."
                                                     ),
