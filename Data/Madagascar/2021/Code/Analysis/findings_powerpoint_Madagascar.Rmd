---
title: "Global Education Policy Dashboard - Madagascar 2021"
...
## Packages
  ...
  pacman::p_load(
    ...
    tidyverse, haven, BiocManager, kebaleExtra, skimr, flextable, ggridges, scales, spatstat, ggradar, Hmisc, ggpmisc, ggalt, waffle, patchwork, DT, ggplot2, hrbrthemes
...
Roadmap:  
- Below is a set of tables and charts containing findings for the Madagascar 2021 Global Education Policy Dashboard survey. - We will start with breakdowns of our Practice indicators  
...
#set directory to bring in data
  if(Sys.info()["user"] == "wb577189"){
    ...
    work_dir<- "C:/Users/wb577189/OneDrive - WBG/GEPD-Confidential/CNT/MDG/MDG_2021_GEPD/MDG_2021_GEPD_v01_RAW/Data/anonymized/"


    
  } else if (str_to_lower(Sys.info()["user"]) == "wb469649") {
    
    work_dir<- "C:/Users/wb469649/WBG/HEDGE Files - HEDGE Documents/GEPD-Confidential/CNT/MDG/MDG_2021_GEPD/MDG_2021_GEPD_v01_RAW/Data/anonymized/"
...
# School Survey
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Madagascar 2021 - Urban/Rural",
...
#public/private
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Madagascar 2021 - Public/Private Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
...
```{r item_stats}
# Calculate share of each question answered correctly
  items <- c('m8sbq3a_arithmetic',
             'm8sbq3f_arithmetic',
             'm8sbq3j_arithmetic',
             'm8sbq3g_arithmetic',
             'm8saq2_id',
  ...
  item_desc <- c('Student correctly answers 8+7',
                 'Student correctly answers 7x8',
                 'Student correctly answers 75/5',
                 'Student correctly answers 37x40',
                 'Average fraction of letters recognized',
                 'Average fraction of words recognized',
                 'Student correctly answers question about short text'
                 )
  
 item_meta_df <- data.frame(item=items, desc=item_desc)  
 
g4_weights <-  school_dta_short_anon %>%
  select(hashed_school_code, g4_stud_weight_component)

item_df <- assess_4th_grade_anon_anon %>%
  select(hashed_school_code, items, ipw) 

weight <- item_df$ipw

item_df_stats <- item_df %>%
  summarise(across(items, wtd.mean, weights=ipw, .names='{.col}_mean'),
            across(items, wtd.quantile, probs=0.5, weights=ipw, .names='{.col}_median')) %>%
  pivot_longer(
    cols=everything(),
    names_to='item',
    values_to='score'
  ) %>%
  mutate(type=if_else(grepl('mean',item),"Mean", "Median"),
         item=str_remove(item,"_mean"),
         item=str_remove(item,"_median")) %>%
  pivot_wider(
    names_from=type,
    values_from=score
  )
  
items_df_tab <-  item_df_stats %>%
  left_join(item_meta_df) %>%
  select(item, desc, everything())

flextable(items_df_tab) %>%
  autofit()
...
```{r waffle, fig.height=7, fig.width=7}

# 2nd visit

absence_plt <- teacher_absence_final_anon %>%
  mutate(absence_1st_cat = case_when(
    s_m2ab1a_01==5 ~ 'Absent from School',
    s_m2ab1a_01==3 ~ 'Absent from Classroom',
    TRUE ~ 'Present'
                ),
    absence_2nd_cat = case_when(
    s_m2ab1b_01==5 ~ 'Absent from School',
    s_m2ab1b_01==3 ~ 'Absent from Classroom',
    TRUE ~ 'Present'
                )
    )
...
## AC - alternative because the lines above do not work for me
#install.packages("waffle", repos = "https://cinc.rud.is")
  ...
  theme(strip.text.x = element_text(hjust = 0.5),
        legend.position = 'bottom') +
  ggtitle("Teacher Presense in 2nd visit") +
...
#         legend.position = 'bottom'
#         ) +
#   ggtitle("Teacher Presence in 2nd visit") +
#   labs(caption='One tile = One teacher',
#        fill='Teacher Status') 
...
absence_plt1 <- absence_plt %>%
  mutate(absence_1st_fact=factor(absence_1st_cat, levels=c('Present','Absent from School', 'Absent from Classroom'))) %>%
  arrange(absence_1st_fact)

absence_perc1 <- absence_plt1 %>%
    ungroup() %>% 
    select(-s_ecole_code) %>% 
    count(urban_rural, absence_1st_fact) %>% 
    arrange(desc(n)) %>%
    group_by(absence_1st_fact) %>% 
    mutate(cumulative = sum(n)) %>% 
    ungroup() %>% 
    mutate(Group = factor(absence_1st_fact, levels = c("Present", "Absent from School", "Absent from Classroom")),
         midpoint = cumulative - n / 2,
         label = paste0(Group, " (", round(cumulative / sum(n) * 100, 1), "%)"))
...
ggplot(absence_perc1, aes(fill=label, values=n)) +
  facet_wrap(~urban_rural, nrow = 1, strip.position = "bottom") +
    geom_waffle(
    n_rows = 32, color = "white", flip = TRUE
  ) +
  ...
  theme(strip.text.x = element_text(hjust = 0.5),
        legend.position = 'bottom') +
  ggtitle("Teacher Presense in 1st visit") +
  labs(caption='One tile = One teacher',
       fill='Teacher Status') +
  theme_minimal(base_family = "Roboto Condensed") +
  theme(panel.grid = element_blank(), axis.ticks.y = element_line()) +
...
inpt_list<-c('blackboard_functional', 'share_pencil', 'share_exbook','share_textbook',   'used_ict', 'access_ict')

input_plt <- final_indicator_data_INPT_anon %>%
  select(hashed_school_code, inpt_list, ipw) %>%
  pivot_longer(
    cols=inpt_list,
    names_to='indicator',
    values_to='values'
  ...
  mutate(indicator=case_when(
    indicator=='blackboard_functional' ~ "Functional Blackboard", 
    indicator=='share_pencil' ~ "Share with Pencils", 
    indicator=='share_exbook' ~ "Share with Exercise Books",
    indicator=='share_textbook' ~ "Share with Textbook",   
    indicator=='used_ict' ~ "Used ICT", 
    indicator=='access_ict' ~ "Access to ICT"
  )) %>%
  mutate(indicator=factor(indicator, levels=c("Functional Blackboard","Share with Pencils", "Share with Exercise Books", "Share with Textbook", "Used ICT", "Access to ICT" )))


wgt_mean_input_plt <- input_plt %>%
  group_by(indicator) %>%
  summarise(values=weighted.mean(values, weights=ipw))

  ggplot(data=wgt_mean_input_plt, aes(x=indicator, y=values, fill='#ff0000')) + #now plot the output in a bar graph
    geom_col() +
    # scale_x_discrete(labels = str_wrap(indicator_labels, width = 25)) + 
  xlab(str_wrap("Inputs", width=75))+
  scale_y_continuous(labels=scales::percent) +
  ylab("Average") +
  ggtitle(str_wrap("Average Availability of Inputs Across Schools", width=75)) +
  theme_bw() +
...
```
```{r infra_plot, echo=FALSE, fig_height=8, fig_width=9}
 #############################
    ...
    ##############################
      ...
      regplots<- ggplot(data=df_reg, aes(x=infrastructure, y=student_knowledge, weight=ipw, color="#0081a8")) +
        geom_point() +
        ...
        expand_limits(x = c(0,5), y = c(0,100)) +
        labs(colour = "Indicator") +
        ylab(paste0("4th Grade Score")) +
        xlab(paste0("Infrastructure Score")) +
      ...
      regplots +
          plot_annotation(title = str_wrap("Scatterplot of School Average 4th Grade Student Scores against School Infrastructure Scores",90))

      

      
   mod <- lm(student_knowledge~infrastructure, data=df_reg, weights = ipw)  
...
```{r 1st_grade_plot, echo=FALSE, fig_height=8, fig_width=9}
 #############################
   ...
   mod <- lm(student_knowledge~ecd_student_knowledge, data=df_reg, weights = ipw)  
...
```{r content_knowledge_region, echo=FALSE, warning=FALSE, fig.height=6, fig.width=10}
...
region_content_plot <- region_content %>%
  ...
  ggtitle("Teacher Content Knowledge Across Urban/Rural")
  region_content_plot
...
p1
  ...
  p1<-ggplot(contMean, aes(x=Assessment, y= MD, fill=Type)) + #now plot the output in a bar graph
    geom_col( position="dodge") +
    geom_text(data = contMean, aes(x=Assessment, y=MD, label=paste0(MD,"%")), 
              position = position_dodge(width = 0.8), size = 5, hjust = -1) +
    # geom_text(data = contMean, aes(x=Assessment, y=25, label=Type), 
    #           position = position_dodge(width = 0.8), size = 5, hjust = 0) +
  xlab(str_wrap("Subject", width=75))+
  ylab(str_wrap("Teacher Content Knowledge Proficiency",35)) +
  scale_fill_discrete(name="Urban/Rural") +
...
```{r intmot_plot, echo=FALSE, fig_height=8, fig_width=9}
 #############################
    # Create database with just learning outcomes for regressions
    ##############################
      ...
      regplots<- ggplot(data=df_reg, aes(x=intrinsic_motivation, y=student_knowledge, weight=ipw, color="#0081a8")) +
        geom_point() +
        geom_smooth(method='lm') +
        scale_color_manual(labels = c( "Principal Indicator "),  values= c( "#0081a8")) +
        theme_bw() +
        ...
        labs(colour = "Indicator") +
        ylab(paste0("4th Grade Score")) +
        xlab(paste0("Teacher Intrinsic Motivation Score")) +
        stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
                     label.x.npc = "right", label.y.npc = 0.2,
                     formula = 'y~x', parse = TRUE, size = 5)
      ...
      regplots +
          plot_annotation(title = str_wrap("Scatterplot of School Average 4th Grade Student Scores against Teacher Intrinsic Motivation Scores",90))

      

      
   mod <- lm(student_knowledge~intrinsic_motivation, data=df_reg, weights = ipw)  
...
textbooks <- final_indicator_data_PKNW_anon %>%
  ...
  mutate(share_textbook_guess=(m7sfq10_pknw)/(s_m4ab_01)) %>%
  mutate(gap_textbooks=(m7sfq10_pknw-s_m4ab_04)) %>%
  mutate(Type=factor(urban_rural.x, levels=c("Rural", "Urban"), labels = c("Rural", "Urban"))) %>%
  filter(!is.na(Type)) %>%
  filter(abs(gap_textbooks)<80) #drop some outlier observations that are probably miscodings



ggplot(data=textbooks, aes(x=s_m4ab_04, y=m7sfq10_pknw, color=Type)) +
  geom_point() +
  annotate("text", label=wrapper(paste("Principals over-estimate by ", round(mean(textbooks$gap_textbooks, na.rm=T),1)," pupils on average"),15), x = 30, y = 20) +
  geom_line(aes(x=s_m4ab_04, y=s_m4ab_04, color="45 Degree Line")) +
  theme_bw() +
...
main_indicator_labels<-c( 
  ...
  expert_dir<- "C:/Users/wb577189/OneDrive - WBG/My files/Dashboard (Team Folder)/Country_Work/Madagascar/2021/Policy Survey Data"
    ...
    expert_dta_radar <- expert_dta_final %>%
                  rename(
      sch_monitoring= principal_monitor_criteria, 
       standards_monitoring=inputs_standards
...
```
```{r radar2, eval=FALSE, warning=FALSE, include=FALSE}
...
```{r discuss_results, eval=FALSE, include=FALSE}
...
teacher_discuss_df <- teacher_questionnaire_dta_anon %>%
  zap_labels() %>%
...
```{r overall, eval=FALSE, include=FALSE}
...
data_plot <- public_officials_dta_clean_anon %>%
  ...
  mutate(govt_tier=case_when(
    ...
    govt_tier=='Ministry of Education '  ~ 'MINEDUC'
  )) %>%
  group_by(govt_tier) %>%
