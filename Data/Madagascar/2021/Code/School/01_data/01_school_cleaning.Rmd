...
```{r tatt}
 ...
 hist(teacher_questionnaire_dta$teacher_attraction)    
  ...
  final_indicator_data_TATT <- teacher_questionnaire_dta %>%
    ...
    group_by(s_ecole_code) %>%
    select(s_ecole_code, teacher_attraction,teacher_satisfied_job,teacher_satisfied_status,better_teachers_promoted,teacher_bonus,salary_delays) %>%
    summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
 ...
 hist(teacher_questionnaire_dta$teacher_support) 
  ...
  final_indicator_data_TSUP <- teacher_questionnaire_dta %>%
    ...
    select(s_ecole_code, teacher_support,pre_service,practicum,in_service,opportunities_teachers_share,
           ...
           in_service_exists,in_servce_lngth,in_service_classroom) %>%
    summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.)))  
...
#plot the indicator  
 ...
 hist(teacher_questionnaire_dta$miss_class_admin)
  ...
  final_indicator_data_TMNA <- teacher_questionnaire_dta %>%
    ...
    group_by(s_ecole_code) %>% 
    select(s_ecole_code,teacher_monitoring,attendance_evaluated,attendance_rewarded,attendence_sanctions,miss_class_admin ) %>%
    summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 
...
#plot the indicator  
 ...
 hist(teacher_questionnaire_dta$d_m2bd_02)
  ...
  final_indicator_data_TINM <- teacher_questionnaire_dta %>%
    ...
    group_by(s_ecole_code) %>%
    summarise_all( ~(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.)))  %>%
    select(s_ecole_code, intrinsic_motivation,acceptable_absent,students_deserve_attention,motivation_teaching,d_m2bd_02)
...
```{r SATT}
  ...
  final_indicator_data_SATT <- school_data_SATT %>%
    ...
    select( -starts_with('interview'), -starts_with('enumerator'))  %>%
    select(s_ecole_code, sch_management_attraction,principal_satisfaction,principal_salary_score )
...
```{r SSUP}
  ...
  final_indicator_data_SSUP <- school_data_SSUP %>%
    ...
    select(s_ecole_code, sch_support,prinicipal_trained,principal_training,principal_used_skills,principal_offered) %>%
    select( -starts_with('interview'), -starts_with('enumerator'))  
