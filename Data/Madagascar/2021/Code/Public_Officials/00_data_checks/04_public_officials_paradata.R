...
# POST(paste(server_add,"/api/v1/export/paradata/dd5d031dba1e4333b4ff5dfc7bff4e2d$",quest_version,"/start", approval, sep=""),
#      authenticate(user, password))
# 
# #sleep for 10 seconds to wait for stata file to compile
# Sys.sleep(10)
# 
# dataDownload <- GET(paste(server_add,"/api/v1/export/paradata/dd5d031dba1e4333b4ff5dfc7bff4e2d$", quest_version,"/",approval,sep=""),
#                     authenticate(user, password))
# 
...
para_df<-read.delim(paste(download_folder, "paradata.tab", sep="/"), sep="\t") %>%
  rename(interview__id=ï..interview__id)
...
var.labels=c(
  ...
  tz_offset = "Timezone offset relative to UTC",
...
# 
# para_df_tab <- para_df %>%
#   select( responsible, date, module, section, question, varlabel, timelength_sec, ?..interview__id,)
# 
...
# 
# para_df_tab <- para_df %>%
#   select( responsible, date, timestamp, module, section, question, varlabel, timelength_sec, ?..interview__id,)
# 
# linked_df<-SharedData$new(para_df_tab)
# 
# 
