...
office_list <- read_csv("//wbgfscifs01/GEDEDU/datalib-edu/projects/GEPD-Confidential/CNT/RWA/RWA_2020_GEPD/RWA_2020_GEPD_v01_RAW/Data/sampling/district_sample_2019-08-26.csv")
...
region_regex<-c('North', 'South', 'East','West')

maps <- fortify(fc_Rwanda, region="ADM1_NAME") %>%
  mutate(id=str_trim(str_split(id,pattern="//",1)))



#collapse to governorate level for mapping
po_df_collapse <- public_officials_agg %>%
  left_join(office_list,  by=c('school_district_preload'='sector')) %>%
...
merge_df_collapse<-school_dta_matched %>%
  ...
  select(governorate,student_knowledge, mandates_accountability, quality_bureaucracy, national_learning_goals, impartial_decision_making , total_enrolled) %>%
...
merge_df_collapse<-school_dta_matched %>%
  group_by(governorate) %>%
  select(governorate,ecd_student_knowledge, mandates_accountability, quality_bureaucracy, national_learning_goals, impartial_decision_making , total_enrolled) %>%
  summarise_all(list(~if(is.numeric(.)) wtd.mean(.,w=total_enrolled, na.rm = TRUE) else first(.)))
...
merge_df_collapse<-school_dta_matched %>%
  group_by(governorate) %>%
  select(governorate,presence_rate, mandates_accountability, quality_bureaucracy, national_learning_goals, impartial_decision_making , total_enrolled) %>%
  summarise_all(list(~if(is.numeric(.)) wtd.mean(.,w=total_enrolled, na.rm = TRUE) else first(.)))
