...
public_officials_agg <- public_officials_agg %>%
  ...
  # )) %>%
  filter(!is.na(school_province_preload)) %>%
  mutate(
  ...
  group_by(school_province_preload) %>%
...
school_dta_matched <- school_dta_short %>%
  filter(school_province_preload %in% office_list$province) %>%
...
public_offical_link_key <- public_officials_agg %>%
  select(office_preload, school_province_preload)
...
po_df_collapse <- public_officials_agg %>%
  mutate(province=if_else(govt_tier!='Regional office (or equivalent)',
                          province,school_province_preload)) %>%
...
school_df_collapse <- school_dta_matched %>%
  mutate(n_interviews=n()) %>%
  left_join(auxillary_frame_info,by=c('school_province_preload'='province')) %>%
  mutate(id=region) %>%
  mutate(id=if_else(id=="Kigali city", "Kigali City", id)) %>%
  group_by(id) %>%
