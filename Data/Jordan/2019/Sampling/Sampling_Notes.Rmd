...
map2 <- function(df_input) {    leaflet(df_input) %>% 
    ...
    addCircleMarkers( lng=~longitude,  lat=~latitude, color=~pal(sample_text),
                popup =  paste("School Code: ", sample_map$organization_code, " <br>",
                               "School Name: ", sample_map$school_name, " <br>",
                               "Replacement for School (code):", sample_map$replace_school_code_2, " <br>",
                               "Replacement for School (name):", sample_map$replace_school_name_2, " <br>",
...
bscols(widths=c(3,NA),
  list(
  filter_select("school", "School Code", linked_df_orphan, ~replace_school_code),
