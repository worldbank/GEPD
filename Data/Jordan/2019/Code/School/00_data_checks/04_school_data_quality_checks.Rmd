...
school_dta_short <- school_dta_short %>%
  ...
  mutate(school_ipw=if_else(is.na(school_ipw), 1, school_ipw)*total_students_grade_4)
...
knitr::kable(sumstats_df, caption="Summary Statistics of Key Indicator Variables" , col.names = c("Indicator", "Mean", "95% Confidence Interval", "Std Dev","Min", "25th Percentile", "Median", "75th Percentile", "Max", "# Complete Cases", "# Missing Cases", "Histogram"), digits = 2 ) %>%
...
teacher_questionnaire_list<-teacher_questionnaire %>%
  select(interview__key, school_code, teacher_name, teacher_number, m3sb_troster, m3sb_tnumber) %>%
...
orphans_number_questionnaire <- teacher_questionnaire_list %>%
  ...
  bind_rows(teacher_roster_list) %>%
  arrange(school_code) %>%
  distinct(school_code, teacher_name, teacher_number, .keep_all = T)

non_orphans_number_questionnaire <- teacher_questionnaire_list  %>%
  left_join(teacher_roster_list) %>%
  select(school_code, teacher_name, teacher_number,  everything()) %>%
  bind_rows(teacher_roster_list) %>%
  arrange(school_code) %>%
...
write_excel_csv(orphans_number_questionnaire, path = paste(save_folder, "jordan_orphan_teachers.csv", sep="/")) 
write_excel_csv(non_orphans_number_questionnaire, path = paste(save_folder, "jordan_non_orphan_teachers_merged.csv", sep="/")) 
...
## Test of Differences Between Sampling Information in Sampling Frame and Data Collected

In the following, we test for differences in the number of students listed in the school based on EMIS data compared to the number of students reported by the principal when the enumerator visited.  Significant differences could affect the validity of our results, as survey weights are based on the EMIS data.  We plot the two numbers against one another, and also check for a relationship between the discrepancies and our dashboard indicators. 

```{r sampling_frame_test}

frame_test <- school_dta_short %>%
  mutate(total_enrollment_emis=total_male_students+total_female_students) %>%
  mutate(total_enrollment_gap=total_enrolled-total_enrollment_emis)

covariates<-indicator_list

my_formula <- as.formula(paste('total_enrollment_gap ~ ', paste(covariates, collapse=" + "), sep=""))
multi_reg<-lm(my_formula, frame_test)   
# Adjust standard errors
cov1_multi         <- sandwich::vcovHC(multi_reg, type = "HC1")
robust_multi_se    <- sqrt(diag(cov1_multi))



ggplot(frame_test, aes(x=total_enrollment_emis, y=total_enrolled)) +
  geom_point() +
  geom_smooth() +
  xlab("# of Students based on EMIS") +
  ylab("# of Students based on Survey") +
  ggtitle(str_wrap('Plot of # of Students based on EMIS data on # of Students based on Survey Data for Dashboard Schools',65)) +
  theme_bw() 
  

```

```{r reg_results, results='asis', echo=FALSE}
stargazer( multi_reg, type = "html",
           se        = list(robust_multi_se),
           title = "Regression of Total Enrollment as Collected during School Survey Versus EMIS data",
           style='aer',
           notes= c(
