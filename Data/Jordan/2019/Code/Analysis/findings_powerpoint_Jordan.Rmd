...
#     plot_annotation(title = str_wrap('Differences in Dashboard Indicators Between Urban/Rural Schools',80))
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Jordan 2019 - Evening Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
          , "Mean", "95% Confident Interval", "Ratio of Non-Evening to Evening"),
          digits = 1, 
          escape=F) %>%
    kable_styling("striped") %>%
    add_header_above(c(" "=1, "Overall"=2,"Non-Evening"=2,"Evening"=2, " "=1))
  ...
  sumstats_df %>%
    ...
    # )
    # ) %>%
    kable(caption = "Summary Statistics of Dashboard Indicators - Jordan 2019 - Region",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
...
- Next we offer breakdowns by whether or not the school survey took place at an Evening School
...
learning_data_plot2 <- final_indicator_data_LERN_anon %>%
  ...
  select(-Female, -Male) %>%
  mutate(Type=if_else(foundation_period=="Evening", "Evening School", "Non-Evening School"))
...
learning_dataMedian <- summarise(group_by(learning_data_plot2, Gender , Type), MD = round(weighted.median(student_knowledge, ipw, na.rm=T),1))
  ...
  ggplot(learning_data_plot2, aes(x=Type, y=student_knowledge, fill=Gender)) + #now plot the output in a bar graph
    ...
    geom_text(data = learning_dataMedian, aes(x=Type, y=MD, label=MD), 
              position = position_dodge(width = 0.8), size = 3, vjust = 1) +
  ggtitle("4th Grade Student Assessment Scores by Gender and Evening School Status") +
  xlab(str_wrap("Evening School Status", width=75))+
  ylab(str_wrap("Fraction Correcton on 4th Grade Assessment",35)) +
  scale_fill_discrete(name="Gender", 
                      labels=c('All Students','Girls','Boys' )) +
  theme_bw() +
...
- Additionally, We compare Teacher pedagogical skill and infrastructure for Evening versus Non-Evening schools.  
- Gaps between Evening and Non-Evening schools tend to be larger than the Urban/Rural divide.
...
pedgMedian <- summarise(group_by(pedg_evening, Type), MD = round(weighted.median(teach_score, ipw, na.rm=T),1))
  ...
  labs(x='Teacher Pedagogical Skill',
       y='Density')+
  ggtitle("Teacher Pedagogy (Scale 1-5) in Evening/Non-Evening School")
...
inpt_evening <- evening_input %>%
  ...
  ggplot(aes(x=inputs))  +
    ...
    theme_bw() +
  labs(x='Input Index')+
  ggtitle("Input Across Evening/Non-Evening Schools")
...
- The South region has teacher content knowledge proficiency levels around 16 points higher than in the North.
...
#     plot_annotation(title = str_wrap('Differences in Dashboard Indicators Between Urban/Rural Schools',80))
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Sub-Indicators - Jordan 2019 - Evening Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
          , "Mean", "95% Confident Interval", "Ratio of Non-Evening to Evening"),
          digits = 2, 
          escape=F) %>%
  ...
  sumstats_df %>%
    ...
    # )
    # ) %>%
    kable(caption = "Summary Statistics of Dashboard Sub-Indicators - Jordan 2019 - Region",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
          , "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"),
          digits = 2, 
          escape=F) %>%
