---
title: "Global Education Policy Dashboard - Jordan 2019"
...
Roadmap:  
- Below is a set of tables and charts containing findings for the Jordan 2019 Global Education Policy Dashboard survey. - We will start with breakdowns of our Practice indicators  
...
work_dir<- "C:/Users/wb577189/OneDrive - WBG/GEPD/CNT/JOR/JOR_2019_GEPD/JOR_2019_GEPD_v01_M/Data/"
...
## Main Indicator Tables by Urban/Rural, Public/Private, Evening/Non-Evening, Regions, Owned/Rented School Property
...
# School Survey
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Jordan 2019 - Urban/Rural",
...
#public/private
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Jordan 2019 - Public/Private Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
...
#     plot_annotation(title = str_wrap('Differences in Dashboard Indicators Between Urban/Rural Schools',80))
      ...
      #Now do breakdown by non-syrian/syrian
      #non-syrian
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Jordan 2019 - Syrian Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
          , "Mean", "95% Confident Interval", "Ratio of Non-Syrian to Syrian"),
          digits = 1, 
          escape=F) %>%
    kable_styling("striped") %>%
    add_header_above(c(" "=1, "Overall"=2,"Non-Syrian"=2,"Syrian"=2, " "=1))
      ...
      sumstats_school_north <- school_dta_short_anon %>%
        filter(territory=="North" & Syrian_school==FALSE) 
      ...
      sumstats_school_middle <- school_dta_short_anon  %>%
        filter(territory=="Middle" & Syrian_school==FALSE) 
      ...
      sumstats_school_south <- school_dta_short_anon  %>%
        filter(territory=="South" & Syrian_school==FALSE) 
  ...
  sumstats_df %>%
    ...
    # )
    # ) %>%
    kable(caption = "Summary Statistics of Dashboard Indicators - Jordan 2019 - Region (excludes Syrian Schools)",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
...
#rented/owned
      ...
      #Now do breakdown by non-evening/Rural
      #non-evening
      sumstats_school_nonrented <- school_dta_short_anon %>%
        filter(property_type!="Tenant") 
      
      
      sch_ipw<-sumstats_school_nonrented$ipw 
      
      sumstats_school_nonrented <- sumstats_school_nonrented %>%
        select(one_of(indicators_list)) 
      
      
      
      sumstats_school_nonrented_df<-my_skim(sumstats_school_nonrented) %>%
        yank("numeric") %>%
        mutate(variable=skim_variable) %>%
        select(variable, mean, sd, p0, p25, p50, p75, p100, complete,  hist) 
      ...
      #add variable label
      sumstats_school_nonrented_df <- sumstats_school_nonrented_df %>%
        mutate(name=variable,
               indicators=variable) %>%
        left_join(labels_df_2) %>%
        mutate(varlabel=indicator_labels) %>%
      ...
      #rented
      sumstats_school_rented <- school_dta_short_anon  %>%
        filter(property_type=="Tenant") 
      
      
      sch_ipw<-sumstats_school_rented$ipw 
      
      sumstats_school_rented <- sumstats_school_rented %>%
        select(one_of(indicators_list)) 
      
      
      sumstats_school_rented_df<-my_skim(sumstats_school_rented) %>%
        yank("numeric") %>%
        mutate(variable=skim_variable) %>%
        select(variable, mean, sd, p0, p25, p50, p75, p100, complete,  hist) 
      ...
      #add variable label
      sumstats_school_rented_df <- sumstats_school_rented_df %>%
        mutate(name=variable,
               indicators=variable) %>%
        left_join(labels_df_2) %>%
        mutate(varlabel=indicator_labels) %>%
      ...
      #now bind urban/rural with the main results
      sumstats_school_df_final <- sumstats_school_df %>%
        left_join(sumstats_school_nonrented_df) %>%
        left_join(sumstats_school_rented_df)
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Indicators - Jordan 2019 - Owned/Rented Property Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
          , "Mean", "95% Confident Interval", "Ratio of Owned to Rented"),
          digits = 1, 
          escape=F) %>%
    kable_styling("striped") %>%
    add_header_above(c(" "=1, "Overall"=2,"Owned"=2,"Rented"=2, " "=1))
...
- Next we offer breakdowns by whether or not the school survey took place at a Syrian School
...
learning_data_plot2 <- final_indicator_data_LERN_anon %>%
  ...
  select(-Female, -Male) %>%
  mutate(Type=if_else(Syrian_school==TRUE, "Syrian School", "Non-Syrian School"))


learning_dataMedian <- summarise(group_by(learning_data_plot2, Gender , Type), MD = round(weighted.median(student_knowledge, ipw, na.rm=T),1))

  ggplot(learning_data_plot2, aes(x=Type, y=student_knowledge, fill=Gender)) + #now plot the output in a bar graph
    geom_boxplot(aes(weight=ipw), position="dodge") +
    geom_text(data = learning_dataMedian, aes(x=Type, y=MD, label=MD), 
              position = position_dodge(width = 0.8), size = 3, vjust = 1) +
  ggtitle("4th Grade Student Assessment Scores by Gender and Syrian School Status") +
  xlab(str_wrap("Syrian School Status", width=75))+
  ylab(str_wrap("Fraction Correcton on 4th Grade Assessment",35)) +
  scale_fill_discrete(name="Gender", 
                      labels=c('All Students','Girls','Boys' )) +
  theme_bw() +
...
- Additionally, We compare Teacher pedagogical skill and infrastructure for Syrian versus Non-Syrian schools.  
- Gaps between Syrian and Non-Syrian schools tend to be larger than the Urban/Rural divide.

```{r evening, echo=FALSE}


evening <- school_dta_short_anon %>%
  mutate(Type=if_else(Syrian_school==TRUE, "Syrian School", "Non-Syrian School"))



pedg_evening <- evening %>%
  group_by(Type) %>%
  mutate(weight=ipw/sum(ipw)) %>%
  ungroup()

pedgMedian <- summarise(group_by(pedg_evening, Type), MD = round(weighted.median(teach_score, ipw, na.rm=T),1))


  
  ggplot(pedg_evening,aes(x=Type,y=teach_score, fill=Type))  +
  geom_boxplot(aes(weight=ipw), position="dodge")+
  geom_text(data = pedgMedian, aes(x=Type, y=MD, label=MD), 
            position = position_dodge(width = 0.8), size = 3, vjust = 1)  +
  theme_bw() +
    scale_fill_discrete(name="Type")+
  labs(x='Teacher Pedagogical Skill',
       y='Density')+
  ggtitle("Teacher Pedagogy (Scale 1-5) in Syrian/Non-Syrian School")
  

```


```{r evening_input, echo=FALSE}


evening_input <- school_dta_short_anon %>%
  mutate(Type=if_else(Syrian_school==TRUE, "Syrian School", "Non-Syrian School"))



inpt_evening <- evening_input %>%
  group_by(Type) %>%
  mutate(weight=ipw/sum(ipw)) %>%
  ungroup() %>%
  ggplot(aes(x=inputs))  +
    ...
    theme_bw() +
  labs(x='Input Index')+
  ggtitle("Input Across Syrian/Non-Syrian Schools")
...
```{r 1st_grade_plot, echo=FALSE, fig_height=8, fig_width=9}
 #############################
    ...
    ##############################
      ...
      regplots2 +
          plot_annotation(title = str_wrap("Scatterplot of School Average 4th Grade Student Scores against Teacher Pedagogy Scores",90))
...
kg_df <- ecd_dta_anon %>%
  ...
  mutate(attend_kg_num = case_when(
    m6s1kg==1 ~ as.numeric(1),
    m6s1kg==0 ~ as.numeric(0),
    TRUE ~ as.numeric(NA)
  )) %>%
...
#overall
kg_dfMedian <- summarise(group_by(kg_df,  attend_kg), MD = round(weighted.median(ecd_student_knowledge, ipw, na.rm=T),1))

  ggplot(kg_df, aes(x=attend_kg, y=ecd_student_knowledge, fill='#0081a8')) + #now plot the output in a bar graph
    geom_boxplot(aes(weight=ipw), position="dodge") +
...
#literacy
  kg_dfMedian <- summarise(group_by(kg_df,  attend_kg), MD = round(weighted.median(ecd_literacy_student_knowledge, ipw, na.rm=T),1))

  p1<-ggplot(kg_df, aes(x=attend_kg, y=ecd_literacy_student_knowledge, fill='#0081a8')) + #now plot the output in a bar graph
    geom_boxplot(aes(weight=ipw), position="dodge") +
    geom_text(data = kg_dfMedian, aes(x=attend_kg, y=MD, label=MD), 
              position = position_dodge(width = 0.8), size = 3, vjust = 1) +
  xlab(str_wrap("Kindergarten Status", width=75))+
...
#math
    kg_dfMedian <- summarise(group_by(kg_df,  attend_kg), MD = round(weighted.median(ecd_math_student_knowledge, ipw, na.rm=T),1))

  p2<-ggplot(kg_df, aes(x=attend_kg, y=ecd_math_student_knowledge, fill='#0081a8')) + #now plot the output in a bar graph
    geom_boxplot(aes(weight=ipw), position="dodge") +
    geom_text(data = kg_dfMedian, aes(x=attend_kg, y=MD, label=MD), 
              position = position_dodge(width = 0.8), size = 3, vjust = 1) +
  xlab(str_wrap("Kindergarten Status", width=75))+
...
#executive functioning
    kg_dfMedian <- summarise(group_by(kg_df,  attend_kg), MD = round(weighted.median(ecd_exec_student_knowledge, ipw, na.rm=T),1))

  p3<-ggplot(kg_df, aes(x=attend_kg, y=ecd_exec_student_knowledge, fill='#0081a8')) + #now plot the output in a bar graph
    geom_boxplot(aes(weight=ipw), position="dodge") +
    geom_text(data = kg_dfMedian, aes(x=attend_kg, y=MD, label=MD), 
              position = position_dodge(width = 0.8), size = 3, vjust = 1) +
  xlab(str_wrap("Kindergarten Status", width=75))+
  ...
  theme_bw() +
    theme(legend.position = "none") 
#socio-emotional  
  kg_dfMedian <- summarise(group_by(kg_df,  attend_kg), MD = round(weighted.median(ecd_soc_student_knowledge, ipw, na.rm=T),1))

  p4<-ggplot(kg_df, aes(x=attend_kg, y=ecd_soc_student_knowledge, fill='#0081a8')) + #now plot the output in a bar graph
    geom_boxplot(aes(weight=ipw), position="dodge") +
    geom_text(data = kg_dfMedian, aes(x=attend_kg, y=MD, label=MD), 
              position = position_dodge(width = 0.8), size = 3, vjust = 1) +
  xlab(str_wrap("Kindergarten Status", width=75))+
...
- Regions (North, Middle, South) differ in terms of their service delivery and learning outcomes.  
- This is particularly true for teacher effort and skill.  
- Below we plot teacher content knowledge by region to highlight some of these differences (Syrian schools are excluded).  
- The South region has teacher content knowledge proficiency levels around 13 points higher than in the North.
...
#public/private
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Sub-Indicators - Jordan 2019 - Public/Private Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
          , "Mean", "95% Confident Interval", "Ratio of Public to Private"),
          digits = 2, 
          escape=F) %>%
...
#     plot_annotation(title = str_wrap('Differences in Dashboard Indicators Between Urban/Rural Schools',80))
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Sub-Indicators - Jordan 2019 - Syrian Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
          , "Mean", "95% Confident Interval", "Ratio of Non-Syrian to Syrian"),
          digits = 2, 
          escape=F) %>%
  ...
  sumstats_df %>%
    ...
    # )
    # ) %>%
    kable(caption = "Summary Statistics of Dashboard Sub-Indicators - Jordan 2019 - Region (excludes Syrian Schools)",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
          , "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"),
          digits = 2, 
          escape=F) %>%
...
#rented/owned
  ...
  sumstats_df %>%
    ...
    kable(caption = "Summary Statistics of Dashboard Sub-Indicators - Jordan 2019 - Owned/Rented Property Schools",
          col.names = c("Indicator", "Mean", "95% Confident Interval", "Mean", "95% Confident Interval"
          , "Mean", "95% Confident Interval", "Ratio of Owned to Rented"),
          digits = 2, 
          escape=F) %>%
